...
VIDEO_SUFFIX = {'.mp4', '.mkv', '.avi', '.flv', '.hevc', '.wmv'}
assert '.jpg' not in VIDEO_SUFFIX
COMMON_SUFFIX = {'.jpg', '.nfo', '.ass', '.gif', '.json', '.html', '.iso', '.png'}
...
def thumb_nail(
    ...
    if save_path.exists():
        if save_path.stat().st_size != 8 and not force:
    ...
    with tqdm.tqdm(total=len(frames_idx)) as t:
        ...
        for idx in frames_idx:
            ...
            res_map[row * resize_height:(row + 1) * resize_height, col * resize_width:(col + 1) * resize_width] = bgr
            col += 1
            if col >= map_width:
                col = 0
                row += 1
...
def thumb_from_dir(start_path: str = None, video_suffix: Set[str] = None) -> bool:
    force = check_force()
    if not video_suffix:
