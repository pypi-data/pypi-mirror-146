...
from typing import cast, Optional, TextIO, Type
...
class SubcommandProtocol(Protocol):
    @staticmethod
    def main(
        conninfo: Connection, credentials: Optional[Credentials], __args: argparse.Namespace
    ) -> None:
        pass
...
def run_subcommand(
    ...
    if issubclass(subcommand, HelpCommand):
        # qq help commands are not REST wrappers, and therefore do not need conninfo or credentials
        help_cmd = cast(HelpCommandProtocol, subcommand)
        help_cmd.main(args)
    else:
        sub_cmd = cast(SubcommandProtocol, subcommand)
        sub_cmd.main(conninfo, credentials, args)
