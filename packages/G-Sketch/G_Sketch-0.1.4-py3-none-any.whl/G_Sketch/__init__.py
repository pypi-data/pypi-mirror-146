...
class Sketch:
    ...
    def __init__(self, img_name):
        ...
        self.set_min_distance(1)

        for row in self.coords:
            for coord in row:
                if self.is_edge(coord):

                    if not coord.get_checked():
                        body = Body([])
    ...
    def set_min_distance(self, min_dist):
        ...
        for i in range(len(self.bodies)):
            ...
            for pixel in self.bodies[i].outline_pixels:
                ...
                if min_dist < distance(new_pixels[-1].get_coords(), pixel.get_coords()) and distance(
                        new_pixels[-1].get_coords(), pixel.get_coords()) < min_dist + 5:
    ...
    @staticmethod
    def is_white(pixel):
        return not (pixel[0] == pixel[1] and pixel[0] == pixel[2] and pixel[0] == 0)
