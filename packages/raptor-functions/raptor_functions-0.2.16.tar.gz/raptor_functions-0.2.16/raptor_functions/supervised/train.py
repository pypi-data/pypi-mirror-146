...
def objective(trial, df, study_name, model_mode):
        ...
        fc_parameters = settings.from_columns(features)

        # remove unwanted characters from the column names
        df.columns = df.columns.str.replace('[",\d, (, )]', '')


        clf = setup(df, target=TARGET_COL, silent=True, log_plots=True, experiment_name=study_name, preprocess=False)
        ...
        if model_mode == 'random':
            ...
            signature = infer_signature(X, predict_model(model, data=X))
        ...
        try:
            ...
            mlflow.set_tag('fc_parameters', fc_parameters)
...
def train_experiments(df, study_name='raptor', direction='maximize', model_mode='random', n_trials=5):
    ...
    study.optimize(lambda trial: objective(trial, df, study_name, model_mode), n_trials=n_trials)

    print('Click on this link to track experiments: ',
        REMOTE_TRACKING_URI
        )
