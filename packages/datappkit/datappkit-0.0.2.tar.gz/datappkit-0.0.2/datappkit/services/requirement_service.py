...
class RequirementService:
    ...
    def get(self, requirement_id):
        ...
        if res['code'] == 10000:
            data = {
                ...
                "state_desc": res['data']['state_object']['desc'],
                "create_time": res['data']['created_at'],

                "name": res['data']['title'],
                "label_type_id": res['data']['task_type']['id'],
                "label_type_name": res['data']['task_type']['name'],
                "estimated_count": res['data']['estimated_count'],
                "start_time": res['data']['expect_start_at'],
                "end_time": res['data']['expect_end_at'],
                # todo 需要转换为bpp统一的值：1公开、2内部、3秘密、4绝密-低级、5绝密-高级（现在的值是：7内部、
                "security_level_id": res['data']['security_level'],
                "payer_group_id": res['data']['payer_group_id'],
                "data_manager": res['data']['data_manager_id'],
                "researcher_id": res['data']['researcher_id'],
    ...
    def get_list(self, page, per_page):
        ...
        if res['code'] == 10000:
            ...
            res['data'] = {'pagination': res['data']['pagination'], 'items': re_list}
        return res
