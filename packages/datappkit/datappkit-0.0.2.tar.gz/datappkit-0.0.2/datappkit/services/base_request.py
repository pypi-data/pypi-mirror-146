...
class BaseRequest(RequestEncodingMixin):  # noqa: WPS338
    ...
    def request(self) -> Dict[str, Any]:
        ...
        if not config.authorization() and "login" not in self._uri:
            message['code'] = 10401
            message['error_message'] = "please log in"
            message['data'] = {}
        ...
        if res.status_code // 100 == 2:
            ...
            if "login" in self._uri:
                config.set_authorization("JWT {}".format(res.json()['jwt']))
            message['code'] = 10000
            message['error_message'] = ""
            message['data'] = res.json()
            return message

        message['code'] = res.status_code + 10000
        try:
            message['error_message'] = res.json()['message']
        except (ValueError, KeyError):
            message['error_message'] = res.text
            message['data'] = {}
        return message
