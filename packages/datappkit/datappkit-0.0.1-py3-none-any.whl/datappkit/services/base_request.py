...
class BaseRequest(RequestEncodingMixin):  # noqa: WPS338
    ...
    def _headers(self):
        ...
        if config.JWT and "login" not in self._uri:
    ...
    def request(self) -> Dict[str, Any]:
        message = {}
        if not config.JWT and "login" not in self._uri:
            message['code'] = 401
            message['text'] = "please log in"
            raise DatappAPIClientException(message)
        res = self._request()
        # if config.debug():
        if True:
        ...
        if res.status_code // 100 == 2:
            if res.status_code == 204:  # noqa: WPS432
                return {}
            if "login" in self._uri:
                config.JWT = "JWT {}".format(res.json()['jwt'])
            return res.json()

        message['code'] = res.status_code
        try:
            message['text'] = res.json()['message']
        except (ValueError, KeyError):
            message['text'] = res.text
        if res.status_code // 100 == 4:
            raise DatappAPIClientException(message)
        elif res.status_code // 100 == 5:
            raise DatappAPIServicesException(message)
        else:
            raise DatappAPIUnknownException(message)
