...
import bancorml
from bancorml.utils import get_logger


def print_info():
    """Prints information about the system, bancorml, and dependencies of bancorml."""
    logger = get_logger(__name__)
    logger.info("BancorML version: %s" % bancorml.__version__)
    logger.info("BancorML installation directory: %s" % get_bancorml_root())
    print_sys_info()
    print_deps()
...
def get_bancorml_root():
    """Gets location where bancorml is installed.

    Returns:
        Location where bancorml is installed.
    """
    return os.path.dirname(bancorml.__file__)
