...
class EnvBase(ABC):
    ...
    def check_pool_shutdown(self, warning_only=True):
        ...
        for tkn in self.whitelisted_tokens:
            bnt_per_tkn = self.get_tkn_over_bnt_internal_rate(tkn)
            try:
                if self.vault.ledger[tkn][-1] < 0 or self.available_liquidity.ledger[tkn][-1] < 0 or self.staking.ledger[tkn][-1] < 0 or self.external_protection_wallet.ledger[tkn][-1] < 0:
                    if not warning_only:
                        raise UserWarning(f'Error. {tkn} balance cannot be less than zero. {self.vault.ledger[tkn][-1]}')
            except:
                pass

            vault_tkn = self.validate_type(self.vault.ledger[tkn][-1])
            bnt_per_tkn = self.validate_type(bnt_per_tkn)
            vault_bnt = self.tokenomics.handle_multiplication(vault_tkn, bnt_per_tkn)
