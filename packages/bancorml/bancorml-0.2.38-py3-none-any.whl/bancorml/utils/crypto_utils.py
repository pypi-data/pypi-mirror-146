...
def convert_to_fixedpoint(val):
    return int(Decimal(val) * 10 ** 18)


def is_solidity_converted(val, src=None):
    if not isinstance(val, Fraction):
        return (Decimal(val) / (10 ** 18)) >= .9999999
    else:
        return True
...
def getProtocols():
    """Get list all DeFi components2 across all blockchains
...
def gecko_list(page=1, per_page=250):
...
def get_gecko_spot_price(tokens, quote):
...
def getGeckoIDs():
    ...
    for i in range(20):
        ...
        ids_list = ids_list + gecko_list(page=i, per_page=250)['id'].tolist()
    return ids_list
