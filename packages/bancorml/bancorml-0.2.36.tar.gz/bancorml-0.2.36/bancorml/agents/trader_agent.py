...
import random
import pandas as pd
from bancorml.utils import convert_to_fixedpoint
...
class TraderAgent(AgentBase):
    ...
    def trade(self, tkn1, x, tkn2, fake_wallet=True, block_num=0, update_liquidity=False):
        '''
        ...
        for key in self.wallet:
            ...
            if tkn1 == key:
                trade_output = floor(self.protocol.trade(tkn1, x, tkn2, block_num, update_liquidity))
                self.wallet[tkn1][-1] -= x
                self.wallet[tkn2][-1] += trade_output
