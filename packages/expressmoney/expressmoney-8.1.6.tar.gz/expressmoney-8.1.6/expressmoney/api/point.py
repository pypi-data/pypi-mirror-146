...
class PaginationError(PointError):
    pass
...
class Point:
    ...
    def _query_params(self) -> dict:
        if self._is_async:
            SyncOnly('Query params only for sync queries.')
    ...
    def _get_page_data(self, url=None) -> list:
        ...
        if self._cache is not None or self._pagination_pages is None:
            ...
            if not isinstance(page_data, list):
                raise PaginationError('Endpoint pagination enable.')
        else:
            if isinstance(get_data, list):
                raise PaginationError('Endpoint pagination disable.')
