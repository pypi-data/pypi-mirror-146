...
struct compare_indirect_index
 ...
 extern "C" double OWA(int n, double x[],double w[])
...
extern "C" double OWASorted(int n, double x[],double w[])
 { /*  sorting is needed in general */
	 int *indices= new int [n];
  //  int indices[n];
	::iota( indices, indices+n, 0 );  // found in <numeric>
	sort( indices, indices+n, compare_indirect_index <double *> ( x ) );
...
extern "C" double weightedf(double x[], double p[], double w[], int n,
...
extern "C" void weightedOWAQuantifierBuild( double p[], double w[], int n, double temp[], int *T) // NH changed &T to *T
/*
 ...
 */
{
	...
	*T= BuildMonotonSpline(x,y,n+1,h,alpha,beta,gamma,ttemp); // NH changed T to *T
...
extern "C" double weightedOWAQuantifier(double x[], double p[], double w[], int n, double temp[], int T)
/*
...
#define MEPS 10e-12
extern "C" double ImplicitWOWA(double x[], double p[], double w[], int n )
/*
...
*/
{
 ...
 // int indices[n];
  ::iota( indices, indices+n, 0 );  // found in <numeric>
  sort( indices, indices+n, compare_indirect_index <double *> ( p ) );
  ...
  ::iota( indices, indices+n, 0 );  // found in <numeric>
  sort( indices, indices+n, compare_indirect_index <double *> ( px) );
...
/* prepare the multiplicites based on the weights w */
	...
	INT N[n]; /* multiplicities of x based on the weights */
	for(int i=0;i<n-1;i++)
	{
	   	N[i]=w[i]*C+0.5; /**/
		t+=N[i];
	...
	return node_RWAnT(x,N,L,k,F);
}

extern "C" double WAn(double * x, double * w, int n, int L, double(*F)( double, double))
{
