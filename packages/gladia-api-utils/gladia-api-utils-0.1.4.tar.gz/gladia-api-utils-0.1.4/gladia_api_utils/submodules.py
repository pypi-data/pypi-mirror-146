        ...
        async def apply(*args, **kwargs):
            ...
            if os.path.isfile(os.path.join(module_path, 'env.yaml')):
                if not os.path.isdir(os.path.join(module_path, '.venv')):
                    os.system("python3 setup_custom_envs.py")
                ...
                for input in inputs:
                    if input["type"] in ["image", "sound", "video"]:
                        kwargs[input["name"]] = write_tmp_file(kwargs[input["name"]])

                output_tmp_result = tempfile.NamedTemporaryFile().name
                sync = False
                cmd = f"""cd /app/{module_path} && \
pipenv run python3 -c "import importlib
this_module = importlib.import_module('{model}')
