...
from ..common.constants import SAMPLING_RATE, FRAME_SIZE, VOCAL_GROUPS, SATURATION_LEVEL
from ..common.log import logger
from ..common.singleton import register_process, unregister_process
from ..common.utils import my_assert, to_gain, remove_non_ascii
    ...
    def _processing(self):
        ...
        if self.rec:
            if self.cursor == -1:  # recording complete
                latency = calculate_latency(self.out_signal[:self.weep_len].mean(axis=1), self.in_signal * self.gain)
    ...
    def _level_updater(self):
        ...
        while True:
            ...
            if isinstance(sample, int):  # stream ended
                smoother = None
                self.monitor.update_level(0)
            else:  # actual numpy array
                if smoother is None:
                    smoother = SignalSmoother(0)
                    self._stream_active()
                peak = np.abs(sample).max()
                if peak * self.gain >= SATURATION_LEVEL:
                    level = 10
                else:
                    dbs = SignalLevel.calculate_dbs(sample * self.gain)
                    level = max(1, min(9, int(round(dbs / 2 + 10))))
                level = smoother(level)
                if level is not None:
                    self.monitor.update_level(level)
...
class SlaveClientGui:
    def __init__(self, save_config=None):
        ...
        self.monitor = tk.Canvas(master=self.root, width=20, height=20)
        self.monitor.grid(row=1, column=2, padx=10, pady=10)
        self.monitor_tag = self.monitor.create_oval(10, 10, 20, 20, fill='white')
    ...
    # this is to be called from another thread
    def set_signal_level(self, level: SignalLevel):
        if self.state == GuiState.DEAD:
            return  # ignore post-mortem commands
        if level == SignalLevel.white:  # no delay in white
            self.monitor.itemconfigure(self.monitor_tag, fill=level.name)
            self.signal_smoother.reset()
            return
        level = self.signal_smoother(level)
        if level is not None:
            logger.debug(f"Changing level to {level.name}")
            self.monitor.itemconfigure(self.monitor_tag, fill=level.name)
