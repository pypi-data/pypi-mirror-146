...
import pandas, numpy, inspect, re, string
...
class SklearnPredictor:
  ...
  def makeDeployment(self):
    ...
    varName = "skpredictor"
    if self.name: varName = self.name
    ...
    if guessedArgCount:
      ...
      funcSource = "\n".join([
        f"def predict({argsWithTypes}):",
        f"  return float({varName}.predict([[{', '.join(argNames)}]])[0])",
        f""
      ])
    else:
      funcSource = "\n".join([
        f"def predict(*args: Any):",
        f"  return float({varName}.predict([args])[0])",
        f""
      ])
