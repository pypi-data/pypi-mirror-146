...
import logging
from typing import Union, List, Optional, Dict, Any, Callable, Awaitable
...
TEXT_ERROR_HTTP_HEADERS = {'Content-Type': 'text/plain; charset=UTF-8'}


def status_generic(status: int, body: Optional[Union[bytes, str]] = None, headers: Optional[Dict[str, str]] = None) -> web.Response:
    """
...
def status_not_found(body: Optional[Union[bytes, str]] = None) -> web.Response:
    """
    ...
    """
    body_ = body.encode() if isinstance(body, str) else body
    headers = TEXT_ERROR_HTTP_HEADERS if isinstance(body, str) else None
    return web.HTTPNotFound(body=body_, headers=headers)
...
def status_ok(body: Optional[Union[bytes, str]] = None, content_type: str = 'application/json') -> web.Response:
    """
    ...
    """
    body_ = body.encode() if isinstance(body, str) else body
    if isinstance(body, str) and content_type is None:
        return web.HTTPOk(headers=TEXT_ERROR_HTTP_HEADERS, body=body_)
    elif content_type is not None:
        return web.HTTPOk(headers={'Content-Type': content_type}, body=body_)
    else:
        return web.HTTPOk(body=body_)
...
async def get_multiple_choices(request: web.Request, result: Union[DesktopObjectDict, List[DesktopObjectDict]]) -> web.Response:
    """
