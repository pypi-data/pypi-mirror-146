...
def run_core(
    ...
    **fit_kwargs,
    ...
    """
    CS, CU_params, TM, TM_params, mode_opts = prepare_model_objects(dresser, fit_TM, fit_mode, init_set, telescope_db)
...
def prepare_model_objects(
    ...
    CU_params = CUModelParams(numx, numy, numl, dresser.numdrawers, dresser.nummod,
                                   fit_mode=fit_mode, init_set=init_set,
                                   header=dresser.drawers[0].data_list[0][0].header)
    TM_params = TelescopeModelParams(numx, numy, numl, header=dresser.drawers[0].data_list[0][0].header)
            ...
            for k in range(numl):
                ...
                try:
                    ...
                    if 'NaN values detected' in str(e):
                        logging.info('NaN values detected either in input or fitting calculations. Assigning garbage results.')
                        fit_out = None
                        TMf = None
                        minimizer = None
                    else:
...
def multiproc_drawer_fit(Dresser, TM, CS, mode_opts, CU_params, TM_params, numx, numy, numl, numthread, pickle_path,
                         super_name='', **kwargs) -> Tuple[CUModelParams, TelescopeModelParams, TelescopeModel, np.ndarray]:
...
def fit_parameters(
    ...
    **fit_kwargs,
    ...
    logging.info(f'minimization completed in {time.time() - t1:4.1f} s. Chisq = {fit_out.chisqr:8.2e}, redchi = {fit_out.redchi:8.2e}')
