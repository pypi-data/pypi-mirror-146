...
def cli():
    ...
    ** How to use **
    ...
    Create sonar scannr properties file:
...
def workflow_template(action: str):
    ...
    if action not in valid_actions:
        print(f"Can't determine file type {action}. Please use one of: 'create', 'delete', 'instantiate'")
        return
...
@cli.command()
def create_dag():
    """! Create a DAG which will instantiate the workflow template.
    It will start as paused and will be set up for a daily activation
    """
    current_folder = os.path.abspath(os.getcwd())

    os.environ["PYTHONPATH"] = current_folder
    dag_folder = current_folder + '/batch_files/workflow_dag/creating_dag.py'
    basic_command = 'python ' + dag_folder

    print(f"Attempting to create a DAG")
    subprocess.call(basic_command, shell=True)
