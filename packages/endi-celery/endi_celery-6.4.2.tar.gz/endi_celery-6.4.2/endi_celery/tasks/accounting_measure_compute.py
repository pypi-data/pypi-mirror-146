...
def get_upload_from_id(upload_id):
...
def compile_measures_task(self, upload_id, grid_type=None):
    ...
    transaction.begin()
    upload = get_upload_from_id(upload_id)

    operations = None
    if grid_type == "treasury":
        operations = _collect_operations(upload_id, upload.filetype, upload.date)
    else:
