...
item* get_item(item* items, npy_intp size, const int key) {
...
npy_intp inner(int prio, char *vec, npy_intp step, npy_intp n, item d[], int dn){
    int sign = prio < 0 ? -1 : 1;
    prio = sign * prio;
    npy_intp p, res;
                            ...
                            npy_intp* steps, void* data)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        tmp = *(int64_t *)in;
        tmp = inner(tmp, vec, in_step, n, d, 0);
        *((int64_t *)out) = tmp;
