...
class HanLPClient(object):
    ...
    def keyphrase_extraction(
        ...
        scores = self._send_post_json(self._url + '/keyphrase_extraction', {
            'text': text, 'language': language or self._language, 'topk': topk,
        })
        scores = dict(sorted(scores.items(), key=lambda x: (x[1], x[0]), reverse=True))
        return scores
