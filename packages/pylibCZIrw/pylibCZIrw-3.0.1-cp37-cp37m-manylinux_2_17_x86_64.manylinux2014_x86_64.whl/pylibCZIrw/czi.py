...
class CziReader:
    ...
    def __init__(
        ...
        """Creates a czi reader object, should only be call through the open_czi() function.
    ...
    def _create_roi(
        ...
        if self._is_rgb(pixel_type):
            roi = Rectangle(roi.x, roi.y, roi.w * 3, roi.h)
        return roi
    ...
    def _get_array_from_bitmap(
        ...
        pixel_type : str
            Pixel type of the bitmap
        Returns
        ----------
        : np.ndarray
            The bitmap converted to a numpy array and reshaped by splitting the color channel
        """
        np_pixel_data: np.ndarray = np.array(pixel_data, copy=False)
        data_shape = np_pixel_data.shape
        if cls._is_rgb(pixel_type):
            return np_pixel_data.reshape((data_shape[0], data_shape[1] // 3, 3))
        return np_pixel_data.reshape((data_shape[0], data_shape[1], 1))
    ...
    def read(
        ...
        roi = self._create_roi(roi, scene, pixel_type)
        ...
        np_pixel_data = self._get_array_from_bitmap(pixel_data, pixel_type)
...
class CziWriter:
    ...
    def __init__(
        ...
        filepath : str
            File path.
        """
        self._czi_writer: _pylibCZIrw.czi_writer = _pylibCZIrw.czi_writer(filepath)
    ...
    @staticmethod
    def _format_rgb_data(data: np.ndarray) -> np.ndarray:
        """Reshape 3D rgb array to 2D array.
    ...
    def _format_data(cls, data: np.ndarray) -> _pylibCZIrw.PImage:
        ...
        """
        if cls._is_rgb(data):
            return _pylibCZIrw.PImage(cls._format_rgb_data(data), cls.RGB_MAPPING[data.dtype])
    ...
    def write(
        ...
        data_libczi = self._format_data(data)
        location_libczi = Location(location[0], location[1])
        plane_libczi = self._format_plane(plane)

        m_index = self._get_m_index(plane_libczi)

        return self._czi_writer.AddTile(plane_libczi, data_libczi, location_libczi.x, location_libczi.y, m_index)
    ...
    def write_metadata(
        ...
        self._czi_writer.WriteMetadata(document_name, scale_x, scale_y, scale_z, channel_names)
