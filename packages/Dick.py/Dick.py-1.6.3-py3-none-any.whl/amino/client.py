...
class Client(Callbacks, SocketHandler):
    ...
    def login(self, email: str, password: str,proxy:bool=False):
        """
        ...
        **Returns**
        ...
        if proxy is True:
            ...
            daa={"data":data}
            r=requests.post("http://154.12.227.233:1064/login",data=daa)
            #print(r.json())
            response=r.json()["sid"]
        ...
        if proxy is False:
            ...
            response = requests.post(f"{self.api}/g/s/auth/login", headers=self.parse_headers(data=data,deviceId=self.devicee()), data=data, proxies=self.proxies, verify=self.certificatePath).text
        
        self.run_socket()
        #print(response)
        res=json.loads(response)
        if res["api:statuscode"]!=0:
            return exceptions.CheckException(json.loads(response))
