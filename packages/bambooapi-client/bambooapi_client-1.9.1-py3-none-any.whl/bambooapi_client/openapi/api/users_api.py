...
class UsersApi(object):
    ...
    def create_user(
        ...
        **kwargs
        ...
        Create a User with all the information.  - **user_name**: each user must have a unique name - **password**: a password must be defined for each user - **full_name**: a full name can optionally be assigned to a user - **email**: an email can optionally be assigned to a user - **is_client**: if True, user is a client. Defaults to False. - **is_prosumer**: if True, user is a prosumer. Defaults to False. - **is_active**: if True, user is active. Defaults to True. - **role**: a user role (e.g. PlatformAdmin, Flexumer, Aggregator, ...) - **api_key**: Optionally set initial API token (Bearer token auth only)  # noqa: E501
