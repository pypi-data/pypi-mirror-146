...
class ResourcesClient(base_client.BaseClient):
        ...
        with self.request("get", endpoint, stream=True) as ls_stream:
            ...
            for chunk in ls_stream.iter_lines(decode_unicode=True):
                try:
                    ...
                    message = f"Error decoding the ls response chunk: {e}"
    ...
    def tar_of_path(self, path):
        ...
        endpoint = url_path_join(RESOURCES_RESOURCE_URL, self.owner, COPY_RESOURCE_URL, *path.split(os.path.sep))
        ...
        with self.request("get", endpoint, headers=headers, stream=True) as response:
            ...
            if not content_type.startswith("application/x-tar"):
                raise ClientException(f"Unsupported Content-Type: '{content_type}'", response)

            with tarfile.open(fileobj=response.raw, mode="r|*") as tar:
                yield tar

    def download_resources(self, resources, dest, echo, quiet=False):
        """Download resources.

        Keyword arguments:
        resources -- a list of resource paths.
        dest -- download destination.
        echo -- a function for logging to stdout (the CLI and Python API have different loggers)
        quiet -- suppresses stdout output.
        """
        for resource in resources:
            with self.tar_of_path(resource) as tar:
                for file in tar:
                    if not quiet:
                        echo(f"Extracting {file.name}...")
