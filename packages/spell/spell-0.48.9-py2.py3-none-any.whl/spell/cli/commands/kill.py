...
def kill(ctx, run_ids, quiet):
    ...
    if len(run_ids) <= 0:
        ...
        with api_client_exception_handler():
            logger.info(f"Killing run {run_id}")
        ...
        with api_client_exception_handler():
            logger.info(f"Killing runs (batch) {run_ids}")
            data = client.kill_runs(run_ids)
        logger.info(f"Succesfully killed runs {data['successful_run_ids']}")
        if not quiet:
            if len(data["final_state_run_ids"]) > 0:
                click.echo(f"| Already finished: {' '.join([str(i) for i in data['final_state_run_ids']])}")
            if len(data["non_existent_run_ids"]) > 0:
                click.echo(f"| Run ID does not exist: {' '.join([str(i) for i in data['non_existent_run_ids']])}")
            if len(data["successful_run_ids"]) > 0:
                click.echo(f"| Successfully killed: {' '.join([str(i) for i in data['successful_run_ids']])}")
