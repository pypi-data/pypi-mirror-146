...
def stop(ctx, run_ids, quiet):
    ...
    if len(run_ids) <= 0:
        ...
        if not quiet:
            click.echo(f"Stopping run {run_id}. Use 'spell logs -f {run_id}' to view logs while the job finishes.")
    else:
        with api_client_exception_handler():
            data = client.stop_runs(run_ids)
        click.echo("Executing batch stop... Use 'spell logs -f RUN_ID' to view logs for a run while the job finishes.")
        if not quiet:
            if len(data["post_running_run_ids"]) > 0:
                click.echo(f"| Already finished: {' '.join([str(i) for i in data['post_running_run_ids']])}")
