...
def gke_update(ctx, credentials, cluster, gke_cluster_name, support_runs):
    ...
    if gke_cluster_name and not click.confirm(f"Are you sure you want to use {gke_cluster_name}?"):
...
def gke_configure_k8s(
    ctx,
    service_account,
    cluster,
    is_public,
    support_runs=False,
    run_namespace="spell-run",
    system_namespace="elastic-system",
):
    ...
    try:
        ...
        logger.error(f"Couldn't add GKE GPU driver Daemonset. Error was: {e}")
...
def get_bucket_with_permissions(ctx, storage_api, service_account, cluster_name, project_id):
    ...
    service_account_tag = f"serviceAccount:{service_account['email']}"
...
def get_region(compute_service, project):
    ...
    if region is not None:
        if click.confirm(
            f"All of this will be done within this project's region '{region}' - continue?",
            default=True,
        ):
            return region
...
def create_vpc(compute_service, cluster_name, project, region, spell_client):
    ...
    click.echo(f"Created a new VPC/network with name {cluster_name}!")
    ...
    # Add firewall rules
    click.echo(f"Creating firewall rule to allow ingress on ports {WORKER_OPEN_PORTS}...")
    trusted_cidrs = spell_client.get_trusted_cidrs()
    request = compute_service.firewalls().insert(
        project=project,
        body={
            "name": f"{cluster_name}-spell-api",
    ...
    click.echo(f"Created a new subnet {cluster_name} within network {cluster_name} in region {region}!")
...
def get_project(resource_service, project_id):
    ...
    if project_id is None or not click.confirm(
        f"All of this will be done within your project '{project_id}' - continue?",
        default=True,
    ):
        return click.prompt(
...
def get_zone_from_region(compute_service, project, region):
    ...
    request = compute_service.zones().list(project=project, filter=f'region = "{region_self_link}"')
    response = request.execute()
    if "items" not in response or len(response["items"]) == 0:
        raise ExitException(f"No compute zones found for region {region}")
...
def get_service_account(iam_service, resource_service, project):
    ...
    service_account_name = f"spell-access-{suffix}"
    service_account = (
        iam_service.projects()
        .serviceAccounts()
        .create(
    ...
    try:
        ...
        account_user_binding = {
            ...
            "members": [
                f"serviceAccount:{SPELL_SERVICE_ACCOUNT}",
                f"serviceAccount:{service_account_email}",
            ],
        }
        token_create_binding = {
            "role": "roles/iam.serviceAccountTokenCreator",
            "members": [f"serviceAccount:{SPELL_SERVICE_ACCOUNT}"],
        }
        ...
        raise ExitException(f"Unable to create and attach IAM policies. GCP error: {e}")

    suffix = str(random.randint(10 ** 6, 10 ** 7))
    role_name = f"SpellAccess_{suffix}"
    ...
    request = iam_service.projects().roles().create(parent=f"projects/{project}", body=create_role_request_body)
    response = request.execute()
    role_id = response["name"]

    click.echo(f"Assigning role {role_name} to service account {service_account_email}...")
    request = resource_service.projects().getIamPolicy(resource=project, body={})
    response = request.execute()

    response["bindings"].append({"members": [f"serviceAccount:{service_account_email}"], "role": role_id})
    ...
    click.echo(f"Successfully set up service account {service_account_email}")
...
def create_gke_cluster(
    ...
    try:
        ...
        subprocess.check_call(cmd)
        click.echo(f"Current gcloud user {gcloud_user} granted cluster-admin")
...
def gke_scale_nodepool(credentials, node_group, min_nodes, max_nodes):
    ...
    name_path = f"{config['parent']}/nodePools/{node_group.name}"
    ...
    try:
        ...
        raise ExitException(f"Error scaling node pool: {str(e)}")
    ...
    try:
        ...
        raise ExitException(f"Error retrieving node pool: {str(e)}")
    ...
    if resize_body is not None:
        ...
        try:
            ...
            raise ExitException(f"Error adjusting node pool size: {str(e)}")
...
def gke_delete_nodepool(credentials, node_group):
    ...
    try:
        ...
        raise ExitException(f"Error deleting node pool: {str(e)}")
...
def gke_add_nodepool(node_group_name, config_contents, gcp_service_account, gcp_creds):
    ...
    try:
        ...
        raise ExitException(f"Error creating node pool: {str(e)}")
...
def is_gs_bucket_public(bucket_name):
    ...
    query_str = "&".join(f"permissions={p}" for p in PERMISSIONS)
...
def get_container_registry_bucket_name(ctx, storage_service):
    ...
    for prefix in multi_region_prefixes:
        bucket_url = f"{prefix}artifacts.{storage_service.project}.appspot.com"
        try:
    ...
    if len(buckets) == 0:
        ...
        click.echo(
            ...
            + "\n".join([f"{i} : {b.location}" for i, b in enumerate(buckets)])
        )
...
def add_gs_bucket(spell_client, cluster, bucket_name):
    ...
    if bucket_name not in bucket_names:
        ...
        if b is None or not b.exists():
            raise ExitException(f"Bucket {bucket_name} doesn't exist.")

        if not is_gs_bucket_public(bucket_name):
            raise ExitException(
                f"Bucket {bucket_name} is neither publicly accessible nor part of project {project_id}."
            )
    ...
    service_account_tag = f"serviceAccount:{service_account_email}"
...
def set_custom_instance_service_acct(spell_client, cluster, custom_instance_service_acct):
    ...
    if custom_instance_service_acct:
        ...
        click.echo(f"Instance Service Account has been UNSET from cluster {cluster_name}!")
...
def update_gcp_cluster(ctx, credentials, cluster):
    ...
    if len(need_to_add) > 0 or len(need_to_remove) > 0:
        ...
        if len(need_to_add) > 0:
            ...
            role = iam_service.projects().roles().get(name=role_name).execute()
            click.echo(f"Successfully updated role {role_name}")
    ...
    if gs_service_acct_api_key:
        click.echo(f"Created a new API key for spell service account {service_account_id}")
    ...
    if current_version < 5:
        ...
        rule_name = f"{cluster['name']}-internal"
        body = {
            "name": rule_name,
            "description": "Allow traffic between all instances within VPC",
            "network": network_url,
...
def delete_gcp_cluster(ctx, cluster):
    ...
    # Delete Firewalls
    delete_firewall(compute_service, project_id, f"{cluster['name']}-spell-api")
    delete_firewall(compute_service, project_id, f"{cluster['name']}-internal")

    # Delete Subnets
    try:
        subnet_name = cluster["name"]
        region = cluster["networking"]["gcp"]["region"]
        click.echo(f"Deleting subnetwork {subnet_name}...")
        response = (
        ...
        click.echo(f"Deleted subnetwork {subnet_name}")
    except HttpError as err:
        if err.resp.status == 404:
            click.echo(f"subnetwork {subnet_name} is already deleted")
        else:
            raise ExitException(f"Error attempting to delete subnetwork {subnet_name}. Err was {err}")

    # Delete VPC
    try:
        vpc_name = cluster["networking"]["gcp"]["vpc"]
        click.echo(f"Deleting VPC {vpc_name}...")
        response = compute_service.networks().delete(project=project_id, network=vpc_name).execute()
        global_progress_bar(project_id, compute_service, response)
        click.echo(f"VPC {vpc_name} deleted")
    except HttpError as err:
        if err.resp.status == 404:
            click.echo(f"VPC {vpc_name} is already deleted")
        else:
            raise ExitException(f"Error attempting to delete VPC {vpc_name}. Err was {err}")
    ...
    for binding in iam_policies["bindings"]:
        if f"serviceAccount:{cluster['role_credentials']['gcp']['service_account_id']}" in binding["members"]:
    ...
    if binding_to_remove is None:
        ...
        click.echo(f"Found role {role_name} to delete, deleting")
        try:
            response = iam_service.projects().roles().delete(name=role_name).execute()
        except HttpError as err:
            if "already deleted" in err._get_reason():
                click.echo(f"Role {role_name} is already deleted")
            else:
                raise ExitException(f"Error attempting to delete role {role_name}. Err was {err}")
        # Delete binding
        click.echo(f"Deleting binding for role {role_name}...")
        ...
        click.echo(f"Deleted binding for role {role_name}")
    ...
    try:
        ...
        if err.resp.status == 404:
            click.echo(f"Service account {service_acct_name} is already deleted")
        else:
            raise ExitException(f"Error attempting to delete service account {service_acct_name}. Err was {err}")

    # Optionally delete the output bucket
    bucket_name = cluster["storage_uri"]
    if click.confirm(f"Delete bucket {bucket_name}? WARNING: This will delete all the data in the bucket"):
        try:
            storage_client = storage.Client(project=project_id)
            ...
            click.echo(f"Bucket {bucket_name} is already deleted!")
        except Exception as err:
            raise ExitException(f"Failed to delete bucket {bucket_name}.Err was {err}")
...
def delete_firewall(compute_service, project_id, firewall_name):
    ...
    try:
        click.echo(f"Deleting Firewall {firewall_name}...")
        response = compute_service.firewalls().delete(project=project_id, firewall=firewall_name).execute()
        global_progress_bar(project_id, compute_service, response)
        click.echo(f"Deleted Firewall {firewall_name}")
    except HttpError as err:
        if err.resp.status == 404:
            click.echo(f"Firewall {firewall_name} is already deleted")
        else:
            raise ExitException(f"Error attempting to delete Firewall {firewall_name}. Err was {err}")
...
def gke_delete_cluster(project_id, cluster):
    ...
    if not click.confirm(f"Delete model serving cluster {gke_name}?"):
        return
    click.echo(f"Deleting GKE cluster {gke_name}. This can take a while!")
    try:
        subprocess.check_call(cmd)
    except subprocess.CalledProcessError:
        click.echo(
...
def extract_gke_cluster_info(project_id, cluster_name):
    ...
    zone_clustername = cluster_name[len(f"gke_{project_id}_") :]
...
def global_progress_bar(project, compute_service, response):
    with click.progressbar(length=100, show_eta=False) as bar:
        ...
        if "error" in response and "errors" in response["error"]:
            ...
            raise GcpException(f"Error with GCP API {first_error['code']}: {first_error['message']}")
        bar.update(100)
