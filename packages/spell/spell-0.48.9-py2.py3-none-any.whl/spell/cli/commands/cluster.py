    ...
    def create_row(cluster):
        ...
        if provider == "AWS" and "custom_instance_profile_arn" in role_creds:
            ...
            if role_match is None or role_match.group(3) is None:
                logger.warning(f"PARSE ERROR! Cannot parse ARN {role_creds['custom_instance_profile_arn']}")
...
def set_instance_permissions(
    ...
    if cluster_type == "AWS":
        ...
        raise ExitException(f"Unknown cluster with provider {cluster_type}, exiting.")
...
def delete(ctx, cluster_name, profile, keep_vpc):
    ...
    if cluster_type == "AWS":
        ...
        raise Exception(f"Unknown cluster with provider {cluster_type}, exiting.")
