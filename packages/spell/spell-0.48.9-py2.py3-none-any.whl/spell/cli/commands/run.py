...
def run(
    ...
    **kwargs,
    ...
    if run.already_existed:
        click.echo(with_emoji("♻️", f"Idempotent: Found existing run {run.id}", utf8) + ellipses(utf8))
    else:
        click.echo(with_emoji("💫", f"Casting spell #{run.id}", utf8) + ellipses(utf8))
    if background:
        click.echo(f"View logs with `spell logs {run.id}`")
    else:
