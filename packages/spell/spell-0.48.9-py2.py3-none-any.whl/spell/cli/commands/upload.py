...
def upload(ctx, path, name, cluster_name, compress):
    ...
    if cluster_name:
        ...
        resume_command = f"spell upload {compress_param}--name '{name}' {path}"
    resume_message = f"To resume this upload, run: \n\n$ {resume_command}"
    ...
    try:
        ...
        raise ExitException(click.wrap_text(f"Your OS does not support multiprocessing: {e}."))
    ...
    if compress:
        click.echo(
            "Warning: Using compress for upload will require additional " "disk space to store the compressed dataset."
        )
    ...
    click.echo(f"Total upload size: {prettify_size(total_bytes)}")

    # create thread to update progress
    def update_progress():
        with click.progressbar(length=total_bytes, label=f"Uploading to uploads/{name}") as bar:
            while True:
                item = q.get()
    ...
    try:
        ...
        raise ExitException(f"Upload aborted. {resume_message}")
    except UnauthorizedRequest:
        raise ExitException(f"Login session has expired. Please log in again ('spell login'). {resume_message}")
    except ClientException as e:
        raise ExitException(click.wrap_text(f"Unexpected upload issue: {e.message}. {resume_message}"))
        ...
        raise ExitException(msg or click.wrap_text(f"Unexpected upload issue: {e}. {resume_message}"))
        ...
        raise ExitException(msg or click.wrap_text(f"Upload issue: {e}. {resume_message}"))
    except Exception as e:
        raise ExitException(click.wrap_text(f"Unexpected upload issue: {e}. {resume_message}"))
    finally:
        q.put("exit")
        t.join()
        pool.terminate()
    ...
    click.echo(f"Upload of {name} ({path}) to 'uploads/{name}' complete.")
