...
def stats(ctx, run_id, raw, follow):
    ...
    with api_client_exception_handler():
        ...
        try:
            ...
            if follow:
                click.echo(f"Run {run_id} is no longer running")
        except KeyboardInterrupt:
            msg = f"Use 'spell stats {'--follow' if follow else ''} {run_id}' to view stats again"
            click.echo(with_emoji("✨", msg, ctx.obj["utf8"]))
...
def print_stats(run_id, cpu_stats, gpu_stats, raw=False, clear=False):
    ...
    if raw:
        click.echo("\n".join([f"{x},{y}" for x, y in [val for val in cpu_stats.__dict__.items()]]))
        if gpu_stats:
            click.echo("\n".join([f"{x},{y}" for x, y in [val for val in gpu_stats.__dict__.items()]]))
    else:
        click.echo(f"CPU statistics for run {run_id}:")
        cpu_stats.cpu_percentage = f"{cpu_stats.cpu_percentage:.2f}%"
        cpu_stats.memory_percentage = f"{cpu_stats.memory_percentage:.2f}%"
        cpu_stats.mem_ratio = f"{prettify_size(cpu_stats.memory)} / {prettify_size(cpu_stats.memory_total)}"
        cpu_stats.net_ratio = f"{prettify_size(cpu_stats.network_rx)} / {prettify_size(cpu_stats.network_tx)}"
        ...
        if gpu_stats:
            click.echo()
            click.echo(f"GPU statistics for run {run_id}:")
            for row in gpu_stats:
                row.power_ratio = f"{row.power_draw} / {row.power_limit}"
                row.mem_ratio = f"{row.memory_used} / {row.memory_total}"
