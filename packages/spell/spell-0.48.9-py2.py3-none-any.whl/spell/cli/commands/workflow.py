...
def create(
    ...
    **kwargs,
    ...
    try:
        ...
        raise ExitException(
            click.wrap_text(f"Incorrect formatting of repo '{e.token}', it must be <label>=<repo_path>[:commit_ref]"),
            SPELL_INVALID_CONFIG,
        )
    ...
    try:
        ...
        raise ExitException(
            click.wrap_text(
                f"Incorrect formatting of GitHub repo '{e.token}', it must be <label>=<githubrepo_url>[:ref]"
            ),
            SPELL_INVALID_CONFIG,
        )
    ...
    utf8 = ctx.obj["utf8"]
    click.echo(with_emoji("💫", f"Casting workflow #{workflow.id}" + ellipses(utf8), utf8))
    if not local:
        if background:
            click.echo(f"View logs with `spell logs {workflow.managing_run.id}`")
        else:
            click.echo(with_emoji("✨", f"Following workflow at run {workflow.managing_run.id}.", utf8))
        ...
        os.system(" ".join((f"'{x}'" for x in (command,) + args)))
...
def archive(ctx, workflow_id):
    ...
    click.echo(f"Archiving workflow {workflow_id}.")
...
def kill(ctx, workflow_id):
    ...
    click.echo(f"Succesfully killed workflow {workflow_id}.")
