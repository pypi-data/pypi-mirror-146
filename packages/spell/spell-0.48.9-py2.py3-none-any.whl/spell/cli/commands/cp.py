...
def cp(ctx, source_path, local_dir, force, ignore):
    ...
    if source_path.startswith(os.path.sep):
        msg = f'Invalid source specification "{source_path}". Source path must be a relative path.'
            ...
            with client.tar_of_path(source_path) as tar:
                ...
                for file in tar:
                    ...
                    if file.isfile() and spinner:
                        ...
                        spinner.text = f"Copying {name} ({prettify_size(file.size)})"
                    tar.extract(file, path=local_dir)
                if spinner:
                    for file_name in dup_dirname_list:
                        report_line = ("File '{}' ignored: a local directory " "with that name already exists.").format(
                            file_name
                        )
                        if ctx.obj["utf8"]:
                    ...
                    report_line = f"Copied {count} files."
                    if skip_count > 0:
                        report_line += f" Skipped {skip_count} files."
                    if ctx.obj["utf8"]:
