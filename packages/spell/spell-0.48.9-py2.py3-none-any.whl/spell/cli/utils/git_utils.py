...
def detect_repo(
    ...
    if git_repo is None:
        ...
        if force:
            ...
            click.confirm(
                "Could not find a git repository, so no user files will be available. " "Continue anyway?",
                default=True,
                abort=True,
            )
        return Repository()
...
def sync_repos(ctx, repo_specs, force, include_uncommitted=False, resource_type="run"):
    ...
    for repo_spec in repo_specs:
        ...
        try:
            ...
            click.echo(click.wrap_text(f"Could not find a repo at {repo_path}"), err=True)
            invalid_repos.append(repo_path)
            continue
        # actually sync the workspace
        click.echo(with_emoji("✨", f"Syncing repo {repo_path}.", ctx.obj["utf8"]))
        try:
        ...
        if repo_spec["name"] in workspace_info:
            invalid_repos.append(repo_path)
            click.echo(
                click.wrap_text(f"Each repo name must be unique, {repo_spec['name']} is repeated"),
                err=True,
            )
            continue
...
def hardlink_repo(ctx, repo):
    ...
    logger.info(f"rsync took: {after - before}")
...
def push_workspace(ctx, git_repo, commit_ref, force=False, include_uncommitted=False, resource_type="run"):
    ...
    try:
        ...
        raise ExitException(
            f'Could not resolve commit "{commit_ref}" for repo {git_repo.working_dir}',
            SPELL_INVALID_COMMIT,
        )
    ...
    if has_untracked(git_repo):
        if not force:
            ...
            logger.warning(f"Repo contains untracked files that will not be included in the {resource_type}.")
    ...
    if (has_staged(git_repo) or has_unstaged(git_repo)) and include_uncommitted:
        click.echo(with_emoji("✨", "Preparing uncommitted changes" + ellipses(ctx.obj["utf8"]), ctx.obj["utf8"]))
    ...
    refspec = f"{git_repo.head}:refs/heads/br_{uncommitted_hash or commit_hash}"
    git_push = [
        "git",
        "--git-dir",
        git_dir,
    ...
    try:
        ...
        v = git_version()
        if v and v < (2, 3):
            msg += f". Git version 2.3 or newer is required -- detected Git version: {v}"
        elif not v:
...
def get_unstaged_filenames(repo):
    return [diff.a_path for diff in repo.index.diff(None) if diff.a_path.split(".")[-1] not in WHITELISTED_FILEEXTS]
...
def get_untracked(repo):
    return "\n".join([f"\t{f}" for f in get_untracked_filenames(repo)])
