...
def send_metric(name, value, index=None):
    ...
    if isinstance(value, (bool, np_bool, np_int, int)):
        ...
        warnings.warn(
            f"metric discarded: 'value' must be a float, int, or string type (type={type(value)})",
            SpellMetricWarning,
        )
        return
    ...
    if not isinstance(name, str):
        warnings.warn(
            f"metric discarded: 'name' must be a string type (type={type(name)})",
            SpellMetricWarning,
        )
        return
    data["name"] = name
    ...
    if (index is not None) and (not isinstance(index, int)):
        warnings.warn(
            f"'index' ignored: must be of type int (type={type(index)})",
            SpellMetricWarning,
        )
    ...
    try:
        url = f"http://{metrics_domain}/user-metrics"
        r = requests.post(url, json=data)
        if r.status_code == requests.codes.bad_request:
            warnings.warn(f"metric discarded: {r.text}", SpellMetricWarning)
    except requests.exceptions.RequestException:
        warnings.warn(f"metric discarded: could not reach {url}", SpellMetricWarning)
