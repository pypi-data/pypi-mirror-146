...
def json_flatten(json_data,list_split_to_many=False,ignore_parent_key=None):
    ...
    :param list_split_to_many: If the final node list data should be splitted to one to many relation(i.e to multiple rows)
    :param ignore_parent_key: Parent Keys to be ignored (string or list)
            ...
            for col_name in id_df.drop('id_col', axis=1).columns:
                ...
                while check:
                    ...
                    if any(col_name + '_' in srchstr for srchstr in final_master.columns):
                        ...
                        temp_df = final_master[['id_col'] + col_name_master]
                        temp_df['temp_index'] = temp_df.reset_index().index
                    else:
                        master_flag = False
                        ...
                        if temp_df[cols].dropna().shape[0] > 0:
                            if type(temp_df[cols].dropna().reset_index(drop=True).iloc[0]) == list:
                                if max(temp_df[cols].str.len()) <= 1:
                                    ...
                                    # list_element_type = [type(item) for item in temp_df[temp_df[cols].str.len() == max(temp_df[cols].str.len())][cols].reset_index(drop=True).iloc[0]]
