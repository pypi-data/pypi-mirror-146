    ...
    def run(self, task, pulsar_topic, rabbitmq_exchange, rabbitmq_routing_key, durable=False,
        ...
        try:
            ...
            self.rabbitmq_connect.receive(rabbitmq_exchange, rabbitmq_routing_key, callback,
                                          durable, rabbitmq_thread_count, heartbeat)
...
class PulsarToRabbitmq(object):
    ...
    def run(self, task, rabbitmq_exchange, rabbitmq_routing_key,
            ...
            durable=False, pulsar_thread_count=None, logger=None, heartbeat=60):
        """
        1. 订阅 pulsar
        2. 处理消费的数据
        ...
        try:
            ...
            def callback(msg):
                result = task(msg)
                if result:
                    self.rabbitmq_connect.send(result, rabbitmq_exchange, rabbitmq_routing_key, durable, heartbeat)
...
class InterService(object):
    ...
    def run(self, pulsar_producer_topic, rabbitmq_send_exchange, rabbitmq_send_routing_key,
        ...
        pool.submit(self.rabbitmq_to_pulsar.run, send_task, pulsar_producer_topic, rabbitmq_consumer_exchange,
                    ...
                    _async=_async, send_callback=send_callback, logger=logger, heartbeat=heartbeat)
        pool.submit(self.pulsar_to_rabbitmq.run, consumer_task, rabbitmq_send_exchange, rabbitmq_send_routing_key,
                    ...
                    pulsar_thread_count=pulsar_thread_count, logger=logger, heartbeat=heartbeat)
...
class InterServiceRQ(object):
    ...
    def run(self, pulsar_producer_topic, rabbitmq_send_exchange, rabbitmq_send_routing_key,
            ...
            consumer_durable=None, producer_durable=None,
            _async=True, send_callback=None, thread_count=5, logger=None, heartbeat=60):
        """
            ...
            if result:
                ...
                def send_consume():
                    ...
                    def callback_task(_msg):
                        ...
                        if consumer_result:
                            self.rabbitmq_connect.send(consumer_result, rabbitmq_send_exchange,
                                                       rabbitmq_send_routing_key,
                                                       producer_durable if producer_durable else durable, heartbeat)
        ...
        self.rabbitmq_connect.receive(rabbitmq_consumer_exchange, rabbitmq_consumer_routing_key, callback,
                                      consumer_durable if consumer_durable else durable, heartbeat)
