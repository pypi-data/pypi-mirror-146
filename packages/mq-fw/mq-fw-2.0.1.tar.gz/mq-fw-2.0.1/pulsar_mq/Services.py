...
import json
import time
from pulsar_mq.Parameter_dict import get_consumer_type
...
class ConsumeProduce(object):
    ...
    def __init__(self, client, consumer_topic, consumer_name: str, consumer_type, producer_topic=None,
...
class ProduceConsume(object):
    ...
    def run(self, data, random_topic=True, timeout_millis=300000, _async=True, callback=None):
        ...
        :return:
        """
        def do(consumer_topic, consumer_name, _random_topic):
            consumer = self.client.subscribe(consumer_topic, consumer_name, schema=self.sch,
        ...
        if self.consumer_topic and self.consumer_name:
            if random_topic:
                raise ValueError(f'parameter values are in conflict, random_topic is {random_topic}, '
                                 f'consumer_topic is {self.consumer_topic}, consumer_name is {self.consumer_name}')
            else:
                return do(self.consumer_topic, self.consumer_name, random_topic)
        else:
            if self.consumer_topic is None and self.consumer_name is None:
                ...
                raise ValueError('consumer_name is None, need cconsumer_name')
