...
class DataLoader():
    ...
    >>> result = dl.get(column_list=[   "date",
                                        "open",
                                        "close",
    ...
    def load_pandas(self, df: pd.DataFrame) -> None:
        ...
        """
        self.df = df
        self.df.loc[len(self.df.index)] = [np.nan] * len(self.df.columns)
    ...
    def get_index_by_date(self, that_date: Union[int, str, datetime], date_column_name: str = "date") -> list:
        """
        ...
        if (type(that_date) == str):
            return self.df.loc[self.df[date_column_name] >= that_date].first_valid_index()
        if (type(that_date) == date):
            return self.df.loc[self.df[date_column_name] >= (that_date).strftime("%Y-%m-%d")].first_valid_index()
        return None

    def get(self, column_list: list, interval: Interval, num_of_observations: int = 0) -> pd.DataFrame:
        """
        Getting pd.DataFrame based on interval's start and end (inclusive)
        ...
        end_index = self.get_index_by_date(
            interval.end, interval.datetime_column_name)    # get the row index based on interval.end
        start_index = None
        ...
        if (start_index == None):
            ...
            start_index = self.get_index_by_date(
                interval.start, interval.datetime_column_name)
        if ((start_index != None) and (end_index != None)):
            ...
            for element in column_list:
                payload[str(element)] = get_data_column(
                    element, self.df, start_index, end_index)
