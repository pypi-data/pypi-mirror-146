...
def get_data_column(data_column, df, start_index: int, end_index: int):
    ...
    if (type(data_column) == str):
        return df.iloc[start_index:end_index + 1][data_column]
...
def pct_change_sign(factor, df, start_index:int, end_index:int) -> pd.Series:
    factor_window = df.iloc[max(start_index-1, 0):end_index+1][factor.column_name]
...
def simple_moving_average(factor, df, start_index: int, end_index: int):
    ...
    return df.iloc[start_index - factor.window_size + 1:end_index+1][factor.column_name].rolling(window=factor.window_size).mean().iloc[factor.window_size-1:]
