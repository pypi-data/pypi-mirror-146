...
def build(serviceName, version, base_url, **kw):
    ...
    Usage Examples - Data Services:
        ...
        api_response = dataservice.subscriptions().list(category_1=c1,category_2=c2,category_3=c3,category_4=c4,category_5=c5,email=True,sms=False)
        api_response = dataservice.subscriptions().insert(user_subscription=user_subscription)
        api_response = dataservice.subscriptions().update(uid=uid, user_subscription=user_subscription)
        api_response = dataservice.subscriptions().delete(uid=uid)
