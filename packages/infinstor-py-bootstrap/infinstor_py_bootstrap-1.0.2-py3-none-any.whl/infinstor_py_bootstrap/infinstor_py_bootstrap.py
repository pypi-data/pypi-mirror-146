...
def _isPipInstallCmd() -> bool:
    ...
    is_pip_install_cmd:bool = (any(s.endswith("pip") for s in sys.argv) or any(s.endswith("pip3") for s in sys.argv)) and any('install' == s for s in sys.argv)
    logger.info(f"_isPipInstallCmd={is_pip_install_cmd}")
...
def _pip_install_or_upgrade(pip_index_url:str, pip_extra_index_urls:list, packages_with_ver:list, logger:logging.Logger) -> bool:
    ...
    # original idea: use popen()/subprocess.run() so that output from pip does not go to stdout (which confuses tools like vscode which parses interpreter startup output)
    #    but popen()/subprocess.run() will not work, since when a 'pip' subprocess is started, the 'pth' hook installed by this module will be called, which will then attempt to start another 'pip' subprocess.  this will lead to infinite recursion 
...
def main():
    try:
        ...
        #    Eventually the VM runs out of memory due to too many processes.
        if _isPipInstallCmd():
            logger.info(f"Not performing auto update since python process is 'pip install': avoid infinite recursion")
            return
