...
def up_zip_lists(list1, list2):
  assert isinstance(list1, list), f'Puddles says: Expecting a list for list1 but instead got a {type(list1)}!'
  assert isinstance(list2, list), f'Puddles says: Expecting a list for list2 but instead got a {type(list2)}!'
...
def up_scale_table(table):
  ...
  assert not residue, f'Puddles says: these columns contain stings {residue}'
...
def up_apply_3sigma(table, column_name):
  ...
  assert all([isinstance(v, (int, float)) for v in table[column_name].to_list()]), f'Puddles says: column_name is not numeric.'
