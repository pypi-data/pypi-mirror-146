...
from .utils.filesize import naturalsize
    ...
    for file in files.json():
        ...
        for share in file["shares"]:
            ...
            if data["expired"]:
                continue
            hours = round(
                (datetime.datetime.fromisoformat(data["expiration"]) - datetime.datetime.now()).total_seconds() / 3600,
                1,
            )
            exp_string = f"Expires in {hours} hours" if data["expiration"] is not None else "Does not expire"
            url_string = f'[u]{config.routes.base}{data["url"]}[/u]'
            share_output_strings.append(f"  Shared at: [green]{url_string} ({exp_string})[reset]")

        file_attribute_string = (
            f"{naturalsize(file['file_size'])}"
            f"{'' if file['is_public'] else ', Not Public'}"
            f"{', Encrypted' if file['is_encrypted'] else ''}"
            f"{', Shared' if share_output_strings else ''}"
        )
