...
# TODO! INCLUDE RANDOM JUPYTER NOTEBOOK CODE IN FLOW
...
class Search():
    def __init__(self, dist=None, code_name="abc", avail=None, weights=None,
                 ...
                 test_alt_var=None, allow_random=True):
        ...
        self.test_av = None  # TODO?
        self.test_weight_var = None  # TODO?
    ...
    def fit_mnl(self, dat, test_df, as_vars, is_vars, bcvars, choice, alt,
        ...
        return(model.bic, val, as_vars, is_vars, rand_vars, bc_vars, cor_vars,
                ...
                if all(v for v in sig <= self.p_val):
                    ...
                    while any([v for v in sig if self.p_val]):
                        ...
                        ns_intercept = [x for x in non_sig if
                                        x.startswith('_intercept.')]  # non-significant intercepts
    ...
    def initialize_memory(self, choice_data, HMS, asvars_avail, isvars_avail,
        ...
        for i in range(len(seeds)):
            ...
            print("opp sol features", op_asvars, op_isvars, op_rvars,
                  op_bcvars, op_corvars, op_bctrans, op_cor, op_asconstant)
            opp_sol, opp_conv = \
            ...
            if opp_conv:
                ...
                if len(opp_HM) > 0:  # only do check if there are already solutions
                    ...
                    if len(unique_opp_HM_discrepancy) > 0:
                        ...
                        # accepts solution if 2 or more aspects of solution are different
                        if similarities > 3:
    ...
    def add_new_class_paramfeature(self, solution):
        """
        """
        class_params_spec = solution[8]
        class_params_spec_new = class_params_spec.copy()
        all_vars = self.asvarnames + self.isvarnames
        for ii, class_i in enumerate(class_params_spec):
            new_params = [var for var in all_vars if var not in class_i]
            new_param = list(np.random.choice(new_params, 1))
            # TODO? remove redundant asvars ?
            new_class_spec = sorted(list(set(class_i).union(new_param)))
            # TODO! Consider randvars
            class_params_spec_new[ii] = np.array(new_class_spec)

        solution[8] = class_params_spec_new
        return solution
    ...
    def add_new_member_paramfeature(self, solution):
        ...
        for ii, class_i in enumerate(member_params_spec):
            ...
            new_param = list(np.random.choice(new_params, 1))
            # TODO? remove redundant asvars ?
            new_member_spec = sorted(list(set(class_i).union(new_param)))
            # TODO! consider randvars
            member_params_spec_new[ii] = np.array(new_member_spec)

        solution[9] = member_params_spec_new
    ...
    def remove_class_paramfeature(self, solution):
        """
        """
        class_params_spec = solution[8].copy()
        if solution[8]:
            for ii, class_i in enumerate(class_params_spec):
    ...
    def pitch_adjustment(self, sol, har_mem, PAR_itr, seeds, itr, HMS):
        ...
        if np.random.choice([0, 1], p=[1-PAR_itr, PAR_itr]) <= PAR_itr:
            if np.random.randint(2):
                ...
                if pa_sol[8] is not None:  # todo? confirm
                ...
                if pa_sol[9] is not None:  # todo? confirm
                ...
                if len(pa_sol) > 8 and pa_sol[8]:  # check if has class_params_spec
                ...
                if len(pa_sol) > 9 and pa_sol[9]:  # check if has member_params_spec
