...
def load_text(filname=None, data=None, label=None,unit='char',mode='next_word',section_delimiter='\n\n',sequence_start_at='section_start',is_onehot=False,encoding='utf-8-sig',sequence_length=64, return_corpus=False,**kwargs):
        ...
        with io.open(filname, encoding=encoding) as f:
            ...
            for item in corpus:
                if len(item) > 0:
                    ...
                    new_corpus[-1]+=section_delimiter
        ...
        if label is not None:
            ...
            for item in output_label:
                if len(item) > 0:
                    ...
                    new_label[-1] = new_label[-1] +section_delimiter
    ...
    if mode=='next_word':
        ...
        labels_seq =TextSequenceDataset(output_corpus,sequence_length=sequence_length,is_onehot=is_onehot,symbol='label',sequence_offset=1,sequence_start_at=sequence_start_at,object_type=ObjectType.corpus)
        ...
        data_seq = TextSequenceDataset(corpus, sequence_length=sequence_length, is_onehot=is_onehot, symbol='input', sequence_offset=0, sequence_start_at=sequence_start_at,object_type=ObjectType.corpus)
        labels_seq = TextSequenceDataset(output_corpus, sequence_length=sequence_length, is_onehot=is_onehot, symbol='label', sequence_offset=[-1,1], sequence_start_at=sequence_start_at,object_type=ObjectType.corpus)
        traindata = Iterator(data=data_seq, label=labels_seq)
        dataprovider = TextSequenceDataProvider(provider_name, traindata=traindata,sequence_length=sequence_length)
    elif mode == '1to1_seq2seq':
        if len(corpus)==len(output_label):
            data_seq = TextSequenceDataset(corpus, sequence_length=sequence_length, is_onehot=is_onehot, symbol='input', sequence_offset=0, sequence_start_at=sequence_start_at,object_type=ObjectType.corpus)
            labels_seq = TextSequenceDataset(output_label, sequence_length=sequence_length, is_onehot=is_onehot, symbol='label', sequence_offset=0, sequence_start_at=sequence_start_at,object_type=ObjectType.sequence_label)
            traindata = Iterator(data=data_seq, label=labels_seq)
            dataprovider = TextSequenceDataProvider(provider_name, traindata=traindata,sequence_length=sequence_length)
        else:
