...
from trident.backend.common import make_dir_if_need, get_plateform, get_time_suffix, sanitize_path, if_none,split_path,OrderedDict
    ...
    def observation(self, observation):
        ...
        if self.obsetvation_type == ObservationType.Box:
            ...
            if self.stay_fool or  ratio>0.5:
                ...
                noise_mask[:,:W//4,:]= np.expand_dims(np.expand_dims(np.linspace(0.2, 1, W//4), 0), -1).astype(np.float32)
                noise_mask=np.concatenate([noise_mask,noise_mask,noise_mask],axis=-1)
                obj_base = np.clip(obj_base * noise_mask + (1 - noise_mask) * (80*np.random.standard_normal((W, W, C))), 0, 255)
        ...
        if self._is_recording and (self.vw is not None and self.frame_steps > 0):
            ...
            if 'keep' in self.video_tags and self.video_tags['keep']:
                if 'name_suffix' in self.video_tags:
                    ...
                    os.rename(self.current_recording_path, os.path.join(folder, filename + '_' + self.video_tags['name_suffix'] + ext))
