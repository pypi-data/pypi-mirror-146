...
class ActorCriticPolicy(MuiltiNetwork):
    ...
    def __init__(self, actor: Layer, critic: Layer, env: gym.Env, state_shape=None, accumulation_steps=1,
        ...
        self.rollout = Rollout(only_cpu=True)
        ...
        try:
            ...
            for i_episode in range(num_episodes):
                ...
                for t in count():
                    ...
                    # Train on_step_end
                    complete = self.episode_complete_criteria()
                    if training and train_timing == 'on_step_end' and ( (len(self.rollout) + 1) % train_every_nstep == 0 or done) and self.frame_steps > 5:
                        try:
...
class PPOPolicy(ActorCriticPolicy):
    ...
    def __init__(self, actor: Layer, critic: Layer, env: gym.Env, state_shape=None, target_update=5, gamma=0.9,
                 beta=0.01, use_gae=False, gae_lambda=0.95, reward_scaling=False, normalize_advantages=False,value_clipping=False,
        ...
        self.target_update = target_update
        self.memory = Rollout(only_cpu=True)
    ...
    def calculate_discounted_returns(self, *args, **kwargs):
        try:
            ...
            if self.reward_scaling:
                ...
                reward_pool=reward_pool/(reward_pool.std()+1e-5)
            ...
            for state in state_pool:
                ...
                if self.value_clipping and len(Values)>0 and Values[-1]!=0:
                    last_v=Values[-1].item()
                    value= clip(value/last_v, 1.0 - 0.2, 1.0 + 0.2)*last_v
                Values.append(value)
            ...
            next_value =to_tensor(0.0).to(get_device())
            if not last_done:
                next_value=self.critic.model(to_tensor(self.state).to(get_device()))[0]

            for value, reward, done in list(zip(Values, reward_pool, done_pool))[::-1]:
                gae = gae * self.gamma * self.gae_lambda
                gae = gae + reward + self.gamma * next_value * (1 - float(done)) - value
                next_value = value
                R.append(gae + value)
            R = R[::-1]
            R = torch.cat(R)
            ...
            self.critic.current_loss= critic_loss
            self.critic.training_context['tmp_losses'].collect('critic_loss',self.train_steps, to_scalar(critic_loss.copy().detach()))
            self.critic.batch_metric_history.collect('critic_rmse', self.train_steps, to_scalar(critic_loss.copy().detach().sqrt()))
            ...
            for i in range(len(reward_pool)):
                ...
                new_log_prob = new_m.log_prob(to_tensor(action_pool[i]).long())
                LogProbs.append(new_log_prob)
                ratio1 = exp(new_log_prob - log_prob_pool[i])
                Ratio1.append(ratio1)
                Entropy.append(expand_dims(new_m.entropy(),0))

            LogProbs= concate(LogProbs, axis=0)
            Ratio1 = concate(Ratio1, axis=0)
            Ratio2 = clip(Ratio1, 1.0 - 0.2, 1.0 + 0.2)
            Entropy = concate(Entropy, axis=0)


            self.actor.batch_metric_history.collect('ratio1', self.train_steps, to_scalar(Ratio1.mean()))
            self.actor.batch_metric_history.collect('ratio2', self.train_steps, to_scalar(Ratio2.mean()))

            surr1 = Ratio1 * advantages
            surr2 = Ratio2 * advantages
         
            clip1 = torch.min(surr1, surr2)
            clip2 = torch.max(clip1, 5 * advantages)
            actor_loss = -torch.where(advantages < 0, clip2, clip1).mean()
            self.actor.current_loss =  actor_loss - 0.01*Entropy.mean()
            # self.actor.training_context['current_loss'] = self.actor.training_context['current_loss'] + actor_loss
            self.actor.training_context['tmp_losses'].collect('actor_loss', self.train_steps,to_scalar(actor_loss.copy().detach()))
            self.actor.training_context['tmp_losses'].collect('entropy_loss', self.train_steps,to_scalar(Entropy.copy().detach()))
            ...
            try:
                ...
                if len(self.rollout) > 3:
                    ...
                    try:
                        ...
                        for idx in list(range(len(R))):
                            if idx >0:
                                if random.random()>0.9 or abs(self.rollout.reward[idx])>=0.15  or (self.rollout.done[-1]  and len(R)>128 and idx>len(R)-10):
                                    self.memory.collect('return', R[idx-1])
                                    self.memory.collect('state', self.rollout.state[idx-1])
                                    self.memory.collect('action', self.rollout.action[idx-1])
                                    self.memory.collect('log_prob',self.rollout.log_prob[idx-1])
                ...
                if self.done == True and len(self.memory) > 5:
                    ...
                    for i in range(8):
                        try:
                            ...
                            batch_data = self.memory.get_samples(builtins.min(self.training_context['batch_size'], len(self.memory)))
                            ...
                            batch_value = concate([self.critic(to_tensor(batch_state[i]))[0] for i in range(len(batch_state))], axis=0)
                            batch_return = to_tensor(batch_return)
                            batch_log_prob = to_tensor(batch_log_prob)
                            batch_advantages = (batch_return.detach() - batch_value)
                            if self.normalize_advantages:
                                batch_advantages = (batch_advantages - batch_advantages.mean())  # / (advantages.std() + 1e-10)

                            batch_critic_loss = 0.5 * (batch_advantages ** 2).mean()
                            self.critic.current_loss = batch_critic_loss
                            self.critic.training_context['tmp_losses'].collect('critic_loss', self.train_steps,to_scalar(batch_critic_loss.copy().detach()))
                            self.critic.batch_metric_history.collect('critic_rmse', self.train_steps,to_scalar((batch_critic_loss.copy().detach()).sqrt()))
                            ...
                            for j in range(len(batch_state)):
                                ...
                                LogProbs.append(new_log_prob)
                                ratio1 = exp(new_log_prob - old_batch_probs)
                                Ratio1.append(ratio1)
                            LogProbs = concate(LogProbs, axis=0)
                            Ratio1 = concate(Ratio1, axis=0)
                            Ratio2 = clip(Ratio1, 1.0 - 0.2, 1.0 + 0.2)
                            surr1 = Ratio1 * batch_advantages
                            surr2 = Ratio2 * batch_advantages
                            clip1 = torch.min(surr1, surr2)
                            clip2 = torch.max(clip1, 5 * batch_advantages)
                            batch_actor_loss = -torch.where(batch_advantages < 0, clip2, clip1).mean()

                            self.actor.current_loss =  batch_actor_loss
                            self.actor.training_context['tmp_losses'].collect('actor_loss', self.train_steps ,to_scalar((batch_actor_loss).copy().detach()))
