...
class RNNCellBase(Layer):
    ...
    def __init__(self,  hidden_size: int, use_bias: bool, num_chunks: int,
                 keep_output=False,filter_index=-1,name=None) -> None:

        super(RNNCellBase, self).__init__(keep_output=keep_output,in_sequence=True,filter_index=filter_index,name=name)
...
class RNNCell(RNNCellBase):
    ...
    Examples::
        ...
        >>> for i in range(6):
                ...
                output.append(hx)
    """
    __constants__ = ['hidden_size', 'bias', 'nonlinearity']
    nonlinearity: str

    def __init__(self,  hidden_size: int, use_bias: bool = True, nonlinearity: str = "tanh") -> None:

        super(RNNCell, self).__init__(hidden_size, use_bias=use_bias, num_chunks=1)
        self.nonlinearity = nonlinearity

    def forward(self, x: Tensor, hx: Optional[Tensor] = None) -> Tensor:
        assert x.dim() in (1, 2), \
            f"RNNCell: Expected input to be 1-D or 2-D but received {x.dim()}-D tensor"
        is_batched = x.dim() == 2
        if not is_batched:
            x = x.unsqueeze(0)
        ...
        if self.nonlinearity == "tanh":
            ...
            ret = x  # TODO: remove when jit supports exception flow
            raise RuntimeError(
...
class LSTMCell(RNNCellBase):
    ...
    def __init__(self,hidden_size: int, use_bias: bool = True) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super(LSTMCell, self).__init__( hidden_size, use_bias, num_chunks=4, **factory_kwargs)

    def forward(self, x: Tensor, hx: Optional[Tuple[Tensor, Tensor]] = None) -> Tuple[Tensor, Tensor]:
        assert x.dim() in (1, 2), \
            f"LSTMCell: Expected input to be 1-D or 2-D but received {x.dim()}-D tensor"
        is_batched = x.dim() == 2
        if not is_batched:
            x = x.unsqueeze(0)
