    ...
    def _synchronize(self):
        ...
        if self.sync_method == "timestamp":
            ...
            if len(self.measurements[GPSSeries]) > 0:
                ...
                logger.warning("(%s) No GPS time recording, falling back to device_time synchronization" % self.filename)
    ...
    def do_map_matching(self, v_thres: float = 1, algorithm: str = "pyridy", alpha: int = 1.0, beta: int = 1.0):
        ...
        if self.osm:
            ...
            if not s_n or not e_n:
                ...
                logger.info("(%s) Found %d nodes that match the files GPS track!" % (self.filename,
    ...
    def load_file(self, path: str):
        ...
        if self.extension == ".rdy":
            ...
            if 'Ridy_Version' in rdy:
                ...
                logger.info("No Ridy_Version in file: %s" % self.filename)
                self.ridy_version = None

            if 'Ridy_Version_Code' in rdy:
                self.ridy_version_code = rdy['Ridy_Version_Code']
            else:
                logger.info("No Ridy_Version_Code in file: %s" % self.filename)
            ...
            if 'ntp_timestamp' in rdy:
                ...
                logger.info("No ntp_timestamp in file: %s" % self.filename)
            ...
            if 'ntp_date_time' in rdy:
                ...
                self.ntp_date_time = None
                logger.info("No ntp_date_time in file: %s" % self.filename)
            ...
            if (self._series is not None and AccelerationUncalibratedSeries in self._series) or self._series is None:
                if "acc_uncal_series" in rdy:
                    ...
                    logger.info("No uncalibrated Acceleration Series in file: %s" % self.filename)
            ...
            if (self._series is not None and MagnetometerUncalibratedSeries in self._series) or self._series is None:
                if "mag_uncal_series" in rdy:
                    ...
                    logger.info("No uncalibrated Magnetometer Series in file: %s" % self.filename)
            ...
            if (self._series is not None and GyroUncalibratedSeries in self._series) or self._series is None:
                if "gyro_uncal_series" in rdy:
                    ...
                    logger.info("No uncalibrated Gyro Series in file: %s" % self.filename)
            ...
            if (self._series is not None and GNSSClockMeasurementSeries in self._series) or self._series is None:
                if "gnss_clock_series" in rdy:
                    ...
                    logger.info("No GNSS Clock Series in file: %s" % self.filename)
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        "DatabaseError occurred when accessing measurement_information_table, file: %s" % self.filename)
                    logger.error(e)
                    info = {}
            ...
            try:
                ...
                logger.error(
                    "DatabaseError occurred when accessing sensor_descriptions_table, file: %s" % self.filename)
                logger.error(e)
            ...
            if (self._series is not None and AccelerationUncalibratedSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error("DatabaseError occurred when accessing acc_uncal_measurements_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and LinearAccelerationSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error(
                        "DatabaseError occurred when accessing lin_acc_measurements_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and MagnetometerUncalibratedSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error("DatabaseError occurred when accessing mag_uncal_measurements_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and OrientationSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error(
                        "DatabaseError occurred when accessing orient_measurements_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and GyroUncalibratedSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error(
                        "DatabaseError occurred when accessing gyro_uncal_measurements_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and GNSSMeasurementSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error("DatabaseError occurred when accessing gnss_measurement_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and GNSSClockMeasurementSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error("DatabaseError occurred when accessing gnss_clock_measurement_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and NMEAMessageSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error("DatabaseError occurred when accessing nmea_messages_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and PressureSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error(
                        "DatabaseError occurred when accessing pressure_measurements_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and TemperatureSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error(
                        "DatabaseError occurred when accessing temperature_measurements_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and HumiditySeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error(
                        "DatabaseError occurred when accessing humidity_measurements_table, file: %s" % self.filename)
                    logger.error(e)
            ...
            if (self._series is not None and SubjectiveComfortSeries in self._series) or self._series is None:
                try:
                    ...
                    logger.error(
                        "DatabaseError occurred when accessing subjective_comfort_measurements_table, file: %s" % self.filename)
                    logger.error(e)
