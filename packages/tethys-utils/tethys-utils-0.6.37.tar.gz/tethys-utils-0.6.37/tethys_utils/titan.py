...
class Titan(object):
    ...
    def update_conflicting_results(self, results_paths, add_old=False, threads=60, max_workers=4):
        ...
        r_chunks = tu.s3.determine_results_chunks_diffs(list(results_paths_set), remote1, add_old=add_old)

        r_chunks1 = r_chunks[r_chunks['_merge'].isin(['old', 'conflict'])].copy()

        results_paths_set1 = results_paths_set.difference(set(list(r_chunks1.file_path.unique())))
        ...
        if not r_chunks1.empty:
            ...
            if max_workers <= 1:
                ...
                for chunk in chunks1:
                    ...
                    metadata = self.datasets[ds_id]
                    p1 = tu.processing.update_compare_results(chunk, metadata, self.max_version_date_key, self.preprocessed_path, add_old=add_old)
                ...
                with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
                    ...
                    for chunk in chunks1:
                        ds_id = str(chunk).split('/')[-3]
                        metadata = self.datasets[ds_id]
                        f = executor.submit(tu.processing.update_compare_results, chunk, metadata, self.max_version_date_key, self.preprocessed_path, add_old=add_old)
                        futures.append(f)
                    runs = concurrent.futures.wait(futures)
        ...
        combo_paths = list(results_paths_set1.union(set(updated_paths)))
        combo_paths.sort()
