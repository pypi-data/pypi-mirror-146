...
def write_nc_zstd(data: Union[xr.DataArray, xr.Dataset], output_path, retries=5):
    ...
    while True:
        try:
            _ = write_pkl_zstd(data.to_netcdf(), output_path)

            ## Test the write
            _ = utils.read_pkl_zstd(output_path)
...
def update_compare_results(previous_result, metadata, version_date, new_path, add_old=True):
    """
    ...
    if not new_results:
        ...
        if ('modified_date' in new_xr) or ('geometry' in dims):
            ...
            up1 = compare_xrs(old_xr, new_xr, add_old)
    ...
    write_nc_zstd(up1, new_result_path)
