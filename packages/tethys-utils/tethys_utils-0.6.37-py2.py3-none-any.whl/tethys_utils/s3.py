...
def determine_results_chunks_diffs(source_paths, remote, add_old=False):
    """
    ...
    for ds_id, g in paths2.groupby('dataset_id'):
        ...
        if ds_id in datasets:
            ...
            if len(vd1) > 0:
                ...
                if (method in ['sensor_recording', 'field_activity', 'sample_analysis']) or add_old:
                    stns = tethys.get_stations(ds_id)
                    old_stn_ids = [s['station_id'] for s in stns]
            ...
            # Load in the appropriate chunks to check - slowest part of the function
            tethys._load_results_chunks(ds_id, old_stns, threads=60)
