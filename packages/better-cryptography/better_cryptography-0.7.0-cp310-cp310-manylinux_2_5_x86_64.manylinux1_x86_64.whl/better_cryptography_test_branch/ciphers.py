...
def hash_(StringToHash:str, hash_code:str, return_type="hex"): # tested
    """
...
def one_time_pad_encrypt(message:str, key:str):
    ...
    # generating the key
    key_bytes = hash_(key, "SHA256", return_type="bytes")
...
def init_diagonostic():
    ...
    assert hash_("Hello World", "SHA256", return_type="bytes") == hash.sha256("Hello World".encode("utf-8")).digest()
    assert hash_("Hello World", "SHA512", return_type="bytes") == hash.sha512("Hello World".encode("utf-8")).digest()
    assert hash_("Hello World", "SHA224", return_type="bytes") == hash.sha224("Hello World".encode("utf-8")).digest()
    assert hash_("Hello World", "MD5", return_type="bytes") == hash.md5("Hello World".encode("utf-8")).digest()
    print ("Bytes match")
