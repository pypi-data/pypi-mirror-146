...
class ConfigTest(TestCase):
    ...
    @patch('montecarlodata.config.mkdirs')
    @patch('builtins.open', new_callable=mock_open, read_data='data')
    def test_write_invalid_token(self, mock_file, create_directory_mock):
        self._parser_mock.sections.return_value = []
        _SAMPLE_OPTIONS['mcd_token'] = 'invalid-short-token'
        self.assertRaises(ValueError, self._service.write_with_validation, **_SAMPLE_OPTIONS)
