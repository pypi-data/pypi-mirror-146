...
def nevergrad_model(df, hyperparameters_dict, medias, others, target, use_intercept=True, use_adstock=True, use_saturation=True,
                    adstock_type='weibull', model_regression='linear', X_transformations_columns=[],
    ...
    def build_model(**all_hyper):
        ...
        #print(df_transf['prediction'])
        #print('FORCE  ',coeffs)
        #print('XLOG  ', X_transformations_columns)
        #print('NO  ', all_features)
    ...
    def build_model_result(**all_hyper):
        ...
        metric_to_optimize, result, model = build_model(**all_hyper)
    ...
    metric, result, model = build_model(**recommendation.value[1])
    return metric, result, model, recommendation.value[1], metric_array
