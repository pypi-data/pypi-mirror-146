...
def show_rssd(df_aggregated, medias, verbose=False):
    # mean absolute percentage error
...
def show_effect_share_vs_spend_share(df_aggregated, show_cpo=True):
    ...
    if show_cpo:
        fig.add_scatter(x=df_spend_var['canale'], y=round(1 / (round(df_spend_var['roi'], 2)), 2), mode="markers",
                        name='cpo',
...
def show_all_decomp_response(df_all_decomp, name_date_column, features, show_target=False, target='conversions',
    ...
    fig = px.histogram(df_all_decomp, x=name_date_column, y=features, nbins=nbins, bargap=bargap)
    fig.show()
...
def show_seasonality(df, seasonality_columns, name_date_column):
    ...
    for s in seasonality_columns:
        ...
        if s == 'yearly':
            fig = px.line(
                x=pd.to_datetime(df[name_date_column]).dt.dayofyear,
                y=df['yearly'])
            fig.update_layout(
        ...
        if s == 'monthly':
            fig = px.line(
                x=pd.to_datetime(df[name_date_column]).dt.day,
                y=df['monthly'])
            fig.update_layout(
        ...
        if s == 'weekly':
            fig = px.line(
                x=pd.to_datetime(df[name_date_column]).dt.day_name(),
                y=df['weekly'])
            fig.update_layout(
                xaxis_title='Days',
                yaxis_title='Weekly')
        ...
        if s == 'daily':
            fig = px.line(
                x=pd.to_datetime(df[name_date_column]).dt.time,
                y=df['daily'])
            fig.update_layout(
