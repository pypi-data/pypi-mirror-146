from cassandra.data.trasformations.trasformations import adstock_weibull, saturation, adstock, adstock_weibull_robyn
            ...
            if adstock_type == 'weibull':
                for m in medias:
                    df_transformations[m] = saturation(
                        adstock_weibull(df[m], trasf[m + '_scale'], trasf[m + '_shape']).replace([np.inf, -np.inf], 0),
                        trasf[m + '_beta'])

            elif adstock_type == 'weibull_robyn':
                for m in medias:
                    df_transformations[m] = saturation(
                        adstock_weibull_robyn(df[m], trasf[m + '_scale'], trasf[m + '_shape']),
                        trasf[m + '_beta'])
...
def choose_optimizer_algoritm(optimizer_algoritm, budget, num_workers, instrum):
    ...
    if optimizer_algoritm == 'PortfolioDiscreteOnePlusOne':
        optimizer = ng.optimizers.PortfolioDiscreteOnePlusOne(parametrization=instrum, budget=budget, num_workers=num_workers)
    ...
    if optimizer_algoritm == 'ScrHammersleySearchPlusMiddlePoint':
        optimizer = ng.optimizers.ScrHammersleySearchPlusMiddlePoint(parametrization=instrum, budget=budget, num_workers=num_workers)
