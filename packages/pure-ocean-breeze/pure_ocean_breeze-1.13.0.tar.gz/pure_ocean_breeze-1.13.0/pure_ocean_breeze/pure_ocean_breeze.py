...
def daily_factor_on300500(fac,hs300=False,zz500=False,zz1000=False):
...
def decap_industry(df,daily=False,monthly=False):
    ...
    df=df.groupby(['date']).apply(neutralize_factors)
    df=df.unstack()
            ...
            for mat in tqdm.tqdm(self.minute_files):
                try:
                    ...
                    if not update:
                        ...
                        self.daily_factors_list_update.append(df)
                        self.daily_factors_update=pd.concat(self.daily_factors_list_update,axis=1)
                        self.daily_factors_update.index=pd.to_datetime(self.daily_factors_update.index.astype(int),
                                                                       format='%Y%m%d')
                except Exception as e:
                    logger.warning(f'{code} 缺失')
                    logger.error(e)
    ...
    def get_daily_factors(self, func, whole=False,add_priclose=False,add_tr=False,start_date=10000000,end_date=30000000):
        ...
        try:
            ...
            if self.daily_factors.index.max()<now_minute_data.index.max():
                ...
                if whole:
                    self.minute_to_daily_whole(func,start_date_update,end_date_update,update=1)
                else:
                    self.minute_to_daily(func,start_date_update,end_date_update,update=1)
        except Exception:
            if whole:
