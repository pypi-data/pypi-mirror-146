...
def get_params_at_slice(params: dict, s: int, slices=None,
                        clip_at_max_slice=True, s_at=None) -> dict:
    s = int(s)
    ret = {n[0]: params[n]
    ...
    if s_at is None:
        s_at = s
    if 'mu' in params:
        ret['r'] = params.get('b', 0.0) * s_at ** power + params['mu']
    elif f'mu{s}' in params:
        ret['r'] = params.get(f'b{s}', 0.0) * s_at ** power + params[f'mu{s}']
    if 'mu_c' in params:
        ret['r_c'] = params.get('b_c', 0.0) * s_at + params['mu_c']
    elif f'mu_c{s}' in params:
        ret['r_c'] = params.get(f'b_c{s}', 0.0) * s_at + params[f'mu_c{s}']
    if 'mu_k' in params:
        ret['k'] = params.get('b_k', 0.0) * np.log(s_at) + params['mu_k']
    elif f'mu_k{s}' in params:
        ret['k'] = params.get(f'b_k{s}', 0.0) * np.log(s_at) + params[f'mu_k{s}']
    if f'p1{s}' in params:
        ret['p1'] = params[f'p1{s}']
    if f'p2{s}' in params:
        ret['p2'] = params[f'p2{s}']
    if clip_at_max_slice and 'r' not in ret:
        s = 0
        for k in params:
            try:
                s = max(s, int(k.split('mu')[1]))
