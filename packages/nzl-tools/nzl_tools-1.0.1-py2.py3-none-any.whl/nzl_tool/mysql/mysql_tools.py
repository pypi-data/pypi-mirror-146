...
class mysql_tool():
    driver = pymysql
    """The mysql_tools is a tool for mysql connect."""
    ...
    def my_fetchall(self, sql, args=None, return_type=None):
        ...
        try:
            ...
            try:
                if ";" not in sql:
                    logging.warning('mysql_tools.my_fetchall: sql not end with \";\";')
                cur.execute(sql, args)
                rows = cur.fetchall()
                rows = [r for r in rows]
            except self.driver.Error as error:
                logging.error('mysql_tools.my_fetchall: sql:' + str(error))
            finally:
                con.close()
        except self.driver.Error as error:
            logging.error('mysql_tools.my_fetchall: default.cnf error:' + str(error))
        if str(rows) == "[(None,)]":
            rows = []
        return rows
    ...
    def my_fetchall_new(self, sql_list, args=None, return_type=None):
        ...
        try:
            ...
            try:
                for sql in sql_list:
                    if ";" not in sql:
                        logging.warning('mysql_tools.my_fetchone: sql not end with \";\";')
                    cur.execute(sql, args)
                    row = cur.fetchone()
                    row_list.append(row)
            except self.driver.Error as error:
                logging.error('mysql_tools.my_fetchone: sql:' + str(error))
            finally:
                con.close()
        except self.driver.Error as error:
            logging.error('mysql_tools.my_fetchone: default.cnf error:' + str(error))
        return row_list
    ...
    def execute(self, sql, args=None):
        ...
        try:
            ...
            try:
                if ";" not in sql:
                    logging.warning('mysql_tools.execute: sql not end with \";\";')
                handled_item = cur.execute(sql, args)
            except self.driver.Error as error:
                raise error
                logging.error('mysql_tools.execute: sql:' + str(error))
            finally:
                con.commit()
                con.close()
        except self.driver.Error as error:
            print(sql)
            raise error
            logging.error('mysql_tools.execute: default.cnf error:' + str(error))
        return handled_item
    ...
    def insert(self, sql, args=None):
        ...
        try:
            ...
            try:
                ...
                logging.error('mysql_tools.execute: sql:' + sql + str(error))
            finally:
                con.commit()
                con.close()
        except self.driver.Error as error:
    ...
    def get_sql_str(self, sql, args=None):
        """
        get_sql_str
        :return:
        """
        ...
        try:
            ...
            cur = con.cursor()
            try:
                sql_str = cur.mogrify(sql, args)
            except self.driver.Error as error:
                logging.error('mysql_tools.get_sql_str: sql:' + sql + str(error))
            finally:
                con.commit()
                con.close()
        except self.driver.Error as error:
            logging.error('mysql_tools.get_sql_str: default.cnf error:' + str(error))
        return sql_str
            ...
            try:
                ...
                if type(array_sql_action) is list:
                    for item in array_sql_action:
                        sql = item
                        if ";" not in sql:
                            logging.warning('mysql_tools.execute_transaction: sql not end with \";\";')
                ...
                handled_item = -1
                con.rollback()
                logging.error('mysql_tools.execute_transaction: sql: %s : %s' % (sql, str(error)))
            finally:
                con.commit()
                con.close()
        except self.driver.Error as error:
            logging.error('mysql_tools.execute_transaction: default.cnf error:' + str(error))
        return handled_item
