...
class ExtendedModel(AbstractObject):
    ...
    def set_parameter_defaults(source:"ROOT.RooArgSet", value:float=None, error:float=None,
    ...
    def pair_constraints(self, to_str=False, base_component:bool=False, sort:bool=True):
        constraint_set = self.unfold_constraints()
        nuisance_parameters = self.nuisance_parameters
        if sort:
            nuisance_parameters = RooArgSet.sort(nuisance_parameters)
    ...
    def as_dataframe(self, variable_type:Union[str, WSVariables]):
        import pandas as pd
        _variable_type = WSVariables.parse(variable_type)
        ...
        if _variable_type == WSVariables.CONSTRAINTS:
            ...
            df = pd.DataFrame(data, columns =['constraint pdf', 'nuisance parameters', 'global observables'])
            return df
        else:
    ...
    def print_summary(self, items:Optional[List]=None, suppress_print:bool=False, detailed:bool=True,
        ...
        if items is None:
            items = ['workspace', 'dataset', 'snapshot', 'category', 'observable', 
        ...
        # snapshots
        if ('snapshot' in items) and (quickstats.root_version >= (6, 26, 0)):
