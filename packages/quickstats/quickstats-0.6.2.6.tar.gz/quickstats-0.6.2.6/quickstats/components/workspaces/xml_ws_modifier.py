...
class XMLWSModifier(XMLWSBase):
    ...
    def create_modified_workspace(self):
        ...
        title_str = format_delimiter_enclosed_text("Step 1: Create new objects", "-")
        self.stdout.info(title_str)
        flag = self.implement_external_pdfs(ws_tmp)
        flag |= self.implement_objects(ws_tmp)
        # nothing is done
        if not flag:
            self.stdout.info("INFO: No new objectes are defined.")
        
        title_str = format_delimiter_enclosed_text("Step 2: Rename variables", "-")
        self.stdout.info(title_str)
        ...
        title_str = format_delimiter_enclosed_text("Step 3: Making output workspace", "-")
        self.stdout.info(title_str)        
