...
class DicomModule(object):
    ...
    def make_sc_array(self, result_json):
        ...
        lfont = ImageFont.truetype(os.path.join(medical_image.__path__[0], 'fonts/NotoSansCJKkr-Medium.otf'), int(result_json['Height'] / 50))
            ...
            for graphic_object in graphic_annotation['GraphicObjectSequence']:
                ...
                if line_thickness == -1:
                    ...
                    if ('shadow_color' not in graphic_object) or (graphic_object['shadow_color'] != 'OFF'):
                        ...
                        contour_image_array = cv2.polylines(contour_image_array, [graphic_data], False, shadow_color, int(line_thickness * 1.67))
        ...
        if web_logo['location'] == 'RU':
            ...
            x = (result_json['Width'] - logo_w) / 2
            y = result_json['Height'] * 0.999 - logo_h - mh
            w = mw
            h = logo_h + mh
        else:
            raise NotImplementedError
        ...
        img_editable.text((x + shadow, y + logo_h + shadow), msg, (167, 167, 167), mfont)
        img_editable.text((x, y + logo_h), msg, (255, 255, 255), mfont)
