...
@asynccontextmanager
async def lockkey(*keys: str, timeout: float = 0, quite: bool = False):
...
def wa_lockkey(*keys: str, timeout: float = 0, quite: bool = False):
    def wraperfun(func: beni.WrappedAsyncFunc) -> beni.WrappedAsyncFunc:
        ...
        async def wraper(*args: Any, **kwargs: Any):
            async with lockkey(*keys, timeout=timeout, quite=quite):
...
_locknum_dict: dict[str, _Locknum] = {}
