...
def get_funkey(fun: Callable):
    xx = inspect.unwrap(fun)
    sourcefile = inspect.getsourcefile(xx)
    sourceline = inspect.getsourcelines(xx)[1]
    return f'{sourcefile}#{sourceline}'


def get_funkey_current():
    xx = inspect.currentframe()
    assert xx
    xx = xx.f_back
    assert xx
    sourcefile = xx.f_code.co_filename
    sourceline = xx.f_code.co_firstlineno
    return f'{sourcefile}#{sourceline}'
