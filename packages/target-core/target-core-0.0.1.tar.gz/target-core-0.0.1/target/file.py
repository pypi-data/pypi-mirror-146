...
def config_file(config_path, datetime_format={
    ...
    config['path_template_default'] = path_template_default
    config['path_template'] = config['path_template'] \
        .replace('{date_time}', '{date_time:' + datetime_format['date_time_format'] + '}')
...
def config_compression(config_default):
    ...
    if f"{config.get('compression')}".lower() == 'gzip':
        ...
        config['path_template_default'] = config['path_template_default'] + '.gz'
        config['path_template'] = config['path_template'] + '.gz'

    elif f"{config.get('compression')}".lower() == 'lzma':
        config['open_func'] = lzma.open
        config['path_template_default'] = config['path_template_default'] + '.xz'
