...
class OsparcAPIcontroller:
    ...
    def run_jobs(self,
        ...
        for simname, (jid, sid) in all_jobs_metadata.items():
            ...
            status: JobStatus = self.solvers_api.start_job(sid, solver_version, jid)
            statuses[jid] = status
        ...
        while not self.all_done(statuses):
            ...
            for simname, (jid, sid) in all_jobs_metadata.items():
                time.sleep(0.5)
                status = self.solvers_api.inspect_job(sid, solver_version, jid)
                statuses[jid] = status
                job_statuses[simname] = f'Progress: {status.progress}/100, State: {status.state}'
    ...
    def download_job_output(self,
        ...
        outputs: JobOutputs = self.solvers_api.get_job_outputs(sid, solver_version, jid)
...
controller = OsparcAPIcontroller(host="https://api.osparc.speag.com",
                                 username="9abfedc6-ea78-54b4-8678-19881f5d2679",
                                 password="a42f3fd5-1e2c-58a2-bc3d-552b34d91781")

controller.print_solvers()

controller.initialize_solvers(solver_list=['isolve-mpi', 's4l Python Runner'])
print('\n')
pprint.pprint(controller.solvers_in_use)
