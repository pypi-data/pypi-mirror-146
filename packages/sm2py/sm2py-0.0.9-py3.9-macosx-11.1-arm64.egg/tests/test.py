...
from sm2py import sm2, functions
...
class TestECDH(TestCase):
    ...
    def test_encrypt_and_decrypt(self):
        with open('/Users/alex/Projects/temp/adv_static/adv/NP-HGL-A06_2_raw.zip', 'rb') as f:
            raw_bin = f.read()
            b64_zip_data = base64.urlsafe_b64encode(raw_bin)

        alice = sm2.make_keypair()
        bob = sm2.make_keypair()
        # enc = alice.publicKey.encrypt(bob.privateKey, raw_text.encode(), raw=True)

        print('origin b64len:', len(b64_zip_data))
        print('origin b64:\n', b64_zip_data)

        enc = alice.publicKey.encrypt(bob.privateKey, b64_zip_data)

        print("enc len:%s" % len(enc['enc']))

        res = bob.privateKey.decrypt(alice.publicKey, enc['enc'], enc['iv'])

        print(type(res))

        #with open('/Users/alex/Projects/temp/adv_static/adv/test-decrypt.zip', 'wb') as f4:
        #    f4.write(res)

        self.assertEqual(b64_zip_data, res)
