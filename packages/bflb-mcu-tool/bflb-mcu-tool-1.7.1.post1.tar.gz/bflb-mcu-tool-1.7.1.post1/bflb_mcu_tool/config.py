# -*- coding: utf-8 -*-

import os
import sys

version = "1.7.1"
chip_name = "tg7100c"
ENABLE_HAIER = False

# Get app path
if getattr(sys, "frozen", False):
    app_path = os.path.dirname(sys.executable)
else:
    app_path = os.path.dirname(__file__)

try:
    import changeconf as cgc
    conf_sign = True
except ImportError:
    cgc = None
    conf_sign = False
       
if not conf_sign:
    back_color = "#EFF4F7"
    list_chip = ["BL561/563", "BL606/608", "BL562/564", "BL602/604", "BL702/704/706"]
    list_chip = ["BL561/563", "BL606/608", "BL562/564", "BL602/604", "BL702/704/706", "BL808", "BL606P"]
    #list_chip = ["BL561/563", "BL606/608", "BL562/564", "BL602/604", "BL702/704/706", "BL808", "BL606P", "BL616", "WB03"]
    type_chip = ("bl602", "bl602")
    dict_chip = {
        "BL561/563": ("bl56x", "bl60x"),
        "BL606/608": ("bl60x", "bl60x"),
        "BL562/564": ("bl562", "bl602"),
        "BL602/604": ("bl602", "bl602"),
        "BL702/704/706": ("bl702", "bl702"),
        "BL808": ("bl808", "bl808"),
        "BL606P": ("bl606p", "bl808"),
        "BL616": ("bl616", "bl616"),
        "WB03": ("wb03", "wb03"),
    }    
    dict_chip_cmd = {
        "bl56x": "bl60x",
        "bl60x": "bl60x",
        "bl562": "bl602",
        "bl602": "bl602",
        "bl702": "bl702",
        "bl808": "bl808",
        "bl606p": "bl808",
        "bl616": "bl616",
        "wb03" : "wb03",
    }
    flash_dict = {
        "bl56x": "bl60x",
        "bl60x": "bl60x",
        "bl562": "bl602",
        "bl602": "bl602",
        "bl702": "bl702",
        "bl808": "bl808",
        "bl606p": "bl808",
        "bl616": "bl616",
        "wb03" : "wb03",
    }
    bl_factory_params_file_prefix = 'bl_factory_params_'      
else:
    dict_chip = cgc.dict_chip
    dict_chip_cmd = cgc.dict_chip_cmd
    list_chip = cgc.list_chip
    type_chip = cgc.type_chip
    back_color = cgc.back_color
    bl_factory_params_file_prefix = cgc.show_text_first_value  
  
       
def read_version_file(file_path):
    version_dict = {}
    with open(file_path, 'r', encoding='utf-8') as fp:
        for line in fp.readlines():
            line_list = line.strip().split('=')
            version_dict[line_list[0].strip()] = line_list[1].strip()
    return version_dict


xtal_type = {}
xtal_type_ = {}
pll_clk = {}
encrypt_type = {}
key_sel = {}
sign_type = {}
cache_way_disable = {}
flash_clk_type = {}
crc_ignore = {}
hash_ignore = {}
img_type = {}
boot_src = {}
cpu_type = {}

# BL60X
xtal_type['bl60x'] = ["None", "32M", "38.4M", "40M", "26M", "52M"]
xtal_type_['bl60x'] = ["XTAL_" + item for item in xtal_type['bl60x']]
pll_clk['bl60x'] = ["160M", "Manual"]
encrypt_type['bl60x'] = ["None", "AES128", "AES256", "AES192"]
key_sel['bl60x'] = ["0", "1", "2", "3"]
sign_type['bl60x'] = ["None", "ECC"]
cache_way_disable['bl60x'] = ["None", "OneWay", "TwoWay", "ThreeWay", "FourWay"]
flash_clk_type['bl60x'] = ["XTAL", "Manual"]
crc_ignore['bl60x'] = ["False", "True"]
hash_ignore['bl60x'] = ["False", "True"]
img_type['bl60x'] = ["CPU0", "CPU1", "SingleCPU", "BLSP_Boot2", "RAW"]
boot_src['bl60x'] = ["Flash", "UART/SDIO"]
cpu_type['bl60x'] = ["CPU0", "CPU1"]

# BL602
xtal_type['bl602'] = ["None", "24M", "32M", "38.4M", "40M", "26M", "RC32M"]
xtal_type_['bl602'] = ["XTAL_" + item for item in xtal_type['bl602']]
pll_clk['bl602'] = ["160M", "Manual"]
encrypt_type['bl602'] = ["None", "AES128", "AES256", "AES192"]
#key_sel['bl602'] = ["0", "1", "2", "3"]
key_sel['bl602'] = ["0"]
sign_type['bl602'] = ["None", "ECC"]
cache_way_disable['bl602'] = ["None", "OneWay", "TwoWay", "ThreeWay", "FourWay"]
flash_clk_type['bl602'] = ["XTAL", "Manual"]
crc_ignore['bl602'] = ["False", "True"]
hash_ignore['bl602'] = ["False", "True"]
img_type['bl602'] = ["SingleCPU", "BLSP_Boot2", "RAW"]
boot_src['bl602'] = ["Flash", "UART/SDIO"]

# BL702
xtal_type['bl702'] = ["None", "32M", "RC32M"]
xtal_type_['bl702'] = ["XTAL_" + item for item in xtal_type['bl702']]
pll_clk['bl702'] = ["144M", "Manual"]
encrypt_type['bl702'] = ["None", "AES128", "AES256", "AES192"]
#key_sel['bl702'] = ["0", "1", "2", "3"]
key_sel['bl702'] = ["1"]
sign_type['bl702'] = ["None", "ECC"]
cache_way_disable['bl702'] = ["None", "OneWay", "TwoWay", "ThreeWay", "FourWay"]
flash_clk_type['bl702'] = ["XCLK", "Manual"]
crc_ignore['bl702'] = ["False", "True"]
hash_ignore['bl702'] = ["False", "True"]
img_type['bl702'] = ["SingleCPU", "BLSP_Boot2", "RAW"]
boot_src['bl702'] = ["Flash", "UART/USB"]

# BL808
xtal_type['bl808'] = ["None", "24M", "32M", "38.4M", "40M", "26M", "RC32M", "Auto"]
xtal_type_['bl808'] = ["XTAL_" + item for item in xtal_type['bl808']]
pll_clk['bl808'] = ["WIFIPLL 320M", "Manual"]
encrypt_type['bl808'] = ["None", "AES CTR128", "AES CTR256", "AES CTR192", "AES XTS128", "AES XTS256", "AES XTS192"]
key_sel['bl808'] = ["0", "1", "2", "3"]
sign_type['bl808'] = ["None", "ECC"]
cache_way_disable['bl808'] = ["None", "OneWay", "TwoWay", "ThreeWay", "FourWay"]
flash_clk_type['bl808'] = ["XTAL", "Manual"]
crc_ignore['bl808'] = ["False", "True"]
hash_ignore['bl808'] = ["False", "True"]
img_type['bl808'] = ["SingleCPU", "BLSP_Boot2", "RAW"]
boot_src['bl808'] = ["Flash", "UART/USB"]
cpu_type['bl808'] = ["Group0", "Group1"]

# BL606P
xtal_type['bl606p'] = ["None", "24M", "32M", "38.4M", "40M", "26M", "RC32M", "Auto"]
xtal_type_['bl606p'] = ["XTAL_" + item for item in xtal_type['bl808']]
pll_clk['bl606p'] = ["WIFIPLL 320M", "Manual"]
encrypt_type['bl606p'] = ["None", "AES CTR128", "AES CTR256", "AES CTR192", "AES XTS128", "AES XTS256", "AES XTS192"]
key_sel['bl606p'] = ["0", "1", "2", "3"]
sign_type['bl606p'] = ["None", "ECC"]
cache_way_disable['bl606p'] = ["None", "OneWay", "TwoWay", "ThreeWay", "FourWay"]
flash_clk_type['bl606p'] = ["XTAL", "Manual"]
crc_ignore['bl606p'] = ["False", "True"]
hash_ignore['bl606p'] = ["False", "True"]
img_type['bl606p'] = ["SingleCPU", "BLSP_Boot2", "RAW"]
boot_src['bl606p'] = ["Flash", "UART/USB"]
cpu_type['bl606p'] = ["Group0", "Group1"]

# BL616
xtal_type['bl616'] = ["None", "24M", "32M", "38.4M", "40M", "26M", "RC32M", "Auto"]
xtal_type_['bl616'] = ["XTAL_" + item for item in xtal_type['bl616']]
pll_clk['bl616'] = ["WIFIPLL 320M", "Manual"]
encrypt_type['bl616'] = ["None", "AES CTR128", "AES CTR256", "AES CTR192", "AES XTS128", "AES XTS256", "AES XTS192"]
key_sel['bl616'] = ["0", "1", "2", "3"]
sign_type['bl616'] = ["None", "ECC"]
cache_way_disable['bl616'] = ["None", "OneWay", "TwoWay", "ThreeWay", "FourWay"]
flash_clk_type['bl616'] = ["XTAL", "Manual"]
crc_ignore['bl616'] = ["False", "True"]
hash_ignore['bl616'] = ["False", "True"]
img_type['bl616'] = ["SingleCPU", "BLSP_Boot2", "RAW"]
boot_src['bl616'] = ["Flash", "UART/USB"]

# WB03
xtal_type['wb03'] = ["None", "24M", "32M", "38.4M", "40M", "26M", "RC32M", "Auto"]
xtal_type_['wb03'] = ["XTAL_" + item for item in xtal_type['wb03']]
pll_clk['wb03'] = ["WIFIPLL 320M", "Manual"]
encrypt_type['wb03'] = ["None", "AES CTR128", "AES CTR256", "AES CTR192", "AES XTS128", "AES XTS256", "AES XTS192"]
key_sel['wb03'] = ["0", "1", "2", "3"]
sign_type['wb03'] = ["None", "ECC"]
cache_way_disable['wb03'] = ["None", "OneWay", "TwoWay", "ThreeWay", "FourWay"]
flash_clk_type['wb03'] = ["XTAL", "Manual"]
crc_ignore['wb03'] = ["False", "True"]
hash_ignore['wb03'] = ["False", "True"]
img_type['wb03'] = ["SingleCPU", "BLSP_Boot2", "RAW"]
boot_src['wb03'] = ["Flash", "UART/USB"]


try:
    from wx.lib.embeddedimage import PyEmbeddedImage
except ImportError:
    pass

try:
    logo1 = PyEmbeddedImage("iVBORw0KGgoAAAANSUhEUgAAAMIAAADCCAYAAAAb4R0xAAAACXBIWXMAAAsTAAALEwEAmp"
                           "wYAAAABGdBTUEAALGOfPtRkwAAACBjSFJNAAB6JQAAgIMAAPn/AACA6QAAdTAAAOpgAAA6"
                           "mAAAF2+SX8VGAAAIOElEQVR42uydPW/bVhhGnxTd7X8gZckUIAw0eTI5ZI6a7ooyBehSFx"
                           "kVIBIQj0U0FfAU2XsCee4gavJEgAYydan0D6RfoA68glnBlkmKlMjLcwAhH7Al8fKe+773"
                           "k09Wq5UA6s5PFAEAIgAgAgAiACACACIAIAIAIgAgAgAiACACACIAIAIAIgAgAgAiACACAC"
                           "IAIAIAIgAgAgAiACACACIAIAIAIgAgAgAiACACACIAIAIAIgAgAgAiACACACIAIAIAIgAg"
                           "AgAiACACACIAIALAofmZIiie1vmkKalp/nksyXngR2fmJUmLoOeFlN5+eLJarSiF/Cq8Yy"
                           "r5+tWU1MjhraeSFpJCSb6kWdDzZpQ4IpSp4rux19EeP35ppPAl+UQPRDhE5e9KaufU2uct"
                           "xljSOOh5C+4WIhSR45+VsPI/lk6tpSCNQoSdBOia1v+04pdybYQYcVcRIWnlPzatf7dCrX"
                           "+a9GksqU+USCGCqRTrfHihaNRiaGP+adKfvqS3Nbn3U0kjW6OE6cu1FQ1izMy1+llFCCW9"
                           "2PjvW0muLTLUUIBN5ub6xxbdU8cMHGyO4r17SPwHRWidT1xJkwc+q/IyIMC9adOw6hF/iw"
                           "SSdBv0POe+38u6xOKFyTUrKUDrfDKS9C8S/I8jSZ8kzVrnk75Ji22SQPdkNzuLIEmnpkIh"
                           "gL1CnFkkwToNTC1CmODz35ZdBgTYSYgvrfPJzAwjV10CmZ/J1FkeS3qd4LtcBj2vWzYB6A"
                           "Pk36ku2yhTCgkkyXto5OgxEbZ1mDeZSmofuqOFAPURwtTPcUIJpkHPczNFBPNhQ0m/J/xu"
                           "t5K6h1gAhgCHEUIHGnY1/ZcvKX7l5bZ6mUSEY9NfSDrLujQtxnCPrcJZwhQO8mc97Drax2"
                           "y1afBGSrfsZRD0vP62H0i0xCJlHhaPDmfbZvN2KIxjRTPeZ7JvGUSVuTZCjAu652fmlaYe"
                           "Juq/Jl5rlFGGtRDDXUOoaQlcRdPmtP7ljxIjRfskxjsK4Ohu5W+Wupdo4jfVorsdZIh/sb"
                           "FJtWbbcjaT8jQV7fRytWUyBEovha8Eu+tMq+/EXu4OET/V6ofUq09zkAGgaFIP56eeWTat"
                           "uGOMAygbgyxzWpmWWJjQ5kq6pNyhRCnYL4+NDuWWGt2TKrVNx4hUCQ7FzpO5uexQM52ckR"
                           "jNgf1HgVzmrHLdqmmiw1CM7UPxXCuap5rl8Wa571neYeIDIAlzRct4/DzftLDN+7G1P22E"
                           "gJwEKGyxX+GnWLAcAnJIgQpZtrFXETakcEyEaKtcM8XT2N9Pa1LB5ro7cLhZskbqWubUvn"
                           "0dO3Owc41i0+mu7g7MfVFA5Q4VHUWz0N2uuzDJUJsRtxn7nqcVrOy+ue5QCQ8P3ji92zV/"
                           "OopO8s67sZjq7hRwv4hFmqUWIeFNiBf+fcQrd+IKvqO8bZX79Lv1grfRPvaFmDVhceL3L8"
                           "4sFoF0qApfGRGqQgk3Ai1NP4wDgBHhIEI4pgU+ZJ9nIEtPIESEYipt27SaSTqIt7HUa52/"
                           "hlveu6/oGJR98ujW2Kub+XqAYp1mOto+nB0fOPA305rOScNHhOqLsGtlXR+ie+85mnuODo"
                           "MkC8yubuaukm9wT1sW3c5JY2xD3eBhguk4Mn2CyX3n/QQ9LzRHCg4K/A5TSU+TrrI0Lbhb"
                           "UFk4ttxYRMhOQ9JXE2W0IURf0kvlu2djqegQWzft2HrnpBGqmCXzx4gAW4lFhz9MJd5FgI"
                           "Gk5o7LC0YFXKY1EYHHyxYvxNAcOdlVumUmc1N5cxkN6pw0/Kub+TTnywsRAdLIsJA5cn3j"
                           "ARZSNDG31N0MeKhoLiD3StY5abjcDUQoTcpkU0tKHwGAiABV4s+//2kW1FkefXj1bIQIUB"
                           "UJxipmkm9kSzkhgt0CdFXsltkQESCPito2r6aqt3tv+eHVM0SAzAIcKxpKrfpzHHw6y5BV"
                           "Akf2nBs7RgTIysKkQjYQIgJk4sOrZzPFtitCeWBCDYCIUDwfv/84lkWrNCXNPr95PkMESE"
                           "tb0leLrucpqRFkwaZoMLAxGiACIqRlQWcZQOp//P7DQQSoeyt6JIsW2tFZ3i9DWTb59PH7"
                           "D+fzm+chIkBiPr957suydTmkRgCIAGAvpEZ74v1VMJRdQ6mS5F90Wn1EgCydZpueJ3f6/i"
                           "oYXnRaC1IjSMRFpzUzEeHWsktr00eA1DJcdFqOoiMcl4hAalR3Ifqmz9A2UaLIvkPRj7h6"
                           "jQiwiwwL7WmW9v1V0FX0mKtGQe/vXnRaPiJA2aUbydKlEfQRABABgNQIduTXv276Ob/l7N"
                           "tvJyNEgKqR99M/p1Xvg5AaASBCLdMih1JABLh74DogQq2jQRGHD/tVLxs6y/XDK+A9Z4gA"
                           "leHbbychpUBqBIAIAKRGxfKpdT75VOPrfxf0vBERAerOmNQI6s5l0PMWiAB1Z0hnGerOdd"
                           "DzQkSAunNm08UgAmRhEPS8GSJAnbkNel7ftotCBEjDUvY8JxoRIDNt21IiRIC0vAt6nm/r"
                           "xSECJJVgZPMFstYIHusTuDbNFxARIC1TSU4dJCAiwENRoB/0vGGdLpqIAHEuTRQY1u3CiQ"
                           "iwVHQ419DWoVFEgIeYKzp5Yhz0vDHFUT8R/Brf64WiZ7iFtuwhyJMnq9WKUoDaQ2cZABEA"
                           "EAEAEQAQAQARABABABEAEAEAEQAQAQARABABABEAEAEAEQAQAQARABABABEAEAEAEQAQAQ"
                           "ARABABABEAEAEAEQAQAQARABABABEAEAEAEQAQAQARABABABEAEAEAEQAQAQARABABABEA"
                           "EAEAEQAQAQARABABoIT8NwBhuwRVZfd7MQAAAABJRU5ErkJggg==")

    logo2 = PyEmbeddedImage('''''')
except Exception:
    logo1 = ""
    logo2 = ""

if conf_sign:
    about_tool = '''
            <html><body>
            <table bgcolor="''' + back_color + '''" width="100%" cellspacing="2" cellpadding="1" border="0">
            <tr>
            <td align="center"><font face="微软雅黑" color=\"grey\"><h2>{}</h2></font></td>
            </tr>
            <tr>
            <td align="center"><font face="微软雅黑" color=\"grey\" size=2>version: '''.format(cgc.title) + version + '''</font></td>
            </tr>
            </table>
            <br><br><br>
            <font color = \"black\" size=3 face="微软雅黑">
            <ul>
            <li>simple flasher
            <li>image tool
            <li>partition tool
            <li>security tool
            <li>efuse key tool
            <li>flash utils
            <li>iap tool
            <li>mfg tool
            </ul>
            </font>
            </body></html>
    '''
else:
    about_tool = '''\
                <html><body>
                <table bgcolor="''' + back_color + '''" width="100%" cellspacing="2"
                cellpadding="1" border="0">
                <tr>
                <td align="center"><font face="微软雅黑" color=\"grey\"><h2>Bouffalo Lab Dev Cube</h2></font></td>
                </tr>
                <tr>
                <td align="center"><font face="微软雅黑" color=\"grey\" size=2>version: ''' + version + '''</font></td>
                </tr>
                <tr>
                <td align="center"><font face="微软雅黑" color=\"grey\" size=2>jxtan@bouffalolab.com</font></td>
                </tr>
                </table>
                <br><br><br>
                <font color = \"black\" size=3 face="微软雅黑">
                <ul>
                <li>iot tool
                <li>mcu tool
                <li>partition tool
                <li>security tool
                <li>efuse key tool
                <li>flash utils
                <li>rf mfg tool
                </ul>
                </font>   
                </body></html>
    '''
