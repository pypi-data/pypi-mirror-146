...
class Test_Error_not_yet_allowed(unittest.TestCase):
    def test_Error_not_yet_allowed_with_message(self):
        a_message = broken_function_with_message(Error_not_yet_allowed)
        DEFAULT = Error_not_yet_allowed.DEFAULT_MESSAGE
        assert (a_message == DEFAULT + " A message ?All goodI continue running !")

    def test_Error_not_yet_allowed_without_message(self):
