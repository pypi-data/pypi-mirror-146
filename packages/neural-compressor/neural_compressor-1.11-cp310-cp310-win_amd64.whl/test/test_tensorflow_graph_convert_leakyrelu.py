...
class TestConvertLeaklyRelu(unittest.TestCase):
    ...
    @disable_random()
    def test_convert_leakyrelu_with_invalid_maximum_ndim(self):
        tf.compat.v1.disable_eager_execution()
        x = tf.compat.v1.placeholder(tf.float32, [1, 3, 12, 12], name="input")
        conv_weights = tf.compat.v1.get_variable("weight", [3, 3, 3, 32],
                                    initializer=tf.compat.v1.random_normal_initializer())
        conv = tf.nn.conv2d(x, conv_weights, strides=[1, 1, 1, 1],
                                    padding="SAME", data_format='NCHW')
        mul = tf.math.multiply(conv, tf.constant(np.random.random((1,32,12,12)).astype(np.float32)))
