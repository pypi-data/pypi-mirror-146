...
class Account(object):
    def __init__(self, provider, signer, account_id):
        ...
        self._access_key = provider.get_access_key(
            account_id, signer._key_pair.encoded_public_key())
    ...
    def _sign_and_submit_tx(self, receiver_id, actions):
        ...
        for outcome in itertools.chain([result['transaction_outcome']],
    ...
    def function_call(self,
        ...
        return self._sign_and_submit_tx(contract_id, [
            transactions.create_function_call_action(method_name, args, gas,
                                                     amount)
        ])
    ...
    def create_and_deploy_contract(self, contract_id, public_key,
                                   contract_code, initial_balance):
        actions = [
            transactions.create_create_account_action(),
            transactions.create_transfer_action(initial_balance),
            transactions.create_deploy_contract_action(contract_code)] + \
    ...
    def create_deploy_and_init_contract(self,
                                        contract_id,
                                        public_key,
                                        contract_code,
                                        initial_balance,
    ...
    def view_function(self, contract_id, method_name, args):
        result = self._provider.view_call(contract_id, method_name,
                                          json.dumps(args).encode('utf8'))
        if "error" in result:
            raise ViewFunctionError(result["error"])
        result["result"] = json.loads(''.join(
            [chr(x) for x in result["result"]]))
        return result
