...
class Hardware(BaseHardware):
  ...
  def HeartBeat(self):
    ...
    if self.delay_config > 0:
      ...
      if self.delay_config == 0:	# Delay sending these config parameters
        for name in ('hermes_disable_watchdog', 'hermes_reset_on_disconnect', 'TheBand'):
          self.ImmediateChange(name)
  ...
  def ImmediateChange(self, name):
    if name == 'keyupDelay':
      ...
      pass
      #try:
      #  band_num = self.conf.BandList.index(self.band) + 1
      #except:
      #  band_num = 0
      #self.WriteI2C(0x7d, 0x1D, 0, band_num)		# Send band number to IO Board
    else:
  ...
  def WriteI2C(self, bus, i2caddr, control, value):
    ...
    self.WriteQueue(1)
    if DEBUG: print ("Write I2C bus 0x%X, i2caddr 0x%X, control 0x%X, value 0x%X" % (bus, i2caddr, control, value))
  def ReadI2C(self, bus, i2caddr, control):
    ...
    if DEBUG: print ("Read  I2C bus 0x%X, 0x%X, 0x%X, 0x%X, 0x%X " % tuple(resp))
    return resp
