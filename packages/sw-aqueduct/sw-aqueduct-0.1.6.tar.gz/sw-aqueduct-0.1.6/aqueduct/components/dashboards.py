...
class Dashboards(Base):
    ...
    def __get_dashboard_items(self, source: Dashboard, destination: Dashboard):
        ...
        return_list = destination.items
        for item in source.items:
            if destination.items and item not in destination.items:
    ...
    def sync_dashboard(self, dashboard: Dashboard):
        ...
        if not self._is_in_include_exclude_lists(dashboard.name, 'dashboards'):
            ...
            dest_dashboard = None
            for d in self.destination_instance.get_dashboards():
            ...
            if not dest_dashboard:
                ...
                if self.update_dashboards:
                    self.__logger.info(f"Dashboard '{dashboard.name}' for workspaces '{dashboard.workspaces}' was found. Checking differences...")
