...
class BaseAuth:
    ...
    def _signin_azure(
        ...
        if result is None:
            logging.info("No suitable token exists in cache and get a new one from AAD.")
    ...
    @argument("device_code",
              type=bool,
              positional=False,
              description="use device authorization grant flow",
              aliases=["i"])
    @argument("daemon",
              type=bool,
              positional=False,
              description="sign in for a long-running, non-interactive daemon process",
              aliases=["d"])
    ...
    @command(aliases=["daemon"],
             help="Authenticate silently to the platform and obtain API access token. "
                  "This requires that SeqsLab API app admin consent has been granted.")
    @argument("scope",
              type=str,
              positional=False,
              description="scope of the application permission requested. default is `management`.",
