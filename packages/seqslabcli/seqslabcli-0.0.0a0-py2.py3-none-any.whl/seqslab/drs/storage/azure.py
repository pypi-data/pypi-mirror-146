...
"""
Copyright (C) 2019, Atgenomix Incorporated.
...
class BlobStorage(BaseBlob):
    ...
    def refresh_token(self, uri: URL, **kwargs):
        ...
        with requests.get(BaseBlob.DRS_REFRESH_SAS_URL,
    ...
    def get_token(self, path: str) -> dict:
        ...
        url = BaseBlob.DRS_SAS_URL.format(name=workspace)
    ...
    def blob_service_url(self, uri: URL, **kwargs) -> URL:
        ...
        if src:
            ...
            if os.path.basename(src) != os.path.basename(str(uri)):
            ...
            name = os.path.commonprefix([str(uri)[::-1], src[::-1]])[::-1]
    ...
    async def upload(
        ...
        return {
            "position": position,
            "dst": [str(uri.with_query("")), str(self._url_transform(uri.with_query("")))],
            "created_time": datetime.datetime.strftime(created_time, "%Y-%m-%dT%H:%M:%S.%f"),
            "region": workspace['location'],
            "access_methods_type": [uri.scheme, self._url_transform(uri.with_query("")).scheme]
        }
    ...
    async def expand_blob(self, drs_id, **kwargs) -> list[dict] or requests.HTTPError:
        ...
        with requests.get(
                BaseBlob.DRS_DOWNLOAD_URL.format(
                    API_HOSTNAME=kwargs.get('self_uri_host', API_HOSTNAME), drs_id=drs_id),
                headers={"Authorization": f"Bearer {token}"},
    ...
    async def download(
        ...
        if overwrite:
            ...
            if os.path.exists(file):
                raise ValueError('Make sure the the file needs to be overwritten or rename the dst name.')
            elif os.path.exists(temp_file):
                # start with temp
        ...
        for _, pos in enumerate(range(start, end_chunk, chunk_size)):
            end = min(pos + chunk_size, file_size) - 1
            tasks.append(
                self.get_blob(url=url, start=pos, file=temp_file, end=end,
                              chunk_size=chunk_size, md5_check=md5_check, bandwidth=kwargs.get('bandwidth'))
            )
        results = await asyncio.gather(*tasks, return_exceptions=True)
        [typed_results.append(x) for x in results if isinstance(x, int)]
        sent = self._sum_send(typed_results)
        ...
        if sent != size:
            # download fail
            try:
                self.success.get(temp_file).sort()
                for index, pos in enumerate(self.success):
                    if pos / chunk_size == index:
                        continue
                    else:
                        break
                length = self.success.get(temp_file)[index]
            except AttributeError:
                # catch self.success.get(temp_file).sort() exception
                length = 0
            os.truncate(temp_file, length)
            position = length
        else:
            position = sent
    ...
    async def get_blob(self, url: URL, file: str, start: int, end: int, **kwargs) -> int:
        ...
        while retry:
            try:
                if len(self.buffer) >= kwargs.get('bandwidth') and not self.error.get(file):
                    await asyncio.sleep(0.5)
                    continue
                elif self.error.get(file):
                    # if one task is failed all task stop
                    await asyncio.sleep(0.5)
                    return 0
                ...
                os.close(o_fd)
                self.buffer.pop(0)
                if not self.success.get(file):
                    self.success.setdefault(file, [start])
                else:
                    self.success[file].append(start)
                return end - start + 1
            except ClientResponseError as err:
                retry -= 1
                if retry == 0:
                    if self.success.get(file):
                        self.error.setdefault(file, 0)
                    else:
                        self.error[file] = 0
                    self.log(logging.ERROR, err)
                    return 0
