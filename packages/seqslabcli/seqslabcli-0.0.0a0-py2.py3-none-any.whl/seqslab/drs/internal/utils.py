...
async def drs_delete(names: List[str], labels: List[str], **kwargs):
    ...
    resp = await asyncio.gather(*search, return_exceptions=True)

    drs_ids = [ob.get('id') for res in resp for ob in res.get('objects')]
    cprint(f'drs to be delete {drs_ids}')

    delete = []
    for drs in drs_ids:
        delete.append(drs_crud(drs, "delete", **kwargs))
    return await asyncio.gather(*delete, return_exceptions=True)
