...
class SQLiteHandler:
    ...
    def __init__(self, name: str = None, *, check_same_thread=False):
        self.name = name
        self.conn = None
        self.cursor = None
        ...
        self.query = ''
        if name:
            self.connect(check_same_thread=check_same_thread)
        logging.debug(f'Created {type(self).__name__}({name=}) object.')
    ...
    def connect(self, name: str = None, *, check_same_thread=False):
        """Connects to the database"""
        if name is not None:
            self.name = name
        try:
            self.conn = sqlite3.connect(self.name, check_same_thread=check_same_thread)
