...
class DialogFolder(TLObject):  # type: ignore
    ...
    Parameters:
        folder (optional): :obj:`Folder <pyrogram.raw.base.Folder>`
        peer (optional): :obj:`Peer <pyrogram.raw.base.Peer>`
        top_message (optional): ``int`` ``32-bit``
        unread_muted_peers_count (optional): ``int`` ``32-bit``
        unread_unmuted_peers_count (optional): ``int`` ``32-bit``
        unread_muted_messages_count (optional): ``int`` ``32-bit``
        unread_unmuted_messages_count (optional): ``int`` ``32-bit``
