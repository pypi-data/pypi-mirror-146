...
class PhoneCall(TLObject):  # type: ignore
    ...
    Parameters:
        id (optional): ``int`` ``64-bit``
        access_hash (optional): ``int`` ``64-bit``
        date (optional): ``int`` ``32-bit``
        admin_id (optional): ``int`` ``64-bit``
        participant_id (optional): ``int`` ``64-bit``
        g_a_or_b (optional): ``bytes``
        key_fingerprint (optional): ``int`` ``64-bit``
        protocol (optional): :obj:`PhoneCallProtocol <pyrogram.raw.base.PhoneCallProtocol>`
        connections (optional): List of :obj:`PhoneConnection <pyrogram.raw.base.PhoneConnection>`
