...
class BindAuthKeyInner(TLObject):  # type: ignore
    ...
    Parameters:
        nonce (optional): ``int`` ``64-bit``
        temp_auth_key_id (optional): ``int`` ``64-bit``
        perm_auth_key_id (optional): ``int`` ``64-bit``
        temp_session_id (optional): ``int`` ``64-bit``
        expires_at (optional): ``int`` ``32-bit``
    """
