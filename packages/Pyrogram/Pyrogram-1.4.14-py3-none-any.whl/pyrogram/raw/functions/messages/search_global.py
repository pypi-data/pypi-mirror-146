...
class SearchGlobal(TLObject):  # type: ignore
    ...
    Parameters:
        q (optional): ``str``
        filter (optional): :obj:`MessagesFilter <pyrogram.raw.base.MessagesFilter>`
        min_date (optional): ``int`` ``32-bit``
        max_date (optional): ``int`` ``32-bit``
        offset_rate (optional): ``int`` ``32-bit``
        offset_peer (optional): :obj:`InputPeer <pyrogram.raw.base.InputPeer>`
        offset_id (optional): ``int`` ``32-bit``
        limit (optional): ``int`` ``32-bit``
