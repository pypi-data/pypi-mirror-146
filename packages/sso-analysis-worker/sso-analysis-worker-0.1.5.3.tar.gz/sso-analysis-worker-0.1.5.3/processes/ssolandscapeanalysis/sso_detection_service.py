...
class SSODetectionService:
    ...
    def automatic_sso_detection(self, chromedriver, latest_login_info, progress_callback=None, run_google=True,
                                run_facebook=True, run_apple=True) -> [tuple]:
        ids = []
        if run_google:
            ...
            if google_locate_result[0]:
                ids.append((self.provider_ids['google'], google_locate_result[1], latest_login_info.loginPath))
        if run_facebook:
            ...
            if facebook_locate_result[0]:
                ids.append((self.provider_ids['facebook'], facebook_locate_result[1], latest_login_info.loginPath))
        if run_apple:
            ...
            if apple_locate_result[0]:
                ids.append((self.provider_ids['apple'], apple_locate_result[1], latest_login_info.loginPath))
        if len(ids) == 0:
