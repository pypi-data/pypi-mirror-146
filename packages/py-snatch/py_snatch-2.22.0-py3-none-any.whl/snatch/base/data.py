    ...
    def find_current_status(self, data):
        ...
        * TIMEOUT     When timeout occurs
        ...
        if self.integration_status.is_success:
            ...
            expired_by_period = (
                data["max_periods_old"] is not None
                and months_old > data["max_periods_old"]
            )
            self.integration_status = (
                IntegrationStatus.EXPIRED
                if expired_by_date is True
                or expired_by_period is True
                or Validator(data).start() is False
