...
def __init__(hub):
    ...
    hub.idem.managed.KEEP_CACHE_FILE = False
...
async def context(
    ...
    try:
        ...
        # Remove the cache file, everything has been stored in the final destiniation
        ...
        try:
            if exception is not None:
                hub.log.error(
                    f"Exception occurred while handling ESM cache: {exception.__class__.__name__}: {exception}"
                )
