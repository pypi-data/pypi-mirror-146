...
class ProjectApps(Controller):
    ...
    def add_env(self):
        ...
        if response.is_succeed:
            if self.app.pargs.deploy:
                self.app.client.deploy_project_app_with_autolatest(
                    project_id=p_id, project_app_id=pa_id
                )
    ...
    @ex(
        help='Deploy Project App With Stable Version',
        arguments=[
            (['project_id'], {
                'help': 'Project ID',
                'action': 'store',
    ...
    def deploy_stable(self):
        project_id = self.app.pargs.project_id
        project_app_id = self.app.pargs.project_app_id

        response = self.app.client.deploy_project_app_with_stable_version(
            project_id=project_id, project_app_id=project_app_id
        )
    
        if response.is_succeed:
            self.app.render({}, renderer_type='text', custom_text="ProjectApp deployment has been started with the stable version.")
        else:
            self.app.render({}, renderer_type='text', custom_text=response.data['non_field_errors'])

    @ex(
        help='Deploy Project App With Last Completed Deployment',
        arguments=[
            (['project_id'], {
                'help': 'Project ID',
                'action': 'store',
    ...
    def redeploy(self):
        ...
        response = self.app.client.deploy_project_app_with_autolatest(
            project_id=project_id, project_app_id=project_app_id
        )
    
        if response.is_succeed:
            self.app.render({}, renderer_type='text', custom_text="ProjectApp deployment has been started with the last completed deployment.")
        else:
            self.app.render({}, renderer_type='text', custom_text=response.data['non_field_errors'])
