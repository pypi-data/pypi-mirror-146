...
class Request(BaseRequest):
    ...
    def __eq__(self, obj: Any, /) -> bool:
        return isinstance(obj, self.__class__) \
            and type(self._method) == type(obj._method) and self._method == obj._method \
            and type(self._params) == type(obj._params) and self._params == obj._params \
...
class BatchRequest(BaseBatchRequest[Request | Error]):
    ...
    def __repr__(self) -> str:
        return f"<{__package__}.{self.__class__.__name__}({self._requests!r})>"

    def __hash__(self) -> int:
        return hash(frozenset(self._requests))

    def __eq__(self, obj: Any, /) -> bool:
        return isinstance(obj, self.__class__) \
            and len(self) == len(obj) \
            and frozenset(self) == frozenset(obj)
    ...
    @property
    def requests(self) -> tuple[Request | Error, ...]:  # pragma: no cover
        """
        Returns the :py:class:`tuple` object representing a copy of the :class:`jsonrpc.BatchRequest` iterable object.
