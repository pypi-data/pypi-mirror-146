...
class RsgExternal(mx.core_RsgExternal):
    def __init__(self, sampleNum, dimension, filename):
...
class Scenario:
    ...
    def clone(self, **kwargs):
        try:
            ...
            _rsg = self.rsg
            require_dimension = (len(self.timegrid) - 1) * len(self.models)

            if isinstance(self.rsg, RsgExternal):
                if self.rsg.dimension != require_dimension:
                    raise Exception('dimension must be {0}'.format(require_dimension))
            else:
                _rsg = Rsg(self.rsg.sampleNum, require_dimension, self.rsg.seed, self.rsg.skip, self.rsg.isMomentMatching,
    ...
    def getModel(self, name) -> mx.core_ProcessModel:
    ...
    def _write_log(self, scen):
        ...
        # results = np.load(scen.filename)
        # d = {
        #     **results,
        #     'input_json': utils.toJsonStr(scen)
        # }

        # np.savez(scen.filename, **d)
    ...
    def generate_clone(self, **kwargs):
        ...
        core = mx.core_ScenarioGenerator2(scen.models, scen.calcs, scen.corr, scen.timegrid, scen.rsg, scen.filename, scen.isMomentMatching, hashCode, json_input)
        core.generate()
