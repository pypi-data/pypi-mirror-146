...
class trd():
    ...
    def ksrun(self):
        ...
        for i in family:# 去除不在染色体上的家族成员
            if i not in gff.keys():
                family.remove(i)
        return lens,chr_list,gff,ks,family,blast
    ...
    def run(self):
        ...
        lens,chr_list,gff,ks,family,blast = self.ksrun()
        ...
        # print(len(tp_list),len(prox_list))
                    ...
                    if id2 in gff.keys():
                        ...
                        if id1 in wgd_list and id2 not in wgd_list:
                            if id2 not in out_list:
                                ...
                                out_list.append(id2)
                                posi = gff[id2]['end'] + self.start_list[gff[id2]['chr']]
                            ...
                            if id1 not in out_list:
                                ...
                                out_list.append(id1)
                                posi = gff[id1]['end'] + self.start_list[gff[id1]['chr']]
                                start = self.to_radian(posi, total_size)
                                self.plot_arc_block(start, self.radius_b)# 基因
        outfile.close()
        ...
        #     if id1 == id2:
        #         continue
        #     if id2 not in tp_list and id1 in wgd_list and id2 not in wgd_list:
        #         if id2 not in out_list:
        #             outfile.write(str(id2) + '\n')
        #             out_list.append(id2)
        #     elif id1 not in tp_list and id2 in wgd_list and id1 not in wgd_list:
        #         if id1 not in out_list:
        #             outfile.write(str(id1) + '\n')
        #             out_list.append(id1)
        # outfile.close()
        ...
        pair = list(d for d in itertools.product(out_list,wgd_list))
        put0 = []
        for i in pair:
            id1,id2 = i[0],i[1]
            pos1,pos2 = gff[id1]['end'],gff[id2]['end']
