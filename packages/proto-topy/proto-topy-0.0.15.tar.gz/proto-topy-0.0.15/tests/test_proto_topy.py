...
def test_compile_missing_dependency():
    with pytest.raises(CompilationFailed, match=r'other.proto: File not found.*'):
        ProtoModule(file_path=Path("test.proto"),
                    source='syntax = "proto3"; import "other.proto";').compiled(protoc_path)
...
def test_encode_message():
    ...
    ProtoCollection(protoc_path, proto1, proto2).compile()
    assert array('B', proto1.py.Test7(foo=124).SerializeToString()) == array('B', [8, 124])
    assert array('B', proto2.py.Test8(foo=123).SerializeToString()) == array('B', [8, 123])
