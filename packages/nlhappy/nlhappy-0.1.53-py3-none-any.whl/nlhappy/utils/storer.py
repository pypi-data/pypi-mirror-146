...
default_spacy_bucket = 'spacy-pipeline'
...
class OSSStorer:
    ...
    def __init__(
        ...
        spacy_bucket: str = default_spacy_bucket
        ):
        super().__init__()
        ...
        self.spacy_bucket = oss2.Bucket(self.auth, endpoint, spacy_bucket)
    ...
    def download_pipeline(
        ...
        file = pipeline + '.zip'
        file_path = localpath + file
        model_path = localpath + pipeline
        if not os.path.exists(model_path):
            try:
                self.spacy_bucket.get_object_to_file(key=file, filename=file_path)
                with zipfile.ZipFile(file=file_path, mode='r') as zf:
                    zf.extractall(path=localpath)
            finally:
                if os.path.exists(file_path):
    ...
    def upload_pipeline(
        ...
        file = pipeline + '.zip'
        file_path = localpath + file
        pipe_path = localpath + pipeline
        with zipfile.ZipFile(file=file_path, mode='w') as z:
            zip_all_files(pipe_path, z, pipeline)
        self.spacy_bucket.put_object_from_file(key=file, filename=file_path)
        if os.path.exists(file_path):
            os.remove(path=file_path)
