...
class RESTAPI :
    ...
    def post_matchParticipants(self, match) :
        for participant in match['info']['participants'] : 
            ...
            league = self.get_league_by_summonerid(summoner['id'])
            if len(league) == [] :
                api_url=URL['leagues'].format(url='/')
                league = r_api.get_league_by_summoner_id(summoner['id'])
                self.post_league(league)

            championMastery = self.get_championmastery_by_championid_and_summonerid(participant['championId'], summoner['id'])
            if len(championMastery) == [] :
