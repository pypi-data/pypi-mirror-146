...
def struct_from_inputfile_CIF ( fname:str ) -> dict:
  ...
  for k,v in data_blocks.items():
    ...
    del v['data']['_shelx_res_file']
    del v['data']['_shelx_hkl_file']
    ...
    struct['abc'] = abc = np.array(abc)

    for l in loops:
      if '_space_group_symop_operation_xyz' in l[0]:

        spos = []
        sspec = []
        labels = ['x', 'y', 'z']
        for ia,tvec in enumerate(abc):
          uvecs = []
          for sop in l[1][1:]:
            tv = tvec.copy()
            sstrs = sop[0][1:-1].replace(' ', '').split(',')
            for i,s in enumerate(sstrs):
              if '-{}'.format(labels[i]) in s:
                tv[i] *= -1
              if '1/2' in s:
                if '-1/2' in s:
                  tv[i] -= .5
            ...
            exists = False
            for uv in uvecs:
              if np.allclose(tv, uv, rtol=1e-2, atol=1e-3):
                exists = True
            if not exists:
              uvecs.append(tv)
              sspec.append(spec[ia])
          spos += uvecs

    struct['abc'] = np.array(spos)
    struct['nat'] = struct['abc'].shape[0]
    struct['species'] = sspec
    print(len(spos))
    print(spos)
  return struct
