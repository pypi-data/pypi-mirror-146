...
from cameo_claw.cameo_claw import it_chunk_pd, it_chunk_pl

from pydantic import BaseModel

from cameo_claw.cameo_claw_reuse import pd_read_csv
...
def it_streaming_pd(i: ItemUrlFilter):
    try:
        yield write_csv_header_to_gz_pd(i)
        for int_progress, bytes1 in it_chunk_pd(
        ...
        print('it_streaming_pd,exception:', e)


def it_streaming_pl(i: ItemUrlFilter):
    try:
        yield write_csv_header_to_gz_pl(i)
        for int_progress, bytes1 in it_chunk_pl(
                i.lst_url, i.target_directory, i.lst_distinct_column, i.lst_column_match, i.sort_column):
            if bytes1:
                yield bytes1
            else:
...
@app.get("/api/streaming_get_pd/")
def streaming_get_pd(md5_hex):
    dic = {'content-disposition': f"attachment; filename*=utf-8''all.csv.gz"}
    return StreamingResponse(it_streaming_pd(dic_item_url_filter[md5_hex]), headers=dic)
...
def server(host='0.0.0.0', int_port=20448):
