...
class ObservableMixin:
    ...
    def add_listener(self, listen: Callback, *,
                     predicate: Optional[Predicate] = None) -> None:
        '''High priority queue is default
        '''

        self._add_listener(listen, queue=self._high_listeners,
                           predicate=predicate)

    def add_listener_low_priority(self, listen: Callback,
    ...
    def _add_listener(self, listen: Callback, *, queue,
    ...
    async def wait_for_notified_tasks(self):
        try:
            await asyncio.wait_for(asyncio.gather(*self.notified_tasks), timeout=5)
        except asyncio.TimeoutError:
            import logging
            logger = logging.getLogger(self.__class__.__name__)
            logger.warning('Timeout while waiting notified tasks')
    ...
    async def conditional_listener(self, action: Callback,
                                   action_condition: Predicate, event: Any) -> None:
