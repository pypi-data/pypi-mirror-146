...
import io
from typing import Tuple, TextIO, NamedTuple, List
...
class SimulatorHandler:
    ...
    def __init__(self, clock: Clock,
                 simulator_environment: SimulatorManagerEnvironment) -> None:
        self.clock = clock
    ...
    async def log_finished_events(self, simulator: Simulator, sim_event_log_file: TextIO):
        ...
        while True:
            try:
                ...
                event_row = f'{event.identifier};{event.environment_id};{event.arrival_time};{event.start_time};{event.finished_time};{event.info}\n'
    ...
    async def async_run(self):
        ...
        if self.debug:
            ...
            self.profile = SimulatorHandler.Profile([str(i) for i in snapshot_stats[:10]],
                                                    f'{total:.2f} {unit}')
    ...
    async def setup(self):
        ...
        self.sim_log_name = os.path.join(simulator_results_path,
                                         LOG_TEMPLATE.format(time=current_time,
                                                             pid=os.getpid(),
                                                             ftype='sim_results'))
        self.sim_event_logger_name = os.path.join(simulator_results_path,
                                                  LOG_TEMPLATE.format(time=current_time,
                                                                      pid=os.getpid(),
                                                                      ftype='events'))
        ...
        with ExitStack() as exit_stack:
            ...
            track_resources_modification = partial(log_resources_modification,
                                                   sim_log_file)
            simulator = Simulator(self.sim_env.event_queue_loader,
                                  self.sim_env.sim_event_scheduler)
    ...
    async def stop(self):
        ...
        if self.running:
            ...
            self.logger.info('simulation has stopped')
