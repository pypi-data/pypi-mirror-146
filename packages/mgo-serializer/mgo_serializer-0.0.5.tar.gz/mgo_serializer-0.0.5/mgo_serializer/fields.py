...
class BaseField(object):
    ...
    def __init__(self, initial=Empty, proto_field=None, default=None, custom=False, context=None, **kwargs):
        ...
        self.pb = self.meta.get('schema', None) if self.meta else None
    ...
    def get_value(self, dictionary, instance):
        ...
        # we convert `_id` to `id` to prevent `_id` from being accessed as a private field
        if '_id' in dictionary and isinstance(dictionary, dict):
...
class ListField(BaseField):
    ...
    ALLOWED_TYPES = ['CharField', 'DateTimeField', 'IntField', 'ObjectIdField', 'CustomField', 'BooleanField', 'FloatField']

    def __init__(self, type=None, **kwargs):
        self.type = type
        if type and type.__class__.__name__ not in self.ALLOWED_TYPES:
            raise ValueError(
...
class TimestampField(BaseField):
    ...
    def __init__(self, type=None, **kwargs):
        self.type = IntField() if type is None else type
        if type and type.__class__.__name__ not in self.ALLOWED_TYPES:
            raise ValueError(
                'type {!r} is not allowed for {!r}. '
                'Allowed types are: {}'.format(
