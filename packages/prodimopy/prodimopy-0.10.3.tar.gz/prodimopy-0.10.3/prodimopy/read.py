...
class Data_ProDiMo(object):
  ...
  def __init__(self,name):
    ...
    """
    self._chemnet=None
    """ :class:`prodimopy.chemistry.ReactionsNetworkPout` :
...
#    The routine tries to read the files from the archive also considering the `directory` path
  ...
  # FIXME: this is inconsistent with spmasses, where the electon is at the end
  ...
  for iz in range(data.nz):
    ...
    for ix in range(data.nx):
      ...
      data.radFields[ix,zidx,:]=fields[(iASpec+iASpecskip):iAJJ]
...
def read_species(directory,pdata,filename="Species.out",tarfile=None):
  ...
  ..todo :
    * this should read everything from Species.out
    * having a Data Structure for Species would also be nice
    * setting of nspec here is also not very nice
  ...
  if not "e-" in pdata.spmasses.keys():
    pdata.spmasses["e-"]=const.m_e.cgs.value

  pdata.nspec=len(pdata.spmasses)
