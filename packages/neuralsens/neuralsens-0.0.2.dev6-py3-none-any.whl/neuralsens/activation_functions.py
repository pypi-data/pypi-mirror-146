...
def der_2_activation_function(func:str):
    def sigmoid(x):
        ...
        fx = expit(x)
        return np.fill_diagonal(zeros_3d, fx * (1 - fx) * (1 - 2 * fx))
    def identity(x):
        return np.zeros((x.shape[0],x.shape[0],x.shape[0]), dtype=int)
    def tanh(x):
        zeros_3d = np.zeros((x.shape[0],x.shape[0],x.shape[0]),dtype=float)
        fx = tanh(x)
        return np.fill_diagonal(zeros_3d, -2 * fx * (1 - fx^2))
    def relu(x):
        return np.zeros((x.shape[0],x.shape[0],x.shape[0]), dtype=int)
    der2actfunc = {
        'logistic': sigmoid,
...
def der_3_activation_function(func:str):
    def sigmoid(x):
        ...
        fx = expit(x)
        return np.fill_diagonal(zeros_4d, fx * (1 - fx) * (1 - 2 * fx) * (1 - 3 * fx))
    def identity(x):
        return np.zeros((x.shape[0],x.shape[0],x.shape[0],x.shape[0]), dtype=int)
    def tanh(x):
        zeros_4d = np.zeros((x.shape[0],x.shape[0],x.shape[0], x.shape[0]),dtype=float)
        fx = tanh(x)
        return np.fill_diagonal(zeros_4d, -2 * (1 - 4 * fx ** 2 + fx ** 4))
    def relu(x):
        return np.zeros((x.shape[0],x.shape[0],x.shape[0],x.shape[0]), dtype=int)
    der3actfunc = {
        'logistic': sigmoid,
