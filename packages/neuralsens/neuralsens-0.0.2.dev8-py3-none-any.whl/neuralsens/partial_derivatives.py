...
def hessian_mlp(wts, bias, actfunc, X, y, 
    ...
    D_ = [np.identity(mlpstr[sens_origin_layer]) for irow in n_samples]
    if sens_origin_input:
    ...
    if not (sens_origin_layer == sens_end_layer):
        for layer in range(sens_origin_layer + 1, sens_end_layer):
            counter += 1 
            # First derivatives der z^L / der x_i
...
def jerkian_mlp(wts, bias, actfunc, X, y, 
    ...
    M = [np.zeros((X.shape[0],mlpstr[0],mlpstr[0],mlpstr[0],mlpstr[0]))]
    N = [np.zeros((X.shape[0],mlpstr[0],mlpstr[0],mlpstr[0],mlpstr[0]))]
    J = [D3[0]]
    ...
    if not (sens_origin_layer == sens_end_layer):
        for layer in range(sens_origin_layer + 1, sens_end_layer):
            ...
            # Third derivatives
            m = np.matmul(np.matmul(J[counter-1], W[layer][1:,:]).swapaxes(0,1), D[counter]).swapaxes(0,1)
            n = np.matmul(d_accum, np.matmul(np.matmul(np.matmul(H[counter-1], W[layer][1:,:]).swapaxes(1, 2), W[layer][1:,:]).swapaxes(0,1), D2[layer])) 
            j = np.matmul(d_accum, np.matmul(d_accum, np.matmul(d_accum, D3[layer].swapaxes(0,1)).swapaxes(0,2)).swapaxes(0,3)).swapaxes(0,1)
            M.append(m)
            N.append(n)
