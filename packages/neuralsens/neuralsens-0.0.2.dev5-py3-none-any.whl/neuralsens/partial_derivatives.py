...
from neuralsens.activation_functions import activation_function, der_activation_function, der_2_activation_function 
...
def jacobian_mlp(wts, bias, actfunc, X, y, 
    ...
    D = [np.array([deractfunc[0](Z[0][irow,]) for irow in n_samples])]
    ...
    for lyr in range(1,len(mlpstr)):
        ...
        D.append(np.array([deractfunc[lyr](Z[lyr][irow,]) for irow in n_samples]))
...
def hessian_mlp(wts, bias, actfunc, X, y, 
                    sens_origin_layer=0, sens_end_layer='last', sens_origin_input=True, sens_end_input=False):
    ### Initialize all the necessary variables
    # Structure of the mlp model
    mlpstr = [wts[0].shape[0]] + [lyr.shape[1] for lyr in wts] 
    ...
    D2 = [np.array([der2actfunc[0](Z[0][irow,]) for irow in n_samples])]
    ...
    for lyr in range(1,len(mlpstr)):
        ...
        D2.append(np.array([der2actfunc[lyr](Z[lyr][irow,]) for irow in n_samples]))
    ...
    D_accum = [np.identity(mlpstr[sens_origin_layer]) for irow in n_samples]
    if sens_origin_input:
        D_accum = [D[sens_origin_layer]]
    Q = [np.zeros((X.shape[0],mlpstr[0],mlpstr[0],mlpstr[0]))]
    H = [D2[0]]
    # d_accum = D_accum[counter - 1] @ W[layer][1:,:] @ D[layer]
    # for irow in range(X.shape[0]):
    #     # Calculate the derivatives of the layer based on the previous and the weights
    #     q[irow,:,:,:] = np.array([np.dot(np.array([np.dot(H[counter-1][irow,:,input,:],W[layer][1:,:]) for input in range(X.shape[1])])[:,input,:], D[counter][irow,:,:]) for input in range(X.shape[1])])
    #     h[irow,:,:,:] = np.transpose(np.array([np.dot(d_accum[irow,:,:], np.array([np.dot(d_accum[irow,:,:],D2[layer][irow,:,:,neuron]) for neuron in range(mlpstr[layer])])[:,:,neuron]) for neuron in range(mlpstr[layer])]),[1,2,0])
    ...
    if not (sens_origin_layer == sens_end_layer):
        for layer in range(sens_origin_layer + 1, sens_end_layer):
            ...
            h = np.zeros((X.shape[0],mlpstr[sens_origin_layer],mlpstr[sens_origin_layer],W[layer].shape[1]))
            ...
            h = np.matmul(d_accum, np.matmul(d_accum, D2[layer].swapaxes(0,1)).swapaxes(0,2)).swapaxes(0,1)
...
class Hessian_mlp:
    ...
    def hesstosens(self):
        ...
        return temp_self
def jerkian_mlp(wts, bias, actfunc, X, y, 
                sens_origin_layer=0, sens_end_layer='last', sens_origin_input=True, sens_end_input=False):
    ### Initialize all the necessary variables
    # Structure of the mlp model
    mlpstr = [wts[0].shape[0]] + [lyr.shape[1] for lyr in wts] 
    ...
    D3 = [np.array([der3actfunc[0](Z[0][irow,]) for irow in range(Z[0].shape[0])])]
    ...
    for lyr in range(1,len(mlpstr)):
        ...
        # Calculate second derivative of each layer
        D3.append(np.array([der3actfunc[lyr](Z[lyr][irow,]) for irow in range(X.shape[0])]))
