...
class KafkaToolbox:
    ...
    def create_topics(self, topic_cluster_dict, num_partitions=3, replication_factor=3, topic_config=None):
        ...
        if "hack" in topic_cluster_dict:  # TODO: get rid of this (exists cuz of dude stuff)
            del topic_cluster_dict['hack']
    ...
    def delete_topics(self, topic_cluster_dict):
        if isinstance(topic_cluster_dict, list):
            ...
            for cluster, client in self.admin_clients.items():
                all_topics.update({topic: cluster for topic in client.list_topics().topics.keys() if topic in topic_cluster_list})
            for topic in topic_cluster_list:
                topic_cluster_dict[topic] = self.topic_configs[topic]["cluster"] if topic in self.topic_configs else all_topics[topic]
