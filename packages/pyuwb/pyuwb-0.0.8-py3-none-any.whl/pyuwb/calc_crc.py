__author__ = "songjiangshan"
__copyright__ = "Copyright (C) 2021 songjiangshan \n All Rights Reserved."
__license__ = ""
__version__ = "1.0"


auchCRCHi=[
        
]

auchCRCLo=[
        
]


def calc_crc(payload:bytes):
    """
    :param payload:不带校验和的报文
    :return: 返回2byte的校验和
    """
    uchCRCHi = 0xFF
    uchCRCLo = 0xFF
    uIndex=0
    num=len(payload)
    index=0
    while (num>0):
        num-=1
        uIndex = uchCRCHi ^ payload[index]
        index += 1
        uIndex &=0xff
        uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex]
        uchCRCLo = auchCRCLo[uIndex]
        uchCRCHi &= 0xff


    return  uchCRCHi.to_bytes(1,'big') + uchCRCLo.to_bytes(1,'big')



def check_crc(payload:bytes):
    """

    :param payload: 带校验和的报文，最后2byte的校验和会被用于比较
    :return: 正确返回True, 否则返回false
    """
    crc = calc_crc(payload[:-2])
    if crc == payload[-2:]:
        return True

    return False


if __name__=='__main__':
    crc = calc_crc(b'\x01\x10\x00\x28\x00\x01\x02\x00\x04')
    print(crc)
    ret = check_crc(b'\x01\x10\x00\x28\x00\x01\x02\x00\x04\xA1\xBB')
    print(ret)