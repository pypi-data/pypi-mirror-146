...
import copy
import json
sys.path.append(os.getcwd().replace("Flat_DQN/dialogue_system/run",""))

from .agent import AgentRule
from .agent import AgentDQN
from .user_simulator import UserRule as User
from .dialogue_manager import DialogueManager
from .agent import dialogue_configuration
import random
...
class RunningSteward(object):
    ...
    def evaluate_model(self, dataset, index, mode = 'eval'):
        ...
        pickle.dump(file=open(os.path.join('Flat_DQN/dialogue_system/records/', name), "wb"), obj=self.dialogue_manager.record)
        ''' 
        ...
        if self.parameter['save_record'] and not(self.parameter['train_mode']):
            ...
            with open('Flat_DQN/dialogue_system/records/' + name + '.json', 'w', encoding = 'utf-8') as f:
