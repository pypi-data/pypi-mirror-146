...
import torch
#import wandb
sys.path.append(os.getcwd().replace("HRL/run",""))

from OpenMedicalChatBox.HRL.agent import AgentDQN
from OpenMedicalChatBox.HRL.utils import verify_params
from OpenMedicalChatBox.HRL import RunningSteward

from OpenMedicalChatBox.HRL.agent import AgentHRL_joint2
...
class HRL:
    def __init__(self, dataset_path, model_save_path, model_load_path, lr = 0.0005, groups = 4, cuda_idx = 0, train_mode = True, max_turn = 10, \
        reward_for_not_come_yet = 0, reward_for_success = 20, reward_for_fail = 0, reward_for_inform_right_symptom = 30, reward_for_reach_max_turn = -100, \
            reward_for_repeated_action = -4, epoch_number = 5000, epoch_size = 100, experience_replay_size = 10000, batch_size = 100, discount_factor = 1, discount_factor_worker = 0.9, greedy = 0.1, \
                reward_shaping = 1):
        '''
                self, dataset_path, model_save_path, model_load_path, lr = 0.0005, groups = 4, cuda_idx = 0, train_mode = True, max_turn = 10, \
        reward_for_not_come_yet = 0, reward_for_success = 20, reward_for_fail = 0, reward_for_inform_right_symptom = 6, reward_for_reach_max_turn = -100, \
            reward_for_repeated_action = -4, epoch_number = 5000, epoch_size = 100, experience_replay_size = 10000, batch_size = 100, discount_factor = 1, warm_start = False, warm_start_number = 30
        '''
        disease_number = 10
        ...
        parser.add_argument("--gamma_worker", dest="gamma_worker", type=float, default=discount_factor_worker, help="The discount factor of immediate reward of the lower agent in HRL.")
        ...
        parser.add_argument("--weight_for_reward_shaping", dest='weight_for_reward_shaping', type=float, default=reward_shaping, help="weight for reward shaping. 0 means no reward shaping.")
...
if __name__ == "__main__":
    HRL_test = HRL(dataset_path = '/remote-home/czhong/RL/OpenMedicalChatBox_install/Data/mz4/HRL//', model_save_path = './simulate', groups = 2, model_load_path = './simulate', cuda_idx = 1, train_mode = True)
    HRL_test.run()
