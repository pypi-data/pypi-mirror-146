...
def trigger_workflow(workflow, event_type, data, service_url, api_access_key):
    ...
    if conf.get('external_workflow_config', ''):
        ...
        if external_workflow_config:
            ...
            if search_object and search_object.get('condition_object', ''):
                ...
                if event_type == 'import' or event_type == 'manual':
                    list_of_data = [data]
                    trigger_workflow(workflow, event_type, list_of_data, vtiger_access.get('service_url'), vtiger_access.get('api_access_key'))
                else:
                    trigger_workflow(workflow, event_type, data, vtiger_access.get('service_url'), vtiger_access.get('api_access_key'))
        else:
...
def invoke_web_service_task(conf, task_id):
    """ Web service task:
        params:
        1. conf: conf object
        2. task_id: current task id
