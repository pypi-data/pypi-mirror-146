...
@data_store_cli.command(name='ls-local')
...
def create_data_stores(ctx, token=None):
    ...
    cmds = [f"gcloud beta healthcare fhir-stores import gcs  {data_store}  --dataset=$GOOGLE_DATASET  --location=$GOOGLE_LOCATION --gcs-uri=gs://$GOOGLE_BUCKET/fhir/IG/*.json --content-structure=resource-pretty" for data_store in workspace_mapping['data_store'].keys() if data_store in data_store_list]
    if len(cmds) > 0:
        print(run_cmd("\n".join(cmds)))
