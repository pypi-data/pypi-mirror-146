...
for cat in catL:
    eng_unit = display_def_unitsD[cat]
    if eng_unit=='degF':
        eng_unit = 'degR'
    pint_eng_unit = equivUnitD.get( eng_unit, eng_unit )
    pint_eng_unit = pint_eng_unit.replace( '-','*' )
    
    print(pad+"# ", cat, " Using English ref of 1.0", eng_unit, '==', pint_eng_unit)
    
    if cat=="Isp" and eng_unit=='sec':
        eng_val = 1.0 * ureg( 'lbf*sec/lbm' )
    else:
        #print('Getting eng_val of eng_unit=', eng_unit)
        eng_val = 1.0 * ureg( pint_eng_unit )
    ...
    for unit in display_unitsD[cat]:
        ...
        try:
            ...
            print( "Failed to convert", eng_unit , " to", unit, '..or..', u )
            val = eng_val.to( unit )
            sys.exit()
        print( pad+"self.assertAlmostEqual(1.0, %g / "%val.magnitude +\
            'convert_value(inp_val=1.0, inp_units="%s", out_units="%s"), places=5)'%(eng_unit, unit))
