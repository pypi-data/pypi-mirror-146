...
class DistributedFeaturizationPhase:
    ...
    def run(workspace_getter: Callable[..., Any],
        ...
        with tracer.start_as_current_span(
            ...
            long_grain_key_values = [dt for dt in all_grain_key_values if convert_grain_dict_to_str(
                dt) not in expr_store.metadata.timeseries.short_grain_names]
            ...
            with logging_utilities.log_activity(logger=logger, activity_name='SavingGlobalStatistics'):
                set_global_statistics(expr_store, long_grain_key_values, list(log_transform_tests_tuple))
...
def _transform_one_grain(grain_keys_values: Dict[str, Any]) -> TransformOneGrainResults:
    ...
    result = _GrainTransformResult(
        stateful_transformers=(distributed_timeseries_util.convert_grain_dict_to_str(
            grain_keys_values), stateful_transformers),
...
def _fit_bootstrap_transformer(
        tst: TimeSeriesTransformer, all_grain_key_values: List[Dict[str, Any]],
    ...
    :param all_grain_key_values: The list of grain_column -> value dictionary.
    ...
    for last_seen_grain, grain_key_values in enumerate(all_grain_key_values):
        grain_str = convert_grain_dict_to_str(grain_key_values)
