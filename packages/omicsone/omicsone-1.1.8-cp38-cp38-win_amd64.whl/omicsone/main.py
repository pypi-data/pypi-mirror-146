...
omicsone_src_dir = os.path.dirname(Path(dir_path))
# print(dir_path)
# print(omicsone_src_dir)
...
class OmicsOneGUI(tk.Frame):
    def __init__(self, master):
        ...
        self.label_protein_file_name = tk.Label(self.input_labelframe,
                                                text='File Type')
        self.label_protein_file_name.grid(
            row=nrow, column=0, columnspan=2, padx=5, pady=2)
        ...
        self.file_type_select.grid(
            row=nrow, column=2, columnspan=3, padx=5, pady=2)
        ...
        # delete all
        self.btn_input_delall = tk.Button(
            self.input_labelframe, text="Delete All", command=self.input_del_all)
        self.btn_input_delall.grid(
            row=nrow, column=10, sticky='we', padx=5, pady=2)

        # delete one
        self.btn_input_delone = tk.Button(
            self.input_labelframe, text="Delete One", command=self.input_del_one)
        self.btn_input_delone.grid(
            row=nrow, column=12, sticky='we', padx=5, pady=2)

        # number of files
        self.strvar_input_files = tk.StringVar(
            self.input_labelframe, value="0")
        self.label_input_files = tk.Label(
            self.input_labelframe, textvariable=self.strvar_input_files)
        self.label_input_files.grid(row=nrow, column=20, padx=5, pady=2)
        self.input_labelframe.pack(fill="both", expand="yes")

        nrow += 1

        self.listbox_yScroll = tk.Scrollbar(
            self.input_labelframe, orient=tk.VERTICAL)

        self.listbox_xScroll = tk.Scrollbar(
            self.input_labelframe, orient=tk.HORIZONTAL)
        ...
        self.lbox_input_files.grid(
            row=nrow, columnspan=20, sticky='we', padx=5, pady=2)
        ...
        # output directory
        self.output_labelframe = tk.LabelFrame(
            self.master, text="Output Directory")
        self.strvar_output_dir = tk.StringVar(self.master, value="")

        nrow = 0
        self.strvar_output_dir.set("")
        self.entry_output_dir = tk.Entry(
            self.output_labelframe, text=self.strvar_output_dir, width=100, bd=5)
        self.entry_output_dir.grid(row=nrow, column=0, padx=5, pady=2)

        self.btn_output_dir = tk.Button(
            self.output_labelframe, text="Select", command=self.ask_open_out_dir)
        self.btn_output_dir.grid(row=nrow, column=1, padx=5, pady=2)
        self.output_labelframe.pack(fill="both", expand="yes")

        nrow = 0
        self.param_labelframe = tk.LabelFrame(
            self.master, text="Parameter File")
        self.strvar_param_file = tk.StringVar(self.master, value="")
        self.strvar_param_file.set("")
        self.entry_param_file = tk.Entry(
            self.param_labelframe, text=self.strvar_param_file, width=100, bd=5)

        self.entry_param_file.grid(row=nrow, column=0, padx=5, pady=2)
        self.btn_param_file = tk.Button(
            self.param_labelframe, text="Select", command=self.ask_open_param_file)
    ...
    def use_sample1(self):
        ...
        self.lbox_input_files.insert(
            0, "Meta=" + sample_dir + os.sep + "CPTAC_OVA_meta_info.txt")
        self.lbox_input_files.insert(
            1, "Protein=" + sample_dir + os.sep + "CPTAC_OVA_protein_log2r_20210811.txt")
        self.lbox_input_files.insert(
            2, "Glycoform=" + sample_dir + os.sep + "CPTAC_OVA_glycoform_log2r_20210811.txt")

        # set up out_dir
        out_dir = sample_dir + os.sep + \
            "OmicsOne_Output_{0}".format(build_time_suffix())
    ...
    def use_sample2(self):
        ...
        sample_dir = r'{}'.format(str(root_dir)) + os.sep + "samples"
        self.lbox_input_files.insert(
            0, "Meta=" + sample_dir + os.sep + "CPTAC_LSCC_meta_info.xlsx")
        self.lbox_input_files.insert(
            1, "Protein=" + sample_dir + os.sep + "CPTAC_LSCC_proteome.xlsx")
    ...
    def start_omicsone_server(self):
        ...
        # start server
        self.omicsone_process = multiprocessing.Process(
            target=thread_server, name='omicsone', args=(url, port, paths))
        self.omicsone_process.start()

        # start client
        threading.Timer(3, lambda: webbrowser.open(
            'http://' + url + ":" + port)).start()
