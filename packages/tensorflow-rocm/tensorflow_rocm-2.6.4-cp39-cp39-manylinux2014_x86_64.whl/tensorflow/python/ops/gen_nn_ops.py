...
@_dispatch.add_dispatch_list
@tf_export('dropout')
def dropout(input, rate, noise_shape, seed, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "Dropout", name, input, rate, noise_shape, seed)
    ...
    try:
      return dropout_eager_fallback(
          input, rate, noise_shape, seed, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
    except (TypeError, ValueError):
      result = _dispatch.dispatch(
  ...
  try:
    _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "Dropout", input=input, rate=rate, noise_shape=noise_shape, seed=seed,
                   name=name)
  except (TypeError, ValueError):
    result = _dispatch.dispatch(
          dropout, (), dict(input=input, rate=rate, noise_shape=noise_shape,
...
def dropout_eager_fallback(input, rate, noise_shape, seed, name, ctx):
  ...
  _inputs_flat = [input, rate, noise_shape, seed]
