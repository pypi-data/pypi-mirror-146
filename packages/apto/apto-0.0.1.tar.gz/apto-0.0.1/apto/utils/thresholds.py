...
class ThresholdMode(str, enum.Enum):
    """Available threshold search modes."""
...
def get_binary_threshold(
    ...
    if np.max(metric_values) == 0.0:
        ...
        argmax = np.argmax(metric_values)
        best_metric_value, best_threshold = metric_values[argmax], thresholds[argmax]
...
def get_multiclass_thresholds(
    ...
    if len(labels.shape) != 2:
        labels_oh = np.zeros((labels.size, num_classes))
...
def get_binary_threshold_cv(
    ...
    if average == "macro":
        best_metric_value = np.mean(fold_metrics)
        best_threshold = np.mean(fold_thresholds)
    else:
        fold_metrics, fold_thresholds = np.array(fold_metrics), np.array(fold_thresholds)
        fold_weights = fold_metrics / fold_metrics.sum()
        best_metric_value = (fold_metrics * fold_weights).sum()
        best_threshold = (fold_thresholds * fold_weights).sum()
...
def find_best_multiclass_thresholds(
    scores: np.ndarray, labels: np.ndarray, objective: METRIC_FN
) -> Tuple[float, List[float]]:
    """Finds best thresholds for multiclass classification task.
...
def find_best_multilabel_thresholds(
    scores: np.ndarray, labels: np.ndarray, objective: METRIC_FN
) -> Tuple[float, List[float]]:
    """Finds best thresholds for multilabel classification task.
...
__all__ = [
    ...
    "find_best_multilabel_thresholds",
    "find_best_multiclass_thresholds",
]
