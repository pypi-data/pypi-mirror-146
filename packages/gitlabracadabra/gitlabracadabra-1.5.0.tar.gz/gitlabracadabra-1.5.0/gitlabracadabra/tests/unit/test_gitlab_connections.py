...
class TestGitlabConnections(TestCase):
    ...
    def fake_config(  # noqa: WPS210
        ...
        config = """
            ...
            private_token = n3k0T
        """
        readlink_error = OSError()
        readlink_error.errno = EINVAL
        with patch('os.readlink', side_effect=readlink_error):
            with patch('os.path.exists') as os_path_exists_mock:
                with patch_open({config_path: config}):
                    with patch.object(Gitlab, 'auth') as auth_mock:
                        ...
                        yield singleton
                        if self.gitlab_version(lt='1.6.0', ge='3.0.0'):
