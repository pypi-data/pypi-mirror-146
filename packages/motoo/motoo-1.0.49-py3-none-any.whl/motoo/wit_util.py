...
class WitUtilIndexer(pd.api.indexers.BaseIndexer):
    def __init__(self, df, days, window_size, nan_day_nan=False):
        self.non_nan_df = ~df.isnull()
        self.end_days = days
    ...
    def process_get_window(self):
        return_value = None
        column_data = self.non_nan_df.iloc[:,self.column_index]
        try:
            ...
            for i in range(column_data.shape[0] - 1, -1, -1):
                if self.nan_day_nan and not column_data.iloc[i]:
                    starts = np.insert(starts, 0, 0, axis=0)
                    ...
                    starts = np.insert(starts, 0, start, axis=0)
                    ends = np.insert(ends, 0, end, axis=0)
    ...
    def get_index(self, non_nan_series, index, num):
        direction = 1
        import traceback
        try:
            series = non_nan_series
            if num == 0: return index
            elif num < 0:
                series = non_nan_series.iloc[:index].iloc[::-1]
                num *= -1
                direction = -1
            elif num > 0:
                series = non_nan_series.iloc[index:]

            if series.isnull().all(): raise Exception
            if len(series) < abs(num): raise Exception
            
            frequency = series.mean()
            skip_index = num // frequency
            skip_index = int(skip_index)
            non_nan_sum = series.iloc[:skip_index].sum()

            if non_nan_sum == num:
                result = skip_index
            else:
                if skip_index > len(series):
                    raise Exception
                result = self.get_index(series, skip_index, num - non_nan_sum)
...
class SpotIndexer(WitUtilIndexer):
    def __init__(self, df, days, nan_day_nan=False):
...
class WitData:
    ...
    def base_calc(self, type_node, begin_days, end_days=0, nan_day_nan=False):
        ...
        if type_node.endswith('POINT'):
            ...
            df = df.rolling(RangeIndexer(df=df, days=end_days, window_size=window_size, nan_day_nan=nan_day_nan), min_periods=window_size).std()
