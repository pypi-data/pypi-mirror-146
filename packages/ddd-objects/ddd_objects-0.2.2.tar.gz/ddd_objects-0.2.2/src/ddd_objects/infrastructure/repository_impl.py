...
class RepositoryImpl(Repository):
    ...
    def clean_cache(self):
        if self.clean_prob<random():
            for key in list(self.cache.keys()):
                if self.cache[key] is None or self.cache[key].is_expired:
                    del self.cache[key]
    ...
    def find_entity_helper(
        ...
        **kwargs
        ...
        if x is None or any(m is None for m in x) if isinstance(x, list) else False or any(m.is_expired for m in x) if isinstance(x, list) else x.is_expired or force_update:
            x = find_method(*args, **kwargs)
