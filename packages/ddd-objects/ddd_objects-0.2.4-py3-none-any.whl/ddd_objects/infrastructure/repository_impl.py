...
class RepositoryImpl(Repository):
    def __init__(self, clean_prob=0.1) -> None:
        self.cache = {}
    ...
    def attach(self, key: str, obj: BaseDO) -> None:
        ...
        if not isinstance(obj, list) \
            ...
            # raise ValueError('Saved object is not list or subclass of ExpiredEntity or ExpiredDO')
            return
        if isinstance(obj, list) \
            and any(not hasattr(m, '_expired_time') for m in obj) \
            and any(not hasattr(m, 'expired_time') for m in obj):
            # raise ValueError("Saved obect has element that doesn't belong to subclass of "
            # "ExpiredEntity or ExpiredDO")
            return
        self.cache[key] = obj
    ...
    def find_entity_helper(
        ...
        **kwargs
        ...
        x = self.find_from_cache(key)
        if verbose:
            logger.info(f'find_entity_helper: cache value is {x}')
        if x is None or x.is_expired or any(m.is_expired for m in x) or force_update:
            x = find_method(*args, **kwargs)
            ...
            if verbose:
                logger.info(f'find_entity_helper: new value is {x}')
            self.attach(key, x)
        return x
