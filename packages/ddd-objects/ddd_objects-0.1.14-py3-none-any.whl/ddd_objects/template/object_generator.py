...
class DOGenerator:
    ...
    def __gen_do_class_block(class_name, items):
        strip1 = [
            ...
            if item.value_default_value is not None 
...
class EntityGenerator:
    ...
    def __gen_entity_class_block(class_name, items):
        ...
        for item in items:
            item: Item
            default = '' if item.default_value is None else f' = {item.entity_default_value}'
    ...
    def __gen_converter_class_block(class_name, items):
        ...
        for item in items:
            ...
            if 'List' in item.item_type:
                ...
                strip2.append(' '*12+f'{item.name} = [m.get_value() for m in x.{item.name}]')
            else:
                strip1.append(' '*12+f'{item.name} = {item.value_type}(do.{item.name})')
                strip2.append(' '*12+f'{item.name} = x.{item.name}.get_value()')
