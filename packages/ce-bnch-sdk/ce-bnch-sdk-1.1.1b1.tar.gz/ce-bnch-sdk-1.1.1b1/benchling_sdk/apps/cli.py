...
from benchling_sdk.apps.benchling_store import BenchlingStore
...
from benchling_sdk.apps.dependencies.benchling_dependency_resolver import BenchlingDependencyResolver
from benchling_sdk.apps.dependencies.dependency_linker import DependencyLinker
...
def install_manifest(hostname: str, manifest_file_path: str = MANIFEST_OPTION, reset_api_key: bool = False):
    ...
    typer.secho(
        f"Successfully installed Benchling App {manifest.info.id} from {manifest_file_path}.", fg="green"
    )
...
def link(
    ...
    """Link your App's dependencies to resources in your tenant."""
    api_key = api_key_from_prompt_or_keyring(hostname, reset_api_key)
    client = Benchling(hostname, auth_method=ApiKeyAuth(api_key))
