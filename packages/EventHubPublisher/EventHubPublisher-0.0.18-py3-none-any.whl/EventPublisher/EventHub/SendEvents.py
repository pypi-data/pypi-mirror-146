...
def run_send_events(event_type, env_name, client_code, events_json):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(send_events(event_type, env_name, client_code, events_json))
    loop.close()
    return result
...
async def send_events_using_command_line(event_type):
    ...
    return await Event.run(event_type, client_code, events)

async def send_events(event_type, env_name, client_code, events_json):
