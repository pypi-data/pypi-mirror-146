...
_EXPERIMENT_NAME_REGEX = re.compile(r"^[a-zA-Z0-9-]*$")
_LOG_TYPE_REGEX = re.compile(r"^[a-zA-Z0-9-]*$")
...
def _validate_run_name(name):
    ...
    if name is None or not _VALID_PARAM_AND_METRIC_NAMES.match(name):
        raise MlflowException(
            "Invalid run  name: '%s'. %s" % (name, _BAD_CHARACTERS_MESSAGE),
            INVALID_PARAMETER_VALUE,
        )
...
def _validate_run_log_input(key, timestamp, step, log_type, value):
    # reusing the validation defined for metric
    _validate_metric(key=key, value=0, timestamp=timestamp, step=step)
    _validate_length_limit("RunLog log type", MAX_ENTITY_KEY_LENGTH, log_type)
    if not log_type or not _LOG_TYPE_REGEX.match(log_type):
        raise MlflowException(
    ...
    try:
        ...
        raise MlflowException(f"{key} key does not contain a valid json", INVALID_PARAMETER_VALUE)
...
def _validate_experiment_name(experiment_name):
    ...
    if not _EXPERIMENT_NAME_REGEX.match(experiment_name):
        raise MlflowException(
            f"Invalid Experiment name {experiment_name} . Name may only contain alphanumerics or dashes (-)",
            error_code=INVALID_PARAMETER_VALUE,
        )
