...
if __name__ == '__main__':
    ...
    scenarios = {'Stage': ('',),  # only ONE Test Stage with empty label
                 'HA': ('A', 'B'),  # Two Hearing-aid programs
                 'CoSS': [f'C{i}' for i in range(1, 8)],  # Seven CoSS categories
                 }  # Categorical variables, same for all (Sub-)Populations
    # NOTE: First scenario dimension is always Stage, even if only ONE stage category
    # User may set arbitrary stage_key label
    # Dimension 'Stage' may be omitted, if only one stage category
    ...
    emf = EmaFrame(scenarios=scenarios,
                   stage_key='Stage',
    ...
    # NOTE: scenarios and attribute grades always analyzed as Categorical variables,
    # even if the categories are defined as numbers here.
    # The rank order of attribute_grades is defined by the order as listed here,
    ...
    ds = EmaDataSet.load(emf, data_path,
                         ...
                         # subject='sheet',   # subject ID defined in xlsx sheet title, OR
                         # subject='Participant',  # subject IDs defined in column 'Participant'
                         # rename_cols={'Hearing Aid: 'HA'},  # dict with (file, new) column names
                         # dtype={'CoSS': 'string'}  # just in case pandas interpreted it as numeric
                         # header=0,  # first row shows column headers
    ...
    nap = ds.nap_table(sc='HA', nap_cat=['A', 'B'], p=0.95)  # aggregated across other scenario dimensions
