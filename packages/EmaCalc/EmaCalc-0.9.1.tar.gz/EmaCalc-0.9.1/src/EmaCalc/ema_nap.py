...
nap_ci_high --- approximate upper limit of confidence interval
    Can be used for eithe one-tailed or two-tailed confidence interval.
...
def nap_pandas(df, col, nap_cat=None,
    ...
    x_n drawn from random variable X, and y_n drawn from Y.
    :param df: a DataFrame containing input data
    ...
    def calc_nap(group_name, group_df):
        ...
        if len(group_cols) == 1:
            group_name = [group_name]  # because pd.group_by made it scalar
        res = {g_col: g_cat
        ...
        for a in grade_cols:
            ...
            if n_x == 0 or n_y == 0:
                ...
                res.update({(a, 'CI_low'): nap_ci_low(nap_point, n_x, n_y, z_tail),
                            (a, 'NAP'): nap_point,
                            (a, 'CI_high'): nap_ci_high(nap_point, n_x, n_y, z_tail)})
        return res
        # ---------------------------------------------------------------------
    if nap_cat is None:
