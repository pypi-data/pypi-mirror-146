...
class DataBeaver:
    ...
    def __init__(self, config_file=None, config_format=None):
        ...
        extension = config_file.split('.')[-1].lower()
        if self._config_format is None and extension is 'ini':
            self._config_format = ConfigFormats.INI
        elif self._config_format is None and extension is 'toml':
            self._config_format = ConfigFormats.TOML
        elif self._config_format is None and extension is 'yaml':
            self._config_format = ConfigFormats.YAML
        elif self._config_format is None and extension is 'json':
        ...
        if self._config_file and self._config_format is ConfigFormats.TOML:
    ...
    def build(self):
        ...
        if not self.instantiated:
            msg = f"No configuration provided."
            raise NotInstantiatedError(message=msg)

        # Check the project file exists and
        # Read YAML file
        with open(self.args.project_file, 'r') as stream:
