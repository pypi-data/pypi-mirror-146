...
class Ls:
    ...
    def type_color(self, i: os.DirEntry, colors: bool) -> str:
        ...
        if i.is_dir():
            if os.path.abspath(self.path).endswith('\\'): # workaround
                if os.path.realpath(i.path) != os.path.abspath(self.path) + i.name: 
            ...
            elif os.path.realpath(i.path) != os.path.abspath(self.path) + f'\\{i.name}': 
                return Fore.CYAN + i.name + Fore.LIGHTBLACK_EX + ' --> ' + os.path.realpath(i.path) + Style.RESET_ALL
            return Fore.LIGHTBLUE_EX + i.name + Style.RESET_ALL
