...
class CKLink(object):
    ...
    def find_library(self):
        if sys.platform == 'win32':
            ...
            ctypes.CDLL("links/CK-Link/libCklink.so", mode=ctypes.RTLD_GLOBAL)
            ctypes.CDLL("libScripts.so", mode=ctypes.RTLD_GLOBAL)
            ctypes.CDLL("libXml.so", mode=ctypes.RTLD_GLOBAL)
