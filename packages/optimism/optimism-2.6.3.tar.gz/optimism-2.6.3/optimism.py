...
- Version 2.6.3 introduces the `checkFileContainsLines` method, and also
  ...
  tracebacks for come checkers (need to standardize that!).
"""
...
default value of `'all'`, ALL checks will be skipped once one fails,
until `clearFailure` is called. If set to `'case'`, subsequent checks for
the same test case will be skipped when one fails. If set to `'manager'`,
then all checks for any case from a case manager will be skipped when any
check for any case derived from that manager fails. Any other value will
disable the skipping of checks based on failures.
"""

CHECK_FAILED = False
"""
Remembers whether we've failed a check yet or not. If True and
`SKIP_ON_FAILURE` is set to `'all'`, all checks will be skipped. Use
`clearFailure` to reset this and resume checking without changing
`SKIP_ON_FAILURE` if you need to.
"""
...
class TestCase:
    ...
    def checkReturnValue(self, expectedValue):
        ...
        if results["error"] is not None:
            ...
            long = len(short_result) > 12 or len(short_expected) > 12
            ...
            and short_expected == full_expected
                ...
                extra_msg = None
                if firstDiff is not None and long:
                    base_msg += (
    ...
    def checkPrintedLines(self, *expectedLines):
        ...
        if results["error"] is not None:
            ...
            if output == expected:
                ...
                noBlankDiff = findFirstDifference(outNoBlanks, expNoBlanks)
                if noBlankDiff is None:
            ...
            lengthy = len(short) > 12 or len(short_exp) > 12
    ...
    def checkFileContainsLines(self, filename, *lines):
        """
        ...
        if results["error"] is not None:
            ...
            try:
                ...
                msg = self._create_failure_message(
                    tag,
                    "Expected file '{filename}' cannot be read.",
                    None
                )
                print_message(msg, color=msg_color("failed"))
                self._register_outcome(False, tag, msg)
            ...
            if fileContents == expected:
                ...
                expNoBlanks = [line for line in lines if line.strip()]
                noBlankDiff = findFirstDifference(outNoBlanks, expNoBlanks)
                if noBlankDiff is None:
                    equivalence = "equivalent (EXCEPT blank lines) to"
                else:
...
def expect(expr, value):
    ...
    lengthy = len(short_result) > 12 or len(short_expected) > 12
...
{indent(short_expected, 4)}"""
    if firstDiff is not None and lengthy:
        message += f"\n  First difference was:\n{indent(firstDiff, 4)}"
    ...
    if simple:
        ...
        and isinstance(ref, (int, float, complex))
            ...
            if '\n' in val or '\n' in ref:
                ...
                valLines = val.splitlines()
                refLines = ref.splitlines()
