...
twin_ledger = client.ledgers.add_twin_ledger_entry(twin_uuid, entries)
...
updated_ledger = client.ledgers.update_twin_ledger_entry(twin_uuid, to_update_ledger)
...
other_ledger = client.ledgers.add_twin_ledger_entry(other_twin_uuid, other_twin_ledger_entries)
...
response = client.ledgers.add_twin_ledger_entry(twin_uuid, twin_2_ref_entry)
...
response = client.ledgers.get_twin_ledger_entry(twin_uuid)
print(response['entries']['referring_key']['ref']['status'] == 'ok')

# delete entry
client.ledgers.delete_twin_ledger_entry(twin_uuid, entries=['referring_key'])
response = client.ledgers.get_twin_ledger_entry(twin_uuid)
print(response['entries'].get('referring_key') is None)

# delete all entries in a ledger:
client.ledgers.delete_twin_ledger_entry(twin_uuid)
response = client.ledgers.get_twin_ledger_entry(twin_uuid)
