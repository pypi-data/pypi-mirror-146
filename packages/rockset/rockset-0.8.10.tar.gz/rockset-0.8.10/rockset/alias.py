...
from rockset.swagger_client.models import CreateAliasRequest, UpdateAliasRequest
...
class AliasClient(object):
    ...
    def create(
        self, name, description=None, workspace="commons", collections=[], **kwargs
    ):
...
class Alias(rockset.swagger_client.models.Alias):
    ...
    def resolve(self):
        ...
        return [
            Collection.retrieve(
                split["name"], workspace=split["workspace"], client=self._client
            )
        ]

    def update(self, description=None, collections=[]):
        request = UpdateAliasRequest(description=description, collections=collections)
