...
class Creon:
    ...
    def get_stockbid(self, code, mode='C', hour=None):
        # mode: 'C' - 체결, 'H' - 호가
        if not code.startswith('A'):
            code = 'A' + code
        self.obj_Dscbo1_StockBid.SetInputValue(0, code)
        self.obj_Dscbo1_StockBid.SetInputValue(2, 80)
        ...
        result = self.request(self.obj_Dscbo1_StockBid, data_fields=data_fields, header_fields=header_fields, cntidx=2, n=80)
        return result
