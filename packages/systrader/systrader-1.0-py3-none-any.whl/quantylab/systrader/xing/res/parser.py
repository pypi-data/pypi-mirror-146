...
import pandas as pd
from quantylab.systrader import xing
from quantylab.systrader.xing import settings
...
def parse(res_name):
    with open(os.path.join(settings.XING_RES_PATH, res_name), 'r') as res:
        return _parse(res)
...
def export_all():
    ...
    # os.mkdir("./blocks")
    for res_name in os.listdir(settings.XING_RES_PATH):
    ...
    with open(os.path.join(settings.XING_RES_BASE, 'blocks.pickle'), 'wb') as f:
        pickle.dump(blocks, f)
        f.close()
    with open(os.path.join(settings.XING_RES_BASE, 'occurs.pickle'), 'wb') as f:
...
def import_all():
    blocks = None
    occurs = None
    if os.path.isfile(os.path.join(settings.XING_RES_BASE, 'blocks.pickle')):
        with open(os.path.join(settings.XING_RES_BASE, 'blocks.pickle'), 'rb') as f:
            blocks = pickle.loads(f.read())
            f.close()
    if os.path.isfile(os.path.join(settings.XING_RES_BASE, 'occurs.pickle')):
        with open(os.path.join(settings.XING_RES_BASE, 'occurs.pickle'), 'rb') as f:
