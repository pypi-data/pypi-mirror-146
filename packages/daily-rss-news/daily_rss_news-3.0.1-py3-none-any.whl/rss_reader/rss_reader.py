...
from converters import displayer
from utils import log, caching
...
class RssReader:
    ...
    def get_news(self):
        ...
        self.parse_url()
        if self.parser.cli_args.limit and self.parser.cli_args.limit <= 0:
            entries = []
        else:
    ...
    def display(self):
        ...
        if self.parser.cli_args.json and not self.parser.cli_args.clear_cache:
            self.format.display_news_json(self.feed)
        elif not self.parser.cli_args.clear_cache:
            self.format.display_news(self.feed)
