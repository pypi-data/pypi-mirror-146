...
def get_datetime(time_zone: str=None) -> datetime:
    return datetime.now() if time_zone is None else datetime.now(pytz.timezone(time_zone))

def get_datetime_str(time_zone: str=None) -> str:
    return datetime.now() if time_zone is None else datetime.now(pytz.timezone(time_zone)).strftime(DATETIME_FMT_FULL)
