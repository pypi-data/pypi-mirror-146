...
class Impac:
    ...
    def _load_arrays(self, img, standardize):
        """
    ...
    def _array2arrays(self, tseries):
        """
        Reshape an array of images into proper shape.
        """
        if len(self.arrays_shape_) == 3:
            arrays = np.reshape(tseries,
                [tseries.shape[0], self.arrays_shape_[1], self.arrays_shape_[2]]
            )
        else:
            arrays = np.reshape(tseries,
                [
                    tseries.shape[0],
                    self.arrays_shape_[1],
                    self.arrays_shape_[2],
                    self.arrays_shape_[3],
                ]
            )
        return arrays

    def _check_size_img(self, arrays):
    ...
    def inverse_transform(self, weights):
        ...
        return self._array2arrays(self.embedding.inverse_transform(weights))
