...
def main():
    try:
        ...
        if ext == '.wav':
            ...
            fft = ForwardTransform(N=mixture.frame_size * 4, R=mixture.frame_size)
            fg = FeatureGenerator(frame_size=mixture.frame_size,
                                  feature_mode=model.feature,
                                  num_classes=1,
                                  truth_mutex=model.mutex)

            transform_frames = len(audio) // mixture.frame_size
            feature_frames = len(audio) // (mixture.frame_size * fg.decimation * fg.step)

            feature = np.empty((feature_frames, fg.stride, fg.num_bands))
            mock_truth = np.empty(fg.num_classes)

            feature_frame = 0
            for transform_frame in range(transform_frames):
                indices = slice(transform_frame * mixture.frame_size, (transform_frame + 1) * mixture.frame_size)
