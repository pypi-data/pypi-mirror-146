...
def __remove_sensitive_data(cloud_participant_response: CloudParticipantResponse) -> dict:
    response: dict = cloud_participant_response.dict()
    if response['auth']['secret_token']:
        response['auth']['secret_token'] = "***"

    if response['role'] == 'key':
        response['active_participant'] = response['active_participant'][-8:]

    if response['auth']['access_token']:
        response['auth']['access_token'] = response['auth']['access_token'][-8:]
