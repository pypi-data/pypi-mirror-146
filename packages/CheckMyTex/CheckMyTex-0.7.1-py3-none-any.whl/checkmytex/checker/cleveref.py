...
class Cleveref(Checker):
    def check(self, document: LatexDocument) -> typing.Iterable[Problem]:
        ...
        for match in re.finditer(r"\\ref\{[^\}]+\}", document.get_source()):
            ...
            message = "Prefer using cleveref's \\cref or \\Cref instead of native \\ref."
            rule = 'CREF'
            yield Problem(origin, context=context, message=message,
                          long_id=rule + "|" + context, tool="CleverrefCheck", rule=rule,
        ...
        for match in re.finditer(expr, document.get_source()):
            origin = document.get_origin_of_source(match.start("cref"), match.end("cref"))
            context = document.get_source_context(origin)
