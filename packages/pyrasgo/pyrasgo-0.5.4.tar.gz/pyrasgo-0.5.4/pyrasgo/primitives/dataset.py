...
from pyrasgo.utils import dbt, naming, polling
...
# ---------------------------------
    ...
    def _get_cte_for_dataset(self) -> str:
        """
        Validate that the dataeset is published, and then get a CTE that would build it
        """
    ...
    def _assert_is_published(self) -> None:
        ...
        if self.status != DS_PUBLISHED_STATUS:
            raise AttributeError(
                f"Dataset must be published in order to perform this action. Please publish your dataset to continue.")
