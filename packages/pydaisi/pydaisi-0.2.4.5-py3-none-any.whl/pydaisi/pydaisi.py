...
class DaisiExecution:
    ...
    @property
    def value(self):
        return asyncio.run(self.fetch_result())

    async def fetch_result(self, timeout: int = 180):  # Set to 180 by default for testing, otherwise logs timeout
        timer = 0
        if self.id is None:
            await self._start_execution()
        if not self.value_fetched:
            ...
            if self._result["outputs"][0].get("label") == "ERROR":
                ...
                r = self.daisi.session.get(
                    f"{self.daisi.base_url}/outputs/html/{error_id}"
                )
...
class Daisi:
    ...
    def __init__(
        ...
        if access_token:
            ...
            self.asession.headers.update({"Authorization": f"token {access_token}"})
    ...
    def _run(
        self, _func="compute", wait_for_completion=True, args: tuple = (), kwargs=None
    ):
        kwargs = kwargs if kwargs is not None else {}
        param_names = [p["id"] for p in self.endpoints[_func]]
        kwargs.update(zip(param_names, args))
        ...
        if wait_for_completion:
            asyncio.run(daisi_execution.fetch_result())
    ...
    def run_parallel(*executions: DaisiExecution):
        return asyncio.run(Daisi._parallel_run(*executions))

    @staticmethod
    async def _parallel_run(*executions: DaisiExecution):
        return await asyncio.gather(*[execution.fetch_result() for execution in executions])
