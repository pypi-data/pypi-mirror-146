...
class VehicleDeduplicationData:
    ...
    def update_drop_list(self, message: PresenceBaseMessage, vehicle: Vehicle):
        drop = vehicle.id not in self.dont_drop_list and datetime.utcnow() < self.timeout and self.plate and \
               (not vehicle.has_media() or self.plate == self.lp_converter.clean(vehicle.lpr.id or "")) and \
               message.present_area_name != self.present_area_name
        if drop:
            self.drop_list.add(vehicle.id)
        elif vehicle.has_media() and self.plate != self.lp_converter.clean(vehicle.lpr.id or ""):
