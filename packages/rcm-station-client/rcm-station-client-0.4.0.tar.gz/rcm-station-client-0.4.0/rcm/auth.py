...
def _read_token():
    ...
    try:
        with open(settings.token_file, constants.MODE_READ, encoding=constants.UTF_8) as file:
...
def _write_token(access_token: str, expires_in: float):
    ...
    """
    try:
        with open(settings.token_file, constants.MODE_WRITE, encoding=constants.UTF_8) as file:
