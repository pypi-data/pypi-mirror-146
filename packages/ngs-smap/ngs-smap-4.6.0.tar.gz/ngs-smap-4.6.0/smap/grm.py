...
class LocusInformationCoefficient(SimilairityCoefficient):
    ...
    def calculate(self, sample1: pd.Series, sample2: pd.Series) -> pd.Index:
        ...
        unique_sample1 = sample1.smap.unique_alleles(sample2, include_non_shared_loci=True)
        unique_sample2 = sample2.smap.unique_alleles(sample1, include_non_shared_loci=True)
        if self._locus_information_criterion == 'Shared':
            ...
            unique_in_either = unique_sample1.union(unique_sample2)
            if self._partially_informative_loci:
                return unique_in_either.unique(level=LOCUS_COLUMN_NAME)
            unique_in_either.droplevel(level=HAPLOTYPES_COLUMN_NAME)\
                            .difference(shared_alleles.droplevel(level=HAPLOTYPES_COLUMN_NAME))\
                            .unique(level=LOCUS_COLUMN_NAME)
...
class SmapAccessor:
    ...
    def unique_alleles(self,
        ...
        ser1, ser2 = self._ser.replace(0, pd.NA), other.replace(0, pd.NA)
        ser1_with_counts, ser2_with_counts = ser1.dropna(), ser2.dropna()
...
def create_information_matrix(haplotypes: FrequencyMatrix | CountMatrix,
    ...
    counts_per_locus = pd.DataFrame(0, index=pd.Index(loci, name='Locus'),
                                    columns=['count', 'informative'])
...
def pool_samples(haplotypes: CountMatrix | FrequencyMatrix,
                 name_mapping: dict[str, str]) -> CountMatrix | FrequencyMatrix:
    logger.info('Pooling samples.')
    to_be_pooled = defaultdict(list)
    for original_sample, new_sample in name_mapping:
        to_be_pooled[new_sample].append([original_sample])
    to_be_pooled = {new_sample: samples_to_pool for new_sample, samples_to_pool
                    in to_be_pooled if len(samples_to_pool) > 2}
    return haplotypes
...
def create_line_curves(haplotypes: CountMatrix | FrequencyMatrix,
    ...
    for sample1_name, other_samples in information_per_sample_pair.items():
        xvalues = [information['number_of_shared_loci']
                   for samples_information in information_per_sample_pair.values()
                   for information in samples_information.values()]
        ...
        filename = f'{similarity_coefficient}_{method.lower().replace(" ", "_")}_{sample1_name}.png'
        create_line_plots(filename,
                          ...
                          ymin=0,
                          ymax=max([max(y) for y in y_values]),
...
def main(args=None) -> None:
    ...
    if args.names:
        ...
        with open(args.names, 'r') as open_names:
            ...
            haplotypes = pool_samples(haplotypes, names_mapping)
            haplotypes.rename_samples(names_mapping)

    line_curve_samples = {sample1: sample2 for sample1, sample2 in
                          combinations_with_replacement(haplotypes.sample_names, 2)}
    if args.line_curve_samples:
        ...
        with args.line_curve_samples.open('r') as open_curves_file:
            ...
            for sample1, sample2 in line_curve_list:
                ...
                line_curve_samples[sample1] = sample2
    ...
    if args.plot >= PLOT_SUMMARY:
        plot_similarity_matrix(args.similarity_coefficient, similairy_matrix, args.plot_type)
    ...
    if args.output_locus_information:
        ...
        informative_loci, number_of_loci, counts_per_locus = create_information_matrix(
            haplotypes, similarity_coefficient=coefficient)
        ...
        number_or_proportion = "Number" if not args.informative_loci_as_proportion \
                               else "Proportion"
        ...
        if args.output_locus_information in ('List', 'All'):
            ...
            lic_representation = "SamplesWithUniqueHaplotypes" \
            ...
            counts_per_locus.columns = ['NumberOfComparisons',
                                        column2_name,
                                        (f'ProportionOfComparisonsWith{lic_representation}Locus')]
            file_name = f'{partial_representation}{args.locus_information_criterion}Loci.txt'
            counts_per_locus.to_csv(file_name, sep="\t")

    if args.plot >= PLOT_ALL:
        create_line_curves(haplotypes, line_curve_samples,
                           args.similarity_coefficient, args.processes)
