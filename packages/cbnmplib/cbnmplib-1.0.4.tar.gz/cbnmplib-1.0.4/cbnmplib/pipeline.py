...
from communicate import _read
from communicate import _write
from communicate import NativeMessage
...
class Pipeline:
    ...
    def port(self) -> Tuple[str,int]:
        ...
        '''
        return self._external.getsockname()
    ...
    def close(self) -> SocketType:
        ...
        # truncate external interface port
        with open(self._lppath, mode='w', encoding=self._enc) as fd:
            fd.truncate(0)
            fd.flush()
    ...
    def enqueue(self) -> bytes:
        ...
        with open(cfname, mode='w', encoding=self._enc) as fd:
