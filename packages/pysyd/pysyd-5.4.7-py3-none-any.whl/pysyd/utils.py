...
class Parameters(Constants):
    ...
    def adjust_parameters(self, star, adjust=True, defaults=None,):
        ...
        if self.params[star]['adjust'] and self.params[star]['numax'] is not None:
            if (self.params[star]['numax'] is not None and self.params[star]['numax'] < 500.) or (self.params[star]['defaults'] is not None and self.params[star]['defaults'] == 'low'):
            ...
            elif (self.params[star]['numax'] is not None and self.params[star]['numax'] >= 500.) or (self.params[star]['defaults'] is not None and self.params[star]['defaults'] == 'high'):                self.params[star]['boxes'] = np.logspace(np.log10(50.), np.log10(500.), self.params['n_trials'])
