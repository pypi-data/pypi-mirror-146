...
from piscat.GUI.Projects.tab_anomaly_localization import Anomaly_Localization_GUI
from piscat.GUI.Projects.tab_handcrafted_anomaly import HandCraftedFeatureMatrix_GUI
...
class AnomalyTabs(QtWidgets.QMainWindow):

    new_update_df_PSFs = QtCore.Signal(object)
    new_update_bg_video = QtCore.Signal(object)
    new_update_anomaly_video = QtCore.Signal(object)
    ...
    def __init__(self, video_in, batch_size, object_update_progressBar, parent=None):
        super(AnomalyTabs, self).__init__(parent)
    ...
    def initUI(self):
        ...
        self.all_tabs = {"Bg_correction": BgCorrection_GUI(input_video=self.input_video, object_update_progressBar=self.object_update_progressBar),
                         "Anomaly_Handcrafted": HandCraftedFeatureMatrix_GUI(input_video_raw=self.input_video, batch_size=self.batch_size),
                         "PSFs_Localization": Anomaly_Localization_GUI(),
        ...
        self.all_tabs["Anomaly_Handcrafted"].output_Tab_anomaly.connect(self.Update_tab_anomaly)
        self.all_tabs["Anomaly_Handcrafted"].output_setting_Tab_anomaly.connect(partial(self.history_update, key='Handcrafted_anomaly_setting'))
        self.all_tabs["Anomaly_Handcrafted"].update_tab_index.connect(self.update_tab)

        self.new_update_bg_video.connect(self.all_tabs["Anomaly_Handcrafted"].update_in_data)
        ...
        self.new_update_anomaly_video.connect(self.all_tabs["PSFs_Localization"].update_in_data_anomaly)
        ...
        self.main_tabWidget.addTab(self.all_tabs["Anomaly_Handcrafted"], "Handcrafted feature matrix")
    ...
    def Update_batchSize(self, batchSize):
        ...
        self.all_tabs["Anomaly_Handcrafted"].update_batchSize(batchSize=batchSize)
    ...
    def Update_tab_bgCorrection(self, data):
        if data[0] is not None:
            ...
            self.visualization_.bg_correction_update(in_video=self.bgCorrectedVideo, label=self.type_bgCorrection,
    ...
    @Slot()
    def Update_tab_anomaly(self, data):
        if data is not None:
            self.anomaly_result = data
            self.new_update_anomaly_video.emit(self.anomaly_result)

            self.visualization_anomaly = Visulization_localization()
            self.visualization_anomaly.bg_correction_update(in_video=self.anomaly_result, label='Anomaly_Handcrafted', object=self.all_tabs["Anomaly_Handcrafted"])
        else:
            self.msg_box2 = QtWidgets.QMessageBox()
            self.msg_box2.setWindowTitle("Warning!")
            self.msg_box2.setText("Anomaly video does not find!")
            self.msg_box2.exec_()
