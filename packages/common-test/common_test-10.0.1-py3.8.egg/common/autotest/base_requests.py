...
from common.autotest.handle_allure import allure_title, allure_feature, allure_step, allure_severity, allure_story, \
    allure_link
...
class BaseRequest(object):
    ...
    def send_request(cls, case: list, host: str = 'host', datatype: str='json') -> object:
        ...
        if isinstance(case, dict):
            case_number = DataProcess.get_key_dic(case, "用例编号")
            case_feature = DataProcess.get_key_dic(case, "所属模块")
            case_title = DataProcess.get_key_dic(case, "用例标题")
            jira_name = DataProcess.get_key_dic(case, Constant.CASE_STORY)
            jira_link = DataProcess.get_key_dic(case, Constant.CASE_LINK)
            case_severity = DataProcess.get_key_dic(case, "优先级")
            path = DataProcess.get_key_dic(case, "接口地址")
            status = DataProcess.get_key_dic(case, "是否执行")
            if DataProcess.get_key_dic(case, "数据类型") != None:
                datatype = DataProcess.get_key_dic(case, "数据类型")
            token = DataProcess.get_key_dic(case, "token操作")
            method = DataProcess.get_key_dic(case, "请求方式")
            parametric_key = DataProcess.get_key_dic(case, "入参关键字")
            file_obj = DataProcess.get_key_dic(case, "上传文件")
            data =  DataProcess.get_key_dic(case, "请求数据")
            sql = DataProcess.get_key_dic(case, "后置sql")
            expect = DataProcess.get_key_dic(case, "预期结果")
        ...
        # allure报告 需求名称
        allure_story(jira_name)
        # allure报告 需求链接
        allure_link(jira_link)
