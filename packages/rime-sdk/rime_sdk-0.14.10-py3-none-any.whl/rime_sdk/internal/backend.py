...
from rime_sdk.protos.continuous.continuous_pb2_grpc import CTServiceStub
...
class RIMEBackend:
    ...
    def __init__(self, domain: str, api_key: str = "", channel_timeout: float = 5.0):
        ...
        self._grpc_web_addr = self._get_continuous_addr(base_domain)
    ...
    def _get_continuous_addr(self, domain: str) -> str:
        """Construct an address to the continuous service from `domain`.
    ...
    def get_continuous_testing_stub(self) -> RIMEConnection[CTServiceStub]:
        """Return a continuous testing client."""
        return RIMEConnection[CTServiceStub](
            CTServiceStub,
            self._grpc_web_addr,
            self._api_key,
            channel_timeout=self._channel_timeout,
