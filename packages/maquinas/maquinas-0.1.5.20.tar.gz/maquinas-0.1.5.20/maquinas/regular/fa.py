...
class FiniteAutomaton:
    ...
    def __init__(
        self, Q=[], sigma=[], q_0=None, A=[], delta=[], force=False, epsilon=False
    ):
    ...
    def acceptor(self, q):
        ...
        :returns: None"""
        if isinstance(q, set):
            if len(q.intersection(self.A)) > 0:
                return True
        else:
            if q in self.A:
                return True
    ...
    def add_error_state(self, e_label="q_E"):
        ...
        for nq, q in enumerate(self.Q):
            for na, a in enumerate(self.sigma):
                if not nq in self.ttable:
                    empty_cell = True
                    break
                elif not na in self.ttable[nq] or (isinstance(self.ttable[nq][na],set) and len(self.ttable[nq][na]) == 0):
                    empty_cell = True
                    break
            if empty_cell:
                break
        if empty_cell:
            ...
            for nq, q in enumerate(self.Q):
                for na, a in enumerate(self.sigma):
                    if not nq in self.ttable or \
                            not na in self.ttable[nq] or \
                            (isinstance(self.ttable[nq][na],set) and len(self.ttable[nq][na]) == 0):
                        self.add_transition(q,a,e_label, force=True)
    ...
    def save_file(self, filename="machine.txt", order_Q=None, order_sigma=None):
        ...
        with open(filename, "w") as f:
            ...
            print("|", end="", file=f)
            h = "|".join(
                "{: ^{width}}".format(a, width=largest_cell) for a in self.sigma
            )
            print(h, end="", file=f)
            print("|", file=f)
            for q_i in order_Q:
                ...
                if q_i == self.q_0:
                    print(
                        " {: <{width}}|".format(q_l, width=largest_q + 1),
                        end="",
                        file=f,
                    )
                else:
