...
class ADAM:
  ...
  def fit(self, x, y, layers, loss, metrics, metrics_per_batch):
    ...
    for i in reversed(range(len(self.layers_))):
      ...
      self.mW[i] = self.mW[i] / (1- np.power(self.b1_, self.step))
      self.vW[i] = self.vW[i] / (1- np.power(self.b2_, self.step))
      self.mB[i] = self.mB[i] / (1- np.power(self.b1_, self.step))
      self.vB[i] = self.vB[i] / (1- np.power(self.b2_, self.step))
      
      dW = -self.learning_rate_ * self.mW[i] / (np.sqrt(self.vW[i]) + self.epsilon_)
      dB = -self.learning_rate_ * self.mB[i] / (np.sqrt(self.vB[i]) + self.epsilon_)
