...
class Repository:
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.engine = create_engine(f'mysql+pymysql://{username}:{password}@{host}:{port}/{database}', echo=False) # , isolation_level='READ UNCOMMITTED')
        Session = sessionmaker(bind=self.engine)
        self.session = scoped_session(Session)

        def auto_add(session):
            Session.add(session)

        def auto_detach(session):
            Session.detach(session)

        # event.listen(Session, 'init', auto_add)
        event.listen(Session, 'after_commit', auto_detach)
        event.listen(Session, 'append', auto_add)
