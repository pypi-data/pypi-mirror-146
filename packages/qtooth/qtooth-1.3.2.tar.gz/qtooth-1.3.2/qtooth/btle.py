...
helperExe = os.path.join(script_path, "qtooth-helper")
...
class BTLEException(Exception):
    ...
    def __init__(self, message, resp_dict=None):
        ...
        # optional messages from qtooth-helper
        self.estat = None
        self.emsg = None
        if resp_dict:
        ...
        while True:
            ...
            if respType in wantType:
                ...
                if errcode=='nomgmt':
                    ...
                    raise BTLEException("Error from qtooth-helper (%s)" % errcode, resp)
            elif respType == 'scan':
...
class ScanEntry:
    ...
    def _update(self, resp):
        ...
        data = resp.get('d', [''])[0]
        self.unparsedData = resp.get('raw', None)
        self.rawData = data
