"""Python setup script for qtooth"""
...
def pre_install():
    """Do the custom compiling of the qtooth-helper executable from the makefile"""
    try:
        print("Working dir is " + os.getcwd())
        with open("qtooth/version.h","w") as verfile:
            verfile.write('#define VERSION_STRING "%s"\n' % VERSION)
        for cmd in [ "make -C ./qtooth clean", "make -C qtooth -j1" ]:
            print("execute " + cmd)
            msgs = subprocess.check_output(shlex.split(cmd), stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        print("Failed to compile qtooth-helper. Exiting install.")
...
setup (
    ...
    package_data={
        'qtooth': ['qtooth-helper', '*.json', 'bluez-src.tgz', 'qtooth-helper.c', 'version.h', 'Makefile']
    },
    cmdclass=setup_cmdclass,
    entry_points={
        'console_scripts': [
            'thingy52=qtooth.thingy52:main',
            'sensortag=qtooth.sensortag:main',
            'blescan=qtooth.blescan:main',
        ]
    }
)
