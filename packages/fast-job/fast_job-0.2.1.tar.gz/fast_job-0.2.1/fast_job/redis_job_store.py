...
class RedisJobStore(BaseJobStore):
    ...
    def add_job(self, job):
        ...
        while not self.redis.set(self.jobs_key + ":" + job.id, value="lock", px=100, nx=True):
            time.sleep(0.01)
        ...
        with self.redis.pipeline(transaction=False) as pipe:
            ...
            if job.next_run_time:
                add_item = {job.id: datetime_to_utc_timestamp(job.next_run_time)}
                pieces = []
                for pair in add_item.items():
                    pieces.append(pair[1])
                    pieces.append(pair[0])
                pipe.execute_command('ZADD', self.run_times_key, *pieces)
    ...
    def remove_job(self, job_id):
        ...
        while not self.redis.set(self.jobs_key + ":" + job_id, value="lock", px=100, nx=True):
            time.sleep(0.01)
        with self.redis.pipeline(transaction=False) as pipe:
