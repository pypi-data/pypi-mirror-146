...
class Redis:
	def __init__(self,localhost,port,password,db=0):
		"""redis中默认16个数据库 默认使用第一个 每个库互相不干扰"""
		pool = redis.ConnectionPool(host=localhost,db=db,port=port,password=password,decode_responses=True)
	...
	def 获取全部key(self):
		return self.r.keys()
	def 获取key模糊查询(self,key):
		return self.r.keys(pattern=f'*{key}*')
	def 获取key数量(self):
		"""返回当前数据库中key的数目"""
...
if __name__ == '__main__':
	r = Redis('58.56.20.70','8015','eagleredis')
	print(r.获取key模糊查询("367"))
	print(r.获取value(r.获取key模糊查询("367")[0]))