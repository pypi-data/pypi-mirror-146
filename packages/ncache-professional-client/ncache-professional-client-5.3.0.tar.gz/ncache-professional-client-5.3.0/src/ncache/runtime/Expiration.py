...
class Expiration:
    ...
    def set_expire_after(self, value):
        ...
        is_function = TimeSpan.type_checkTimespan(value, self.get_cacheitem)
        if is_function:
            javalocktimeout = TimeSpan.get_java_time_span(value)
        else:
            javalocktimeout = value.get_instance()
        self.__expiration.setExpireAfter(javalocktimeout)
