...
class TimeSpan:
    ...
    @staticmethod
    def get_java_time_span(value):
        if value == TimeSpan.time_span_zero:
            result = JavaInstancesFactory.get_java_instance("TimeSpan").Zero
        elif value == TimeSpan.time_span_max_value:
            result = JavaInstancesFactory.get_java_instance("TimeSpan").MaxValue
        elif value == TimeSpan.time_span_min_value:
            result = JavaInstancesFactory.get_java_instance("TimeSpan").MinValue
        return result

    @staticmethod
    def type_checkTimespan(value, function=None):
        v1=type(value)
        if value == TimeSpan.time_span_zero or value == TimeSpan.time_span_max_value or value == TimeSpan.time_span_min_value:
            return True
        elif type(value) is not TimeSpan:
            raise TypeError(ExceptionHandler.get_invalid_type_exception_message(function,TimeSpan, value))
        else:
            return False
