...
const EnvVarForm: React.FC<ImageFormProps> = ({ uiConfig }) => {
  ...
  React.useEffect(() => {
    ...
    APIGet(CM_PATH).then((data: UserConfigMapType) => {
      if (!cancelled) {
        const env_list = data.env;

        const excluded_envs = ["GPU", "emailID", "namespace", "s3BucketName", "accessKeyID", "secretAccessKey", "sharedS3BucketName", "sharedAccessKeyID", "sharedSecretAccessKey"]
        const excluded_envs_set = new Set(excluded_envs)
        const env = env_list.filter((environ) => {
            return !excluded_envs_set.has(environ)
        })
        console.log("Updated list of Environmental Variables:" + JSON.stringify({ env }))
  ...
  const postEnvChange = (updatedRows: VariableRow[]) => {
    const env = updatedRows.reduce((acc, row) => {
      ...
      // console.log("Type of Row Variables:" + rowVariables)
      acc.push(...rowVariables);
      return acc;
    }, [] as EnvVarType[]);
    
    console.log("Env Dictionary:" + JSON.stringify({ env }))
