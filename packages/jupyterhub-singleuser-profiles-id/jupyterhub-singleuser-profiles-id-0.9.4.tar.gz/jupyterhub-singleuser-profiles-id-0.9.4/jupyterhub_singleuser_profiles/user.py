...
class User(object):
  ...
  def get_envs(self, username, for_k8s=False):
    ...
    if data:
      ...
      # Custom ID Logic:
      excluded_envs= ["GPU", "emailID", "namespace", "s3BucketName", "accessKeyID", "secretAccessKey", "sharedS3BucketName", "sharedAccessKeyID", "sharedSecretAccessKey"]

      res = list(filter(lambda i: i['name'] not in excluded_envs, result))

    if for_k8s:
      res = self.openshift.create_pod_mapping(name, res)
