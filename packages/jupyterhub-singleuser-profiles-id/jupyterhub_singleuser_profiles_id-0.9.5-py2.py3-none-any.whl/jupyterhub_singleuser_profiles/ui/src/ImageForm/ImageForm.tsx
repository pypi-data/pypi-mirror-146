...
const ImageForm: React.FC<ImageFormProps> = () => {
  ...
  const postChange = (text) => {
    // Custom Code for updating the ConfigMap with the value of GPU Count according to the image selected by the user:
    let gpuCount = 0;
    if (text.includes("with-gpu")) {
      gpuCount = 1;
    }
    console.log("*********************************")
    console.log("The selected image is: " + text)
    console.log("The GPU Count will be: " + gpuCount)
    console.log("*********************************")
  ...
  // Custom Code for creating a new image list according to the category of images chosen by the user:
  const handleImageCategorySelection = (customOpt: string, checked: boolean) => {
    if (checked) {
      setSelectedCustomValue(customOpt);
      console.log("*********************************")
      console.log("The chosen image category is: " + customOpt);
  ...
  // Custom Code for creating & handling a list of category of images to be selected by the user:
  const customList = ["Tensorflow","Pytorch","Spark","Minimal-Lab"];
  ...
  return (
    <div className="jsp-spawner__option-section m-is-top">
      ...
      <div className="jsp-spawner__option-section__title"><strong> Notebook images </strong></div>
      <div><i> Select a Noteboook category to view Images associated with it </i></div> 
