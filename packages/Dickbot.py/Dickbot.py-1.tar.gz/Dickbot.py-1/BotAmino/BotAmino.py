...
class BotAmino(Command, Client, TimeOut, BannedWords):
    def __init__(self, email: str = None, password: str = None, sid: str = None, deviceId: str = None, proxies: str = None, certificatePath: str = None,proxy:bool=False):
        Command.__init__(self)
        Client.__init__(self, deviceId=deviceId, certificatePath=certificatePath, proxies=proxies)

        if email and password:
            self.login(email=email, password=password,proxy=proxy)
        elif sid:
            ...
            try:
                ...
                if para[2].strip()=="True":
                    self.login(email=para[0].strip(), password=para[1].strip(),proxy=True)
                else:
                    self.login(email=para[0].strip(), password=para[1].strip(),proxy=False)
