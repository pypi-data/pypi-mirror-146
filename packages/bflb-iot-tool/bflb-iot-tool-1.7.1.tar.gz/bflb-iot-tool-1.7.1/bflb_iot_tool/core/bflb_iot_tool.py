        ...
        if bintype == "fw":
            if group_type is None or group_type == "Group0":
                ...
                bootinfo_file = os.path.join(chip_path, chipname,
                                             "img_create_iot", "bootinfo_{0}.bin".format(group_type.lower()))
                img_file = os.path.join(chip_path, chipname,
                                        "img_create_iot", "img_{0}.bin".format(group_type.lower()))
        else:
            bootinfo_file = os.path.join(chip_path, chipname,
    ...
    def bl60x_mfg_flasher_eflash_loader_cfg(self,
        ...
        if bin_file is True:
            if 'fw_cpu0_addr' in pt_parcel:
                ...
                if bind_bootinfo is True:
                    ...
                    whole_img_len = 0x2000 + os.path.getsize(
                        chipnamedir + "/img_create_iot/img_group0.bin")
                    whole_img_data = bl_create_flash_default_data(whole_img_len)
        ...
        if d0fw is True and 'fw_d0_addr' in pt_parcel:
            ...
            if bind_bootinfo is True:
                ...
                whole_img_len = offset + os.path.getsize(
                    chipnamedir + "/img_create_iot/img_d0fw.bin")
                whole_img_data = bl_create_flash_default_data(whole_img_len)
    ...
    def flasher_download_cfg_ini_gen(self, chipname, chiptype, cpu_type, config):
        ...
        if config["check_box"]['bin_download'] is True:
            if chiptype == "bl808":
                ...
                cfg_t.read(self.bh_cfg_file)
                cfg_t.set(bootheader_section_name, 'group_image_offset', "0x%x" % (pt_parcel['fw_addr']+0x1000))
                cfg_t.write(self.bh_cfg_file)
            else:
        ...
        if 'd0_download' in config["check_box"] and 'd0_bin_input' in config['input_path']:    
            if config["check_box"]['d0_download'] is True:
                if chiptype == "bl808":
                    ...
                    cfg_t.read(self.bh_cfg_file)
                    cfg_t.set(bootheader_section_name, 'group_image_offset', "0x%x" % (pt_parcel['fw_d0_addr']+0x1000))
                    cfg_t.write(self.bh_cfg_file)
                else:
        ...
        return self.bl60x_mfg_flasher_eflash_loader_cfg(chipname, chiptype, bin_d, boot2_d, ro_params_d,
                                                pt_parcel, media_bin_d, mfg_bin_d, d0_bin_d, imtb_bin_d)
...
def iot_download_cmd(args):
    ...
    config["input_path"]["boot2_bin_input"] = None
    config["input_path"]["media_bin_input"] = None
    config["input_path"]["romfs_dir_input"] = None
    config["input_path"]["mfg_bin_input"] = None
    config["input_path"]["publickey"] = None
    config["input_path"]["privatekey"] = None
    ...
    if chiptype == "bl702":
        ...
        if not args.dts or not os.path.exists(args.dts): 
            config["input_path"]["dts_input"] = os.path.join(chip_path, chipname, "device_tree", gol.bl_factory_params_file_prefix+"IoTKitA_32M.dts")
        else:   
            config["input_path"]["dts_input"] = args.dts
    elif chiptype == "bl602":
        if not args.boot2:
            boot2_dir = find_boot2(os.path.join(chip_path, chipname, "builtin_imgs"), "boot2_iap_release.bin")
            if boot2_dir:
                config["input_path"]["boot2_bin_input"] = boot2_dir
        else:
            config["input_path"]["boot2_bin_input"] = args.boot2
        if not args.xtal: 
            config["param"]["chip_xtal"] = "40M"
        else:   
            config["param"]["chip_xtal"] = args.xtal 
        if not args.dts or not os.path.exists(args.dts): 
            config["input_path"]["dts_input"] = os.path.join(chip_path, chipname, "device_tree", gol.bl_factory_params_file_prefix+"IoTKitA_40M.dts")
        else:   
            config["input_path"]["dts_input"] = args.dts
    elif chiptype == "bl60x":
        if not args.boot2:
        ...
        if not args.xtal: 
            ...
            config["param"]["chip_xtal"] = args.xtal 
        if not args.dts or not os.path.exists(args.dts):  
            config["input_path"]["dts_input"] = os.path.join(chip_path, chipname, "device_tree", gol.bl_factory_params_file_prefix+"avb_38.4M.dts")
        else:   
            config["input_path"]["dts_input"] = args.dts
    elif chiptype == "bl808":
        if not args.boot2:
    ...
    if args.mfg:
        ...
        config["input_path"]["mfg_bin_input"] = args.mfg         
    if args.media:
        config["check_box"]["media_download"] = True
        config["input_path"]["media_bin_input"] = args.media   
    if args.romfs:
        config["check_box"]["use_romfs"] = True
        config["input_path"]["romfs_dir_input"] = args.media  
    
    bflb_utils.printf("boot2 bin is " + str(config["input_path"]["boot2_bin_input"]))      
    bflb_utils.printf("device tree is " + str(config["input_path"]["dts_input"])) 
    bflb_utils.printf("partition table is " + str(config["input_path"]["pt_bin_input"]))  
    bflb_utils.printf("firmware bin is " + str(config["input_path"]["cfg2_bin_input"]))    
    bflb_utils.printf("mfg bin is " + str(config["input_path"]["mfg_bin_input"])) 
    bflb_utils.printf("media bin is " + str(config["input_path"]["media_bin_input"])) 
    bflb_utils.printf("romfs dir is " + str(config["input_path"]["romfs_dir_input"])) 
...
def run():    
    ...
    parser.add_argument("--dts", dest="dts", default="", help="device tree") 
