...
def img_update_efuse_group1(cfg, sign, pk_hash, flash_encryp_type, flash_key,
                            sec_eng_key_sel, sec_eng_key):
...
def img_create_do(args, img_dir_path=None, config_file=None):
    ...
    if group_type == "group0":
        ...
        ret1, data_tohash1 = img_creat_process("group1", flash_img, cfg)
    elif group_type == "all":
        ret0, data_tohash0 = img_creat_process("group0", flash_img, cfg)
        ret1, data_tohash1 = img_creat_process("group1", flash_img, cfg)
    else:
...
def create_sp_media_image(config, cpu_type=None):
    ...
    cfg.read(config)
    if cpu_type == "Group1":
        img_creat_process("group1", 1, cfg)
    else:
