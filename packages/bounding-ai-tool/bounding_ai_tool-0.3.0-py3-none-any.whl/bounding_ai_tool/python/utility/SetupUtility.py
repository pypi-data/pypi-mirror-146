...
from bounding_ai_tool.python.utility.DefaultConfig import DefaultConfig
...
class SetupUtility:
    ...
    def _pip_install_packages(required_packages, python_bin, packages_path, reinstall_packages: bool = False, dry_run: bool = False, use_custom_package_path: bool = True):
        ...
        for package in required_packages:
            ...
            if not already_installed or reinstall_packages:
                ...
                if os.getenv("BOUNDING_AI_TOOL_NO_PIP_CACHE", 'False').lower() in ('true', '1', 't'):
        ...
        if os.path.exists(path_to_run_file):
            with open(path_to_run_file, "r") as file:
                ...
                for index, line in enumerate(lines):
                    if "import bounding_ai_tool" in line or "from bounding_ai_tool" in line:
                        return
                    else:
                        code = "\n".join(lines[:index + 2])
                        raise Exception('The given script "{}" does not have a bounding_ai_tool import at the top! '
                                        ...
                                        "Your code:\n#####################\n{}\n"
                                        "####################\nReplaces this with:\n"
                                        "import bounding_ai_tool as bproc".format(path_to_run_file, code))
        else:
    ...
    def determine_temp_dir(given_temp_dir: str) -> str:
        ...
        The name of the created temp dir contains a uuid, so multiple Bounding AI Tool processes
        ...
        temp_dir = os.path.join(temp_dir, "bounding_ai_tool_" + str(uuid.uuid4().hex))
        # Create the temp dir
