...
from bounding_ai_tool.python.modules.main.GlobalStorage import GlobalStorage
from bounding_ai_tool.python.modules.utility.Config import Config
from bounding_ai_tool.python.types.StructUtilityFunctions import get_instances
from bounding_ai_tool.version import __version__
...
def resolve_resource(relative_resource_path: str) -> str:
    """ Returns an absolute path to the given Bounding AI Tool resource.

    :param relative_resource_path: The relative path inside the Bounding AI Tool resource folder.
    :return: The absolute path.
    """
    return resolve_path(os.path.join(Utility.bounding_ai_tool_root, "bounding_ai_tool", "resources", relative_resource_path))
...
class Utility:
    bounding_ai_tool_root = os.path.join(os.path.dirname(__file__), "..", "..", "..")
    temp_dir = ""
    used_temp_id = None
            ...
            with Utility.BlockStopWatch("Initializing module " + module_config["module"]):
                for i in range(amount_of_repetitions):
                    ...
                    for suffix in ["Module", ""]:
                        try:
                            ...
                            module = importlib.import_module(
                                "bounding_ai_tool.python.modules." + module_config["module"] + suffix)
                    ...
                    if module_class is None:
                        raise Exception(
                            "The module bounding_ai_tool.python.modules." + module_config["module"] + " was not found!")
    ...
    def get_current_version() -> Optional[str]:
        ...
        :return: a string, the Bounding AI Tool version, or None if unavailable
        """
        if platform == "win32":
            return __version__
        try:
    ...
    class UndoAfterExecution:
        ...
        def __enter__(self):
            if self._perform_undo_op:
                from bounding_ai_tool.python.types.StructUtility import Struct
    ...
    def build_provider(name: str, parameters: Dict[str, Any]) -> "Provider":
        ...
        for suffix in ["Module", ""]:
            try:
                # Import class from bounding_ai_tool.python.modules
                module_class = getattr(importlib.import_module("bounding_ai_tool.python.modules.provider." + name + suffix),
                                       name.split(".")[-1] + suffix)
                break
            except ModuleNotFoundError:
                # Try next suffix
        ...
        # Throw an error if no module/class with the specified name + any suffix has been found
        if module_class is None:
            raise Exception("The module bounding_ai_tool.python.modules.provider." + name + " was not found!")
