...
from bounding_ai_tool.python.utility.Utility import KeyFrame
...
def set_intrinsics_from_blender_params(lens: float = None, image_width: int = None, image_height: int = None, clip_start: float = None, clip_end: float = None, 
    ...
    """ Sets the camera intrinsics using blender's represenation.
...
def set_intrinsics_from_K_matrix(K: Union[np.ndarray, Matrix], image_width: int, image_height: int,
    ...
    if abs(K[0][1]) > 1e-7:
        raise Exception(f"Skew is not supported by blender and therefore nor by Bounding AI Tool, set this to zero: {K[0][1]} and recalibrate")
