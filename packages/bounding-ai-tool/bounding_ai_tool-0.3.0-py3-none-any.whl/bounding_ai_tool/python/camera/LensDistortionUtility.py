...
from bounding_ai_tool.python.utility.MathUtility import change_source_coordinate_frame_of_transformation_matrix


"""
This file provides to functions to first set up the lens distortion used in this particular Bounding AI Tool
    ...
    while res[-1] > 0.15:
        ...
        if (it > 1) and (res[-1] > res[-2] * .99999):
            ...
            if error > 1e9:
                ...
                print("Bounding AI Tool will not generate incomplete images with void regions since these are not useful for ML (data leakage).")
...
def set_camera_parameters_from_config_file(camera_intrinsics_file_path: str, read_the_extrinsics: bool = False,
    ...
    The calibration file allows to use multiple cameras, but only one can be used inside of Bounding AI Tool per run.
    ...
    if extracted_camera_parameters.get("origin") is None or extracted_camera_parameters["origin"] != "center":
        raise Exception("The origin in the DLR-RMC camera calibration file has to be defined and set to center for Bounding AI Tool distortion to work.")
