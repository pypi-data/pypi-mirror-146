...
from bounding_ai_tool.python.postprocessing.PostProcessingUtility import trim_redundant_channels
from bounding_ai_tool.python.postprocessing.PostProcessingUtility import dist2depth, depth2dist
from bounding_ai_tool.python.types.EntityUtility import Entity
from bounding_ai_tool.python.utility.BlenderUtility import load_image
from bounding_ai_tool.python.utility.Utility import resolve_path, Utility, NumpyEncoder
from bounding_ai_tool.python.utility.MathUtility import change_coordinate_frame_of_point, \
    change_source_coordinate_frame_of_transformation_matrix, change_target_coordinate_frame_of_transformation_matrix
import bounding_ai_tool.python.camera.CameraUtility as CameraUtility
...
def write_hdf5(output_dir_path: str, output_data_dict: Dict[str, List[Union[np.ndarray, list, dict]]],
    ...
    for frame in range(bpy.context.scene.frame_start, bpy.context.scene.frame_end):
        ...
        with h5py.File(hdf5_path, "w") as file:
            ...
            bounding_ai_tool_version = Utility.get_current_version()
            if bounding_ai_tool_version is not None:
                WriterUtility._write_to_hdf_file(file, "bounding_ai_tool_version", np.string_(bounding_ai_tool_version))
