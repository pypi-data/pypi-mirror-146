...
if sys.version_info.major < 3:
    raise Exception("Bounding AI Tool requires at least python 3.X to run.")
...
if "INSIDE_OF_THE_INTERNAL_BLENDER_PYTHON_ENVIRONMENT" in os.environ:
    # Remove the parent of the Bounding AI Tool folder, as it might contain other packages
    # that we do not want to import inside the bounding_ai_tool env
    ...
    # this checks if bounding_ai_tool the command line tool or the cli.py script are used. If not an exception is thrown
    import traceback
    ...
    if sys.platform == "win32":
        ...
        # check if the name of this file is either bounding_ai_tool or if the
        # "OUTSIDE_OF_THE_INTERNAL_BLENDER_PYTHON_ENVIRONMENT_BUT_IN_RUN_SCRIPT" is set, which is set in the cli.py
        is_correct_startup_command = first_file_name in ["bounding_ai_tool", "command_line.py"]
    if "OUTSIDE_OF_THE_INTERNAL_BLENDER_PYTHON_ENVIRONMENT_BUT_IN_RUN_SCRIPT" not in os.environ \
        ...
        raise Exception("\n###############\nThis script can only be run by \"bounding_ai_tool run\", instead of calling:"
                        "\n\tpython {}\ncall:\n\tbounding_ai_tool run {}\n###############".format(sys.argv[0], sys.argv[0]))
