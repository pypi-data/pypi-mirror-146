...
import subprocess
from bounding_ai_tool.python.utility.SetupUtility import SetupUtility
from bounding_ai_tool.python.utility.InstallUtility import InstallUtility
...
def cli():
    ...
    parser = argparse.ArgumentParser(description="Bounding AI Tool: A procedural Blender tool for synthetic image generation.", formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument('-v', '--version', action='store_true', help='Version of Bounding AI Tool')
    subparsers = parser.add_subparsers(dest='mode', help="Select a Bounding AI Tool command to run:")

    # Setup different modes
    parser_run = subparsers.add_parser('run', help="Runs the Bounding AI Tool pipeline in normal mode.")
    parser_quickstart = subparsers.add_parser('quickstart', help="Runs a quickstart script bounding_ai_tool/scripts/quickstart.py")
    parser_debug = subparsers.add_parser('debug', help="Runs the Bounding AI Tool pipeline in debug mode. This will open the Blender UI, so the 3D scene created by the pipeline can be visually inspected.")
    parser_vis = subparsers.add_parser('vis', help="Visualize the content of Bounding AI Tool output files. \nOptions: {}".format(", ".join(options['vis'])), formatter_class=argparse.RawTextHelpFormatter)
    ...
    for subparser in [parser_run, parser_debug, parser_quickstart]:
        subparser.add_argument('file', default=None, nargs='?', help='The path to a configuration file which describes what the pipeline should do or a python file which uses Bounding AI Tool via the API.')
    ...
    for subparser in [parser_run, parser_debug, parser_pip, parser_quickstart]:
        ...
        subparser.add_argument('--custom-blender-path', dest='custom_blender_path', default=None, help="Set, if you want to use a custom blender installation to run the Bounding AI Tool. If None is given, blender is installed into the configured blender_install_path. This argument is ignored if it is specified in the given YAML config.")

    args, unknown_args = parser.parse_known_args()

    if args.version:
        from bounding_ai_tool import __version__
        print(__version__)
        ...
        if args.mode == "quickstart":
            path_src_run = os.path.join(repo_root_directory, "bounding_ai_tool", "scripts", "quickstart.py")
            args.file = path_src_run
            print("'bounding_ai_tool quickstart' is an alias for 'bounding_ai_tool run {}'".format(path_src_run))
        elif is_config:
            print("\033[33m" + "Warning: Running Bounding AI Tool with config.yaml files is deprecated and will be removed in future releases.\n",
                "Please switch to the more intuitive Python API introduced in Bounding AI Tool 2.0. It's easy, you won't regret it." + "\033[0m")
            path_src_run = os.path.join(repo_root_directory, "bounding_ai_tool", "run.py")
        else:
        ...
        # this is done to enable the import of bounding_ai_tool inside of the blender internal python environment
        ...
        if args.mode == "debug":
            p = subprocess.Popen([blender_run_path, "--python-use-system-env", "--python-exit-code", "0", "--python", os.path.join(repo_root_directory, "bounding_ai_tool/debug_startup.py"), "--", path_src_run if not is_config else args.file, temp_dir] + unknown_args, env=used_environment)
        else:
    ...
    # Import the required entry point
        ...
        if args.mode == "vis" and args.vis_mode == "hdf5":
            from bounding_ai_tool.scripts.visHdf5Files import cli
        elif args.mode == "vis" and args.vis_mode == "coco":
            from bounding_ai_tool.scripts.vis_coco_annotation import cli
        elif args.mode == "extract" and args.extract_mode == "hdf5":
            from bounding_ai_tool.scripts.saveAsImg import cli
        elif args.mode == "download" and args.download_mode == "blenderkit":
            from bounding_ai_tool.scripts.download_blenderkit import cli
        elif args.mode == "download" and args.download_mode == "cc_textures":
            from bounding_ai_tool.scripts.download_cc_textures import cli
        elif args.mode == "download" and args.download_mode == "haven":
            from bounding_ai_tool.scripts.download_haven import cli
        elif args.mode == "download" and args.download_mode == "ikea":
            from bounding_ai_tool.scripts.download_ikea import cli
        elif args.mode == "download" and args.download_mode == "pix3d":
            from bounding_ai_tool.scripts.download_pix3d import cli
        elif args.mode == "download" and args.download_mode == "scenenet":
            from bounding_ai_tool.scripts.download_scenenet import cli
        else:
