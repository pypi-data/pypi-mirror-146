...
class RunBounding_AI_ToolOperator(bpy.types.Operator):
    bl_idname = "wm.run_bounding_ai_tool"
    bl_label = "Run Bounding AI Tool"
    bl_description = "This operator runs the loaded Bounding AI Tool script and also makes sure to unload all modules before starting."
    bl_options = {"REGISTER"}

    def execute(self, context):
        # Delete all loaded models inside src/, as they are cached inside blender
        for module in list(sys.modules.keys()):
            if module.startswith("bounding_ai_tool") and module not in ["bounding_ai_tool.python.utility.SetupUtility"]:
                del sys.modules[module]

        # Make sure the parent of the bounding_ai_tool folder is in sys.path
...
bpy.utils.register_class(RunBounding_AI_ToolOperator)

# Declare function for drawing the header toolbar of the scripting area
# Mostly copied from blender's script: scripts/startup/bl_ui/space_text.py
def draw(self, context):
    ...
    if text:
        ...
        if is_osl:
            ...
            row.operator("wm.run_bounding_ai_tool", text="Run Bounding AI Tool")
