    ...
    def fit(
        ...
        **kwargs: Any,
        ...
        target_epsilon (Optional[float]): Defaults to None.
            ...
            There are three behaviors depending on the value of `target_epsilon`:
              ...
              * `target_epsilon > 0` training is performed remotely with
                 ...
                 change `epochs`.
              * `target_epsilon is None` The default epsilon is queried from the
                 server. It then falls back to the one of the previously
                 mentionned case.
        ...
        if target_epsilon is None:
            client = self.x.dataset.client

            target_epsilon = client.session.get(
                f"{client.base_url}/datasets/{self.x.dataset.id}/default_epsilon"
            ).json()
