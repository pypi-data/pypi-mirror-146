...
# Example commands
BioSAK get_GTDB_taxon_gnm -meta bac120_metadata_r202.tsv -path genome_paths.tsv -taxon taxons.txt -cpl 85 -ctm 5
BioSAK get_GTDB_taxon_gnm -meta ar122_metadata_r202.tsv -path genome_paths.tsv -taxon taxons.txt -cpl 50 -ctm 10
...
def get_GTDB_taxon_gnm(args):
            ...
            for each_interested_taxon in taxons_to_retrieve_set:
                if each_interested_taxon in gtdb_taxonomy:
                    ...
                    if cpl_cutoff is None:
                        ...
                        if gnm_completeness >= float(cpl_passed):
                            cpl_passed = True
    ...
    for each_gnm_to_retrieve in sorted([i for i in gnm_set_to_retrieve]):
        ...
        gnms_taxonomy_txt_handle.write('%s\t%s\t%s\t%s\n' % (each_gnm_to_retrieve, gnm_cpl_dict[each_gnm_to_retrieve], gnm_ctm_dict[each_gnm_to_retrieve], gnm_taxon))
