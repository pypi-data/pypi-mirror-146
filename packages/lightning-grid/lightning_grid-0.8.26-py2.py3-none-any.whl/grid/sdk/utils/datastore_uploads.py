...
def _do_upload(session: requests.Session, file_path: Path, task: UploadTask, progress_bar: PBar) -> UploadTask:
    ...
    try:
        ...
        response = session.put(task.url, data=data, verify=False)
...
def _upload_next_batch(c: 'GridRestClient', work: Work, progress_bar: PBar) -> Dict[str, DataFile]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=env.UPLOAD_MAX_WORKER_THREADS
                                               ) as executor, requests.Session() as s:
        s.mount(
            'https://',
            HTTPAdapter(
                pool_connections=env.UPLOAD_MAX_WORKER_THREADS,
                pool_maxsize=env.UPLOAD_MAX_WORKER_THREADS,
                max_retries=retries,
            )
        )
        futures = []
