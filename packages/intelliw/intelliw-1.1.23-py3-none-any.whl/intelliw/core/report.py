    ...
    def report(self, msg):
        ...
        if self.addr is not None:
            try:
                ...
                if self.is_local:
                    self.logger.info('{} {}'.format(response, str(response.content)))
            except Exception as e:
                stack_info = traceback.format_exc()
                self.logger.error("failed to report, url: [{}], request: [{}], exception: [{}], stack:\n{}".
                                  format(self.addr, data, e, str(stack_info)))
        if self.is_local:
            frame = sys._getframe(1)
            filename = os.path.basename(frame.f_code.co_filename)
            self.logger.info("trace_id: [{}], file: {}[{}], msg: {}".format(trace_id, filename, frame.f_lineno, data))
