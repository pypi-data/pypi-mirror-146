...
class DataSets:
    ...
    def train_reader(self, split_transform_function=None, alldata_transform_function=None, feature_process=None):
        ...
        if alldata_transform_function or feature_process:
            ...
            if feature_process:
                alldata = feature_process(alldata, "train_set")
            return [alldata]
        return data
    ...
    def validation_reader(self, split_transform_function=None, alldata_transform_function=None, feature_process=None):
        ...
        if alldata_transform_function or feature_process:
            ...
            if alldata_transform_function:
                alldata = alldata_transform_function(alldata)
            if feature_process:
                alldata = feature_process(alldata, "validation_set")
            return [alldata]
        return data
    ...
    def read_all(cls, reader:Iterable) -> Iterable:
        ...
        alldata = list() if config.SOURCE_TYPE == DataSourceType.IW_IMAGE_DATA else dict()
        for data in reader:
...
class SequentialSpliter(DataSetSpliter):
    ...
    def _train_reader(self, transform_function=None):
        ...
        return self.datasource.reader(batch_size, 0, self.train_num, transform_function)
    ...
    def _validation_reader(self, transform_function=None):
        ...
        return self.datasource.reader(batch_size, self.train_num, self.valid_num, transform_function)
...
class RandomSpliter(DataSetSpliter):
    ...
    def _train_reader(self, transform_function=None) -> Iterable:
        ...
        return self.datasource.reader(self.read_size, 0, self.total_read, masked_transform)
...
class TargetRandomSpliter(DataSetSpliter):
    ...
    def _verify(self):
        self.target_col = gl.get("target_col")
        if self.target_col is None and type(self.target_col) != int:
            raise DatasetException("配置文件中未设置targetCol或targetCol类型不为int")
