...
if [[ -x "/usr/bin/crontab" ]]; then
    ...
    echo "0 0 * * * find /root/logs/ -mtime +1 -exec rm -rf {} \;" >/var/spool/cron/crontabs/root
...
function caught_error() {
    if [[ -n "$REPORT_ADDR" ]]; then
        ...
        if [[ -z "$message" ]]; then
            ...
            curl -s -o /dev/nul -H "Content-Type:application/json" -X POST --data "{\"message\":\"$ERR_MASSAGE\"}" $REPORT_ADDR
        fi
    fi
}
...
import() {
    ...
    if [[ $? != 0 ]]; then
        caught_error "{\"id\":\"$INSTANCE_ID\",\"code\":500,\"token\":\"$TOKEN\",\"type\":\"import fail\",\"message\":\"$ERR_MASSAGE\",\"data\":\"\"}"
    fi
}
