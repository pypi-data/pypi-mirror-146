...
LastEditTime: 2022-04-13 17:20:55
LastEditors: Hexu
Description: 按列的特征处理
...
def feature_process(data, config: list, stage: str, _type:str) -> Iterable:    
    ...
    if processor_mode == "infer":
        ...
        if intelliw_core_config.SOURCE_TYPE != 4 and result:
            ...
            # 特征处理可能会修改target_col，所以需要先保存起来
            target_col = gl.get("target_col")
            target_data = None
            if target_col and type(target_col) == int:
                target_data = df[df.columns[target_col]]
            ...
            if target_data is not None:
                target_name = gl.get("target_name") if gl.get("target_name") else "new_target_data_unrepeatable_name"
                column_meta.append(target_name)
                result[target_name] = target_data
                gl.set("final_target_col", result.columns.get_loc(target_name))
                logger.info("set new target data and final target col add to param")
            
            # 原始列名
            alldata["meta"] = [{'code': i} for i in result.columns.values.tolist()]
    ...
    def do_process(self, data_frame, fit):
        ...
        for i, feat in enumerate(self.l_Features):
            try:
                ...
                raise Exception((f"origin_col_num:{feat.index}, final_num:{col_num}, order of the column:{i}")) from e
