...
class BatchService:
    def __init__(self, format, path, src, dst, rownumaddr, response_addr=None, task='infer'):
        ...
        if not self.is_once:
            assert (croniter.is_valid(format))
        self.src = src
        self.dst = dst
    ...
    def run(self):
        if self.is_once:
            ...
            iter = croniter(self.format, datetime.now())
            iter.get_next()
            while True:
                if iter.get_current(datetime) < datetime.now():
                    try:
                        logger.info("{} 开始批处理任务".format(
                            iter.get_current(datetime)))
                        job(self.reporter, self.path, self.src, self.dst,
                            self.rownumaddr, self.task)
                    except:
