...
class ELMClassifier:
    ...
    def __init__(self, input_dimension:int, hidden_dimension:int=1000, load_path:Optional[str]=None) -> None:
        ...
        if load_path is None:
            ...
            self.weights = self._reconstruct_weights(
                positive_indexes=loadtxt(f"{load_path}/positive_indexes.txt",dtype=int),
                negative_indexes=loadtxt(f"{load_path}/negative_indexes.txt",dtype=int),
                shape=(hidden_dimension,len(self.LABELS))
            )
    ...
    def save(self, name:str) -> str:
        ...
        savetxt(f"{path}/positive_indexes.txt",positive_indexes,fmt="%d")
        savetxt(f"{path}/negative_indexes.txt",negative_indexes,fmt="%d")
    ...
    @staticmethod
    def _deconstruct_weights(weights:ndarray, binary_threshold:float=1e-3) -> Tuple[ndarray,ndarray]:
