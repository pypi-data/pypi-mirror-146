...
We will continue to support the original Coiled system for some months in order to allow you to adapt your workflows and make any required changes.  We don't expect many breaking changes, but we want to ensure a smooth experience.
...
- Spot instance support not implemented
- ``worker_class`` parameter not implemented
- ``use_scheduler_public_ip`` not implemented
- ``protocol`` parameter not implemented
- ``environ`` parameter not implemented
...
-   Cluster configurations have been deprecated so the ``configuration`` argument is no longer allowed
    ...
    Configuration is now directly passed to the ``Cluster`` class at creation time
