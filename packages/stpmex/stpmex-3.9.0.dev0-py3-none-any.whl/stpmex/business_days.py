...
def current_cdmx_time_zone(fecha: dt.datetime) -> str:
    ...
    # april_first_sunday = next(day for day in days if day == SUNDAY)

    for i in count():
        day = april + dt.timedelta(days=i)
        if day.weekday() == SUNDAY:
            april_first_sunday = day
            break

    for i in count():
        day = october - dt.timedelta(days=i)
        if day.weekday() == SUNDAY:
            octuber_last_sunday = day
            break
