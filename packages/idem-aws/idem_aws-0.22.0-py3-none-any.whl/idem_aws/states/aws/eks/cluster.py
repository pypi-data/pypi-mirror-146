    ...
    try:
        if before and before["ret"]:
            ...
            try:
                ...
                result["comment"] = (f"Created aws.eks.cluster '{name}'",)
...
async def absent(
    ...
    **Autogenerated function**
    ...
    Args:
        name(Text): A name, ID to identify the resource.
        resource_id(Text): AWS EKS cluster name to identify cluster
    ...
    if not before["ret"]:
        result["comment"] = (f"aws.eks.cluster '{name}' already absent",)
        ...
        try:
            ...
            result["comment"] = result["comment"] + (
                f"Deleted aws.eks.cluster '{name}'",
            )
        except hub.tool.boto3.exception.ClientError as e:
