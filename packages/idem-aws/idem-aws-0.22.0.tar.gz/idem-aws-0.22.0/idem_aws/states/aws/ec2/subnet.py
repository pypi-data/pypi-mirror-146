...
async def absent(hub, ctx, name: str, resource_id: str = None) -> Dict[str, Any]:
    ...
    **Autogenerated function**
    ...
    result = dict(comment="", old_state=None, new_state=None, name=name, result=True)
    if not resource_id:
        result["comment"] = (f"aws.ec2.subnet '{name}' already absent",)
        return result
    resource = hub.tool.boto3.resource.create(ctx, "ec2", "Subnet", resource_id)
    before = await hub.tool.boto3.resource.describe(resource)
    ...
    if not before:
        ...
        try:
            ...
            if not result["result"]:
                result["comment"] = ret["comment"]
                return result
            result["comment"] = (f"Deleted aws.ec2.subnet '{name}'",)
        except hub.tool.boto3.exception.ClientError as e:
            result["comment"] = result["comment"] + (f"{e.__class__.__name__}: {e}",)
...
async def search(
    ...
    if not ret["ret"]["Subnets"]:
        result["comment"] = (
            f"Unable to find aws.ec2.subnet resource with filters {filters} and subnet id {resource_id}",
        )
        return result
