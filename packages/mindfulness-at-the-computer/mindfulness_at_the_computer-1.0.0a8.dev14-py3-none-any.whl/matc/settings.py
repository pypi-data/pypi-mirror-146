...
settings: dict = {
    ...
    SK_DEBUG_USE_SECONDS_FOR_BR_TIMER: False,
}
...
def settings_file_exists() -> bool:
    return os.path.isfile(matc.globa.get_settings_file_path())
...
def backup_settings_file() -> None:
    ...
    shutil.copyfile(matc.globa.get_settings_file_path(), new_file_name_sg)
...
def save_settings_to_json_file(i_json_file_path: str = ""):
    logging.debug("Saving to json file")
    if i_json_file_path:
        json_file_path = i_json_file_path
    else:
        json_file_path = matc.globa.get_settings_file_path()
...
def update_dict_with_json_data(i_min_dict_ref: dict, i_file_path: str):
