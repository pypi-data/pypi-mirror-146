...
class BreathingAndRestDlg(QtWidgets.QDialog):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(matc.constants.APPLICATION_PRETTY_NAME)
    ...
    def close_dlg(self):
        ...
        self.breathing_gv.breathing_count_int = 0
...
TIME_LINE_DOT_DURATION_INT = 5000
...
class BreathingGraphicsView(QtWidgets.QGraphicsView):
    ...
    def __init__(self, i_can_be_closed: bool = True) -> None:
        ...
        self.dot_qtimeline.setCurveShape(QtCore.QTimeLine.LinearCurve)
...
class MyQGraphicsTextItem(QtWidgets.QGraphicsTextItem):
    ...
    def hoverMoveEvent(self, event: QtWidgets.QGraphicsSceneHoverEvent) -> None:
        cursor_posx = event.pos().x()
        cursor_posy = event.pos().y()
        half_height: int = self.boundingRect().height() // 2
        upper_rect = QtCore.QRect(self.boundingRect().x(), self.boundingRect().y(),
            self.boundingRect().width(), half_height)
