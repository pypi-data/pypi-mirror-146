...
class SettingsWin(QtWidgets.QMainWindow):
    show_intro_dialog_signal = QtCore.pyqtSignal()

    def __init__(self):
        super().__init__()
        self.updating_gui: bool = True
        self.widget_w1 = QtWidgets.QWidget()
        self.setGeometry(100, 64, 400, 640)
        ...
        vbox_l2 = QtWidgets.QVBoxLayout()
        self.widget_w1.setLayout(vbox_l2)
        ...
        vbox_l2.addWidget(self.visualizations_qgb)
        vbox_l4 = QtWidgets.QVBoxLayout()
        self.visualizations_qgb.setLayout(vbox_l4)
    ...
    def on_bp_current_row_changed(self):
        self.edit_bp_qpb.setEnabled(
            self.breathing_phrases_qlw.currentRow() != 0
        )
    ...
    def on_add_bp_clicked(self):
        ...
        BreathingPhraseEditDialog.start(new_id)
        self._add_bp_to_gui(new_id)

    def _add_bp_to_gui(self, i_id: int, i_row: int = -1):
    ...
    def on_edit_bp_clicked(self):
        ...
        id: int = current_item.data(QtCore.Qt.UserRole)

        BreathingPhraseEditDialog.start(id)
        qlwi = self.breathing_phrases_qlw.takeItem(current_row)

        self._add_bp_to_gui(id, current_row)
    ...
    def on_br_time_value_changed(self, i_new_value: int):
        if self.updating_gui:
            return
        if matc.settings.settings[matc.settings.SK_DEBUG_USE_SECONDS_FOR_BR_TIMER]:
            matc.settings.settings[matc.settings.SK_BREATHING_BREAK_TIMER_SECS] = i_new_value
        else:
    ...
    def update_menu(self):
        ...
        update_gui_action = QtWidgets.QAction("Update settings GUI", self)
        debug_menu.addAction(update_gui_action)
        update_gui_action.triggered.connect(self.update_gui)
        self.seconds_for_br_timer_action = QtWidgets.QAction("Seconds for breathing timer", self)
        debug_menu.addAction(self.seconds_for_br_timer_action)
        self.seconds_for_br_timer_action.setCheckable(True)
        self.seconds_for_br_timer_action.toggled.connect(self.on_seconds_for_br_timer_toggled)
        self.open_settings_dir_action = QtWidgets.QAction("Open settings dir", self)
    ...
    def on_open_settings_dir_triggered(self):
        path_: str = os.path.dirname(matc.globa.get_settings_file_path())
        QtGui.QDesktopServices.openUrl(QtCore.QUrl(path_))

    def show_intro_dialog(self):
        self.show_intro_dialog_signal.emit()

    def on_seconds_for_br_timer_toggled(self):
        if self.updating_gui:
            return
        matc.settings.settings[matc.settings.SK_DEBUG_USE_SECONDS_FOR_BR_TIMER] = (
            self.seconds_for_br_timer_action.isChecked())

        value: int = self.breathing_break_time_qsb.value()
        if matc.settings.settings[matc.settings.SK_DEBUG_USE_SECONDS_FOR_BR_TIMER]:
            matc.settings.settings[matc.settings.SK_BREATHING_BREAK_TIMER_SECS] = value
        else:
            matc.settings.settings[matc.settings.SK_BREATHING_BREAK_TIMER_SECS] = 60 * value
    ...
    def update_gui(self):
        ...
        if matc.globa.active_phrase_id == matc.globa.NO_PHRASE_SELECTED_INT:
            ...
            # breathing_phrase = matc.model.PhrasesM.get(matc.globa.active_phrase_id_it)
            ...
            # if (i_event_source != matc.globa.EventSource.breathing_list_selection_changed and i_event_source != matc.globa.EventSource.rest_list_selection_changed):
            # self.breathing_settings_wt.phrases_qlw.update_gui()

            # self.update_systray()

            # Update the timers pages in intro or settings
            # if (i_event_source == matc.globa.EventSource.breathing_settings_changed_from_intro or i_event_source == matc.globa.EventSource.rest_settings_changed_from_intro):
            # logging.debug("Updating settings page because settings have changed from intro")
            # self.timing_settings_wt.update_gui()
            pass

        # if (self.intro_dlg and (i_event_source == matc.globa.EventSource.breathing_settings_changed_from_settings or i_event_source == matc.globa.EventSource.rest_settings_changed_from_settings)):
        ...
        if self.seconds_for_br_timer_action:
            seconds_used = matc.settings.settings[matc.settings.SK_DEBUG_USE_SECONDS_FOR_BR_TIMER]
            self.seconds_for_br_timer_action.setChecked(seconds_used)
