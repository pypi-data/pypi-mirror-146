...
from azureml._vendor.azure_cli_core.util import in_cloud_console, can_launch_browser
...
class Profile:
    ...
    def login(self,
        ...
        if not subscriptions and not allow_no_subscriptions:
            raise AzureMLException("No subscriptions found for {}.".format(username))
    ...
    def login_with_managed_identity(self, identity_id=None, allow_no_subscriptions=None):
        ...
        if identity_id:
            if is_valid_resource_id(identity_id):
                ...
                authenticated = False
                from azureml._vendor.azure_cli_core.azclierror import AzureResponseError
                try:
                ...
                if not authenticated:
                    raise AzureMLException('Failed to connect to MSI, check your managed service identity id.')
        ...
        if not subscriptions:
            if allow_no_subscriptions:
                ...
                raise AzureMLException("No access was configured for the VM, hence no subscriptions were found. If this"
                                       " is expected, use '--allow-no-subscriptions' to have tenant level access.")
    ...
    def login_in_cloud_shell(self):
        ...
        subscriptions = subscription_finder.find_using_specific_tenant(tenant, msi_creds)
        if not subscriptions:
            raise AzureMLException('No subscriptions were found in the cloud shell')
    ...
    def get_login_credentials(self, resource=None, client_id=None, subscription_id=None, aux_subscriptions=None,
        ...
        if aux_tenants and aux_subscriptions:
            raise AzureMLException("Please specify only one of aux_subscriptions and aux_tenants, not both")
        ...
        if managed_identity_type is None:
            ...
            from azureml._vendor.azure_cli_core.auth.credential_adaptor import CredentialAdaptor
    ...
    def get_raw_token(self, resource=None, scopes=None, subscription=None, tenant=None):
        ...
        if subscription and tenant:
            raise AzureMLException("Please specify only one of subscription and tenant, not both")
        ...
        if identity_type:
            # MSI
            if tenant:
                raise AzureMLException("Tenant shouldn't be specified for MSI account")
            ...
            # Cloud Shell
            if tenant:
                raise AzureMLException("Tenant shouldn't be specified for Cloud Shell account")
    ...
    def _new_account(self):
        ...
        from azureml._vendor.azure_cli_core.profiles import ResourceType, get_sdk
    ...
    def set_active_subscription(self, subscription):  # take id or name
        ...
        if len(result) != 1:
            raise AzureMLException("The subscription of '{}' {} in cloud '{}'.".format(
    ...
    def get_current_account_user(self):
        try:
            ...
            raise AzureMLException('There are no active accounts.')
    ...
    def get_subscription(self, subscription=None):  # take id or name
        ...
        if not subscriptions:
            raise AzureMLException(_AZ_LOGIN_MESSAGE)
        ...
        if not result and subscription:
            raise AzureMLException("Subscription '{}' not found. Check the spelling and "
                                   "casing and try again.".format(subscription))
        if not result and not subscription:
            raise AzureMLException("No subscription found. Run 'az account set' to select a subscription.")
        if len(result) > 1:
            raise AzureMLException("Multiple subscriptions with the name '{}' found. Specify "
                                   "the subscription ID.".format(subscription))
        return result[0]
    ...
    def get_sp_auth_info(self, subscription_id=None, name=None, password=None, cert_file=None):
        ...
        if name and (password or cert_file):
            ...
            if user_type == _SERVICE_PRINCIPAL:
                ...
                raise AzureMLException('SDK Auth file is only applicable when authenticated using a service principal')
        ...
        from azureml._vendor.azure_cli_core.cloud import CloudEndpointNotSetException
...
class SubscriptionFinder:
    ...
    def _create_subscription_client(self, credential):
        from azureml._vendor.azure_cli_core.profiles import ResourceType, get_api_version
        from azureml._vendor.azure_cli_core.profiles._shared import get_client_class
        from azureml._vendor.azure_cli_core.commands.client_factory import _prepare_mgmt_client_kwargs_track2
