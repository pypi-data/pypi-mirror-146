...
class AbstractDataset(object):
        ...
        if partition_key_values_common_path.common_path:
            ...
            if step_type == 'Microsoft.DPrep.GetDatastoreFilesBlock':
                ...
                for store in stored_datastores:
                    ...
                    stored_path_on_datastore = store['path']
                ...
                for detail in details:
                    stored_path = detail['path']
                    store['path'] = _get_optimal_path(common_path_list, stored_path.split('/'), verbose)
                ...
                for stream_info in stream_infos:
                    ...
                    stored_path_on_datastore = stream_info['resourceidentifier']
                    datastore = common_path_list[0]
                    if stored_datastore != datastore:
                        # we know it doesn't match what we need, so remove the stream_info and continue
                        del stream_info
    ...
    def _get_partition_key_values(self, get_common_path=False, partition_keys=None):
        ...
        if not get_common_path:
            ...
            _remove_data_read_steps_and_drop_columns(dataflow, False)
            ...
            dataflow = self._dataflow.summarize(
                summary_columns=[
    ...
    def _load(path: str, workspace):
        ...
        with open(local_yaml_path, "r") as stream:
            try:
                ...
                if AbstractDataset._should_auto_inference(mltable_yml, activity_data):
    ...
    def _should_auto_inference(mltable_yaml, activity_data):
        ...
        elif 'transformations' in mltable_yaml.keys() and \
                'read_delimited' in [k for d in mltable_yaml['transformations'] for k, v in d.items()]:
            return True
