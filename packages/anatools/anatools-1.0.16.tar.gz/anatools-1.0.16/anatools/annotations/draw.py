...
def draw(image_path, out_dir, draw_type='box_2d', object_ids=None, object_types=None, line_thickness=1): 
    ...
    for type in metadata_types:
        ...
        type_colors[type] = generate_color
    ...
    cv2.imwrite(out_dir+'/'+image_name+'-annotated-'+draw_type+'.'+image_ext, draw_img)
    
    print('Image saved to directory: ' + out_dir)

def box_2d(coordinates, bbox_img, line_thickness, color):
    """
    ...
    bbox_img = cv2.rectangle(bbox_img, start, end, color, line_thickness)
    return bbox_img

def box_3d(coordinates, bbox_img, line_thickness, color):
    """
    ...
    cv2.line(bbox_img, point_1, point_2, color, line_thickness)
    cv2.line(bbox_img, point_1, point_5, color, line_thickness)
    cv2.line(bbox_img, point_5, point_6, color, line_thickness)
    cv2.line(bbox_img, point_2, point_6, color, line_thickness)

    cv2.line(bbox_img, point_3, point_4, color, line_thickness)
    cv2.line(bbox_img, point_3, point_7, color, line_thickness)
    cv2.line(bbox_img, point_7, point_8, color, line_thickness)
    cv2.line(bbox_img, point_4, point_8, color, line_thickness)

    cv2.line(bbox_img, point_3, point_2, color, line_thickness)
    cv2.line(bbox_img, point_7, point_6, color, line_thickness)
    cv2.line(bbox_img, point_4, point_1, color, line_thickness)
    cv2.line(bbox_img, point_8, point_5, color, line_thickness)

    return bbox_img

def segmentation(coordinates, draw_img, line_thickness, color):
    """
    ...
    for i in range(len(coordinates)):
        pts = np.reshape(coordinates[i], [int(len(coordinates[i])/2), 2])
        cv2.polylines(draw_img, [pts], True, color, line_thickness)
