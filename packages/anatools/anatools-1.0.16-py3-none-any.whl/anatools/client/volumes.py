...
def upload_volume_data(self, volumeId, files=[], localDir=None, sync=False):
    ...
    for index, hash in enumerate(source_hashes):
        if (sync == True and (hash in destination_hashes)):
            print(f"\x1b[1K\rsync: {source_files[index]}'s hash exists", end='\n' if self.verbose else '', flush=True)
        elif sync == False or (hash not in destination_hashes):
            self.refresh_token()
            fileinfo = self.ana_api.putVolumeData(volumeId=volumeId, keys=[source_files[index]])[0]
            try:
                filepath = os.path.join(localDir, source_files[index])
                
                print(f"\x1b[1K\rupload: {source_files[index]} to the volume. [{index+1} / {len(source_files)}]", end='\n' if self.verbose else '', flush=True)
                with open(filepath, 'rb') as filebytes:
                    ...
                    if response.status_code != 204:
                        faileduploads.append(source_files[index])
                        print(f"\x1b[1K\rupload: {source_files[index]} failed", end='\n' if self.verbose else '', flush=True)
                    else:
                        print(f"\x1b[1K\rupload: {source_files[index]} completed", end='\n' if self.verbose else '', flush=True)
            except:
                traceback.print_exc()
                faileduploads.append(source_files[index])
                print(f"\x1b[1K\rupload: {source_files[index]} failed", end='\n' if self.verbose else '', flush=True)
    ...
    if len(faileduploads):
        print('The following files failed to upload:', faileduploads, flush=True)
