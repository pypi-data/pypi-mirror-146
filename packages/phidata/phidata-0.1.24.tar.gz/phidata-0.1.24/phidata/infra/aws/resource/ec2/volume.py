...
class EbsVolume(AwsResource):
    ...
    def _create(self, aws_client: AwsApiClient) -> bool:
        ...
        try:
            ...
            # Build create_volume args
            non_null_create_volume_args = {}
            # create a dict of args which are not null, otherwise aws type validation fails
            if self.encrypted:
                non_null_create_volume_args["Encrypted"] = self.encrypted
            if self.iops:
                non_null_create_volume_args["Iops"] = self.iops
            if self.kms_key_id:
                non_null_create_volume_args["KmsKeyId"] = self.kms_key_id
            if self.outpost_arn:
                non_null_create_volume_args["OutpostArn"] = self.outpost_arn
            if self.snapshot_id:
                non_null_create_volume_args["SnapshotId"] = self.snapshot_id
            if self.volume_type:
                non_null_create_volume_args["VolumeType"] = self.volume_type
            if self.dry_run:
                non_null_create_volume_args["DryRun"] = self.dry_run
            if tags:
                non_null_create_volume_args["TagSpecifications"] = [
                    {
                        "ResourceType": "volume",
            ...
            if self.throughput:
                non_null_create_volume_args["Throughput"] = self.throughput
            if self.client_token:
                non_null_create_volume_args["ClientToken"] = self.client_token

            # Create Volume
            service_resource = self.get_service_resource(aws_client)
            print_info(f"Creating EbsVolume: {self.name}")
            ...
            print_error(
                "EbsVolume could not be created, this operation is known to be buggy."
            )
        ...
        if self.wait_for_creation:
            try:
                if self.volume_id is not None:
                    print_info(f"Waiting for EbsVolume to be created, this can take a while. Volume id: {self.volume_id}")
                ...
                print_warning(
                    f"Waiter for {self.get_resource_name()} failed, downstream actions might fail."
                )
                print_warning(e)
                print_warning("---+---")
        return True
    ...
    def _read(self, aws_client: AwsApiClient) -> Optional[Any]:
        ...
        try:
            ...
            print_warning(f"Cannot read {self.get_resource_type()}.")
