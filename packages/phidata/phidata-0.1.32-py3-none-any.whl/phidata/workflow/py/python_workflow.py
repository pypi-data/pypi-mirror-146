...
class PythonWorkflowArgs(WorkflowArgs):
    ...
    # For a PythonWorkflow this task is a python function
    # For example:
...
class PythonWorkflow(Workflow):
    ...
    def __init__(
        ...
        super().__init__()
        self.args: Optional[PythonWorkflowArgs] = None
    ...
    def add_airflow_tasks_to_dag(self, dag: Any) -> bool:
        ...
        task = PythonOperator(
            task_id=task_id,
            # TODO: fix the type issues by removing type: ignore
            python_callable=entrypoint,  # type: ignore
    ...
    def create_db_engines(self):
        ...
        if self.args.entrypoint_args is not None and isinstance(
            self.args.entrypoint_args, list
        ):
            for entrypoint_arg in self.args.entrypoint_args:
                if isinstance(entrypoint_arg, SqlTable):
                    create_db_engine_using_conn_id(entrypoint_arg)
