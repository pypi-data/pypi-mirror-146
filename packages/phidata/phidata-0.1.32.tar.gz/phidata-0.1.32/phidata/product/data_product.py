...
class DataProduct:
    ...
    def create_airflow_dag(
        ...
        dag_id = self.dag_id
        if dag_id is None:
            print_error("DataProduct dag_id unavailable")
            return None
        if self.workflows is None:
            logger.warning(f"DAG {dag_id} has no workflows")
            return None
        dag = DAG(
            dag_id=dag_id,
        ...
        for workflow in self.workflows:
            ...
            if not _add_task_success:
                print_error(f"Tasks for workflow {workflow.name} could not be added")
