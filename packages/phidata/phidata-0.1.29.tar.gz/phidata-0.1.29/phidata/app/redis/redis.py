...
class RedisArgs(DbAppArgs):
    ...
    # Provide REDIS_SCHEMA as redis_schema or REDIS_SCHEMA in env_file
...
class Redis(DbApp):
    ...
    def get_db_schema(self) -> Optional[str]:
        ...
        if redis_schema_var is None and self.args.env_file is not None:
            # read from env_file
            logger.debug(f"Reading REDIS_SCHEMA from env_file")
            env_data_from_file = self.get_env_data_from_file()
            redis_schema_var = env_data_from_file.get("REDIS_SCHEMA", redis_schema_var)
