...
def get_docker_resources_from_group(
    ...
    for resource_key, resource_data in docker_resource_group.__dict__.items():
            ...
            for _r in resource_data:
                if isinstance(_r, DockerResource):
                    if name_filter is not None:
                        ...
                        # logger.debug(f"resource name: {_r.get_resource_name().lower()}")
                        if name_filter.lower() not in _r.get_resource_name().lower():
                            logger.debug(f"skipping {_r.get_resource_type()}:{_r.get_resource_name()}")
                            continue

                    if type_filter is not None:
                        # logger.debug(f"type_filter: {type_filter.lower()}")
                        # logger.debug(f"class: {_r.get_resource_type().lower()}")
                        if type_filter.lower() not in _r.get_resource_type().lower():
                            logger.debug(f"skipping {_r.get_resource_type()}:{_r.get_resource_name()}")
                            continue
        ...
        # DockerResource and add it to the docker_resources list
            ...
            if name_filter is not None:
                if name_filter.lower() not in resource_data.get_resource_name().lower():
                    logger.debug(f"skipping {resource_data.get_resource_type()}:{resource_data.get_resource_name()}")
                    continue

            if type_filter is not None:
                # logger.debug(f"type_filter: {type_filter.lower()}")
                # logger.debug(f"class: {resource_data.get_resource_type().lower()}")
                if type_filter.lower() not in resource_data.get_resource_type().lower():
                    logger.debug(f"skipping {resource_data.get_resource_type()}:{resource_data.get_resource_name()}")
                    continue
