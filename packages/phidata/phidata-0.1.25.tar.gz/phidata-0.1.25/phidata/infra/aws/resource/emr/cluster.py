...
class EmrCluster(AwsResource):
    ...
    # The auto-termination policy defines the amount of idle time in seconds after which a cluster automatically terminates.
    ...
    def _create(self, aws_client: AwsApiClient) -> bool:
        ...
        try:
            ...
            non_null_args = {}
            if self.log_uri:
                non_null_args["LogUri"] = self.log_uri
            if self.log_encryption_kms_key_id:
                non_null_args["LogEncryptionKmsKeyId"] = self.log_encryption_kms_key_id
            if self.additional_info:
                non_null_args["AdditionalInfo"] = self.additional_info
            if self.release_label:
                non_null_args["ReleaseLabel"] = self.release_label
            if self.instances:
                non_null_args["Instances"] = self.instances
            if self.steps:
                non_null_args["Steps"] = self.steps
            if self.bootstrap_actions:
                non_null_args["BootstrapActions"] = self.bootstrap_actions
            if self.supported_products:
                non_null_args["SupportedProducts"] = self.supported_products
            if self.new_supported_products:
                non_null_args["NewSupportedProducts"] = self.new_supported_products
            if self.applications:
                non_null_args["Applications"] = self.applications
            if self.configurations:
                non_null_args["Configurations"] = self.configurations
            if self.job_flow_role:
                non_null_args["JobFlowRole"] = self.job_flow_role
            if self.service_role:
                non_null_args["ServiceRole"] = self.service_role
            if self.tags:
                non_null_args["Tags"] = self.tags
            if self.security_configuration:
                non_null_args["SecurityConfiguration"] = self.security_configuration
            if self.auto_scaling_role:
                non_null_args["AutoScalingRole"] = self.auto_scaling_role
            if self.scale_down_behavior:
                non_null_args["ScaleDownBehavior"] = self.scale_down_behavior
            if self.custom_ami_id:
                non_null_args["CustomAmiId"] = self.custom_ami_id
            if self.ebs_root_volume_size:
                non_null_args["EbsRootVolumeSize"] = self.ebs_root_volume_size
            if self.repo_upgrade_on_boot:
                non_null_args["RepoUpgradeOnBoot"] = self.repo_upgrade_on_boot
            if self.kerberos_attributes:
                non_null_args["KerberosAttributes"] = self.kerberos_attributes
            if self.step_concurrency_level:
                non_null_args["StepConcurrencyLevel"] = self.step_concurrency_level
            if self.managed_scaling_policy:
                non_null_args["ManagedScalingPolicy"] = self.managed_scaling_policy
            if self.placement_group_configs:
                non_null_args["PlacementGroupConfigs"] = self.placement_group_configs
            if self.auto_termination_policy:
                non_null_args["AutoTerminationPolicy"] = self.auto_termination_policy
            ...
            print_info(f"Creating EmrCluster: {self.name}")
            create_response = service_client.run_job_flow(
                Name=self.name,
                **non_null_args,
            )
            # logger.debug(f"EmrCluster: {create_response}")
            # logger.debug(f"EmrCluster type: {type(create_response)}")

            self.job_flow_id = create_response.get("JobFlowId", None)
            self.cluster_arn = create_response.get("ClusterArn", None)
            if create_response is not None:
                print_info(f"EmrCluster created: {self.name}")
            ...
            print_error(
                f"{self.get_resource_type()} could not be created, this operation is known to be buggy."
            )
    ...
    def post_create(self, aws_client: AwsApiClient) -> bool:
        ...
        if self.wait_for_creation:
            try:
                print_info("Waiting for EmrCluster to be active, this can take a while")
                ...
                print_error(
                    f"Waiter ClusterRunning {self.name} failed, downstream actions might fail."
                )
                print_error(e)
                print_error("---+---")
        return True
    ...
    def _read(self, aws_client: AwsApiClient) -> Optional[Any]:
        ...
        try:
            ...
            # logger.debug(f"EmrClusters: {list_response}")
            # logger.debug(f"EmrClusters type: {type(list_response)}")
            ...
            if cluster_summary_list is not None and isinstance(
                ...
                for _cluster_summary in cluster_summary_list:
                    ...
                    if cluster_name == self.name:
                        self.cluster_summary = _cluster_summary
                        break

            if self.cluster_summary is None:
                logger.debug("No EmrCluster found")
                return None

            logger.debug(f"EmrCluster found: {self.name}")
            # logger.debug(f"cluster_summary: {self.cluster_summary}")
            self.job_flow_id = self.cluster_summary.get("Id", None)
            self.cluster_arn = self.cluster_summary.get("ClusterArn", None)
            self.active_resource = self.cluster_summary
    ...
    def _delete(self, aws_client: AwsApiClient) -> bool:
        ...
        try:
            ...
            if self.job_flow_id:
                ...
                print_info(f"EmrCluster deleted: {self.name}/{self.job_flow_id}")
            else:
                print_error("Could not find cluster")
            return True
        except Exception as e:
            print_error(
                f"{self.get_resource_type()} could not be deleted, this operation is known to be buggy."
            )
