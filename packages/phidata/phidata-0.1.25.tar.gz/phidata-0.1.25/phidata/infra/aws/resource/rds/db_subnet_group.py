...
class DbSubnetGroup(AwsResource):
    ...
    def _create(self, aws_client: AwsApiClient) -> bool:
        ...
        try:
            ...
            if self.dbcluster_parameter_group_name:
                ...
                ] = self.dbcluster_parameter_group_name
            ...
            if vpc_security_group_ids is None:
                if self.vpc_stack is not None:
                    vpc_stack_sg = self.vpc_stack.get_security_group(
            ...
            if db_subnet_group_name is None:
                if self.vpc_stack is not None:
                    private_subnets = self.vpc_stack.get_private_subnets(
                        aws_client=aws_client
                    )
            ...
            print_info(f"Creating DbSubnetGroup: {self.name}")
            create_response = service_client.create_db_cluster(
                Engine=self.engine,
                **non_null_args,
            )
            # logger.debug(f"DbSubnetGroup: {create_response}")
            # logger.debug(f"DbSubnetGroup type: {type(create_response)}")

            self.db_cluster = create_response.get("DBCluster", None)
            if create_response is not None:
                # print_info(f"DbSubnetGroup created: {self.name}")
                logger.debug(f"DbSubnetGroup: {self.db_cluster}")
    ...
    def _read(self, aws_client: AwsApiClient) -> Optional[Any]:
        ...
        try:
            ...
            # logger.debug(f"DbSubnetGroups: {describe_response}")
            # logger.debug(f"DbSubnetGroups type: {type(describe_response)}")
            ...
            if self.db_cluster is None:
                logger.debug("No DbSubnetGroup found")
                return None

            # logger.debug(f"DbSubnetGroup found: {db_cluster_identifier}")
            # logger.debug(f"DbSubnetGroup: {self.db_cluster}")
            self.active_resource = self.db_cluster
    ...
    def _delete(self, aws_client: AwsApiClient) -> bool:
        ...
        try:
            ...
            print_info(f"DbSubnetGroup deleted: {db_cluster_identifier}")
            return True
        except Exception as e:
            print_error(
