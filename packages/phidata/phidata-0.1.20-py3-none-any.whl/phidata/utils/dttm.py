...
def dttm_to_dttm_str(
    dttm: dt.datetime, dttm_format: str = "%Y-%m-%dT%H:%M:%S"
) -> Optional[str]:
    """Convert a datetime object to formatted string"""

    if dttm and isinstance(dttm, dt.datetime):
        try:
            dttm_str = dt.datetime.strftime(dttm, dttm_format)
            return dttm_str
        except Exception as e:
            pass
    return None
...
def days_ago(n=1, hour=0, minute=0, second=0, microsecond=0):
    ...
    """
    today = utc_now().replace(hour=hour, minute=minute, second=second, microsecond=microsecond)
