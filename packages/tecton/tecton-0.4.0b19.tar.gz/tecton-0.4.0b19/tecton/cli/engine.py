...
class PlanDiff:
    ...
    def _pprint_materialization_diff(self, item: state_update_pb2.FcoDiff):
        colwidth = 16
        indent = 4
        diffs = item.materialization_info.materialization_task_diffs
        # TODO: Seperate materialization_task_diffs into different fields: streaming, backfill, and recurring batch
        streaming_jobs = [diff.display_string for diff in diffs if "streaming" in diff.display_string.lower()]
        backfill_jobs = [diff.display_string for diff in diffs if "backfill" in diff.display_string.lower()]
        summary_list = []
        if len(backfill_jobs) == 1:
            summary_list.append("1 backfill")
        elif len(backfill_jobs) > 0:
            summary_list.append(str(len(backfill_jobs)) + " backfills")
        if len(streaming_jobs) > 0:
        ...
        if len(backfill_jobs) > 0:
            printer.safe_print(indent * " " + f"{'> backfill:'.ljust(colwidth)} {backfill_jobs[0]}")
            for batch_job in backfill_jobs[1:]:
                printer.safe_print(indent * " " + f"{' '.ljust(colwidth)} {batch_job}")
        if len(streaming_jobs) > 0:
            printer.safe_print(indent * " " + f"{'> incremental:'.ljust(colwidth)} {streaming_jobs[0]}")
    ...
    def _pprint_plan_item_with_warnings(
        ...
        if len(item.materialization_info.materialization_task_diffs) > 0:
...
def get_declared_fco_args(objects) -> Tuple[List[FcoArgs], FeatureRepoSourceInfo]:
    ...
    for fco_obj in objects:
        ...
        elif isinstance(fco_obj, (Entity, fwv5.Entity)):
        ...
        elif isinstance(fco_obj, (FeatureService, fwv5.FeatureService)):
