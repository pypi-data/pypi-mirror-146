...
class OpenDssWriter:
    ...
    def reg_control_to_str(reg_control: RegControl) -> str:
        tapwinding = f' tapwinding={reg_control.tap_winding}' if reg_control.tap_winding else ''
    ...
    def master_to_str(master: Master) -> str:
        model_files_str = ""
        if OpenDssWriter.has_growthshape(master):
            years = []
            multi = []
            for index, value in enumerate(master.load_model.growthshape.shape):
                years.append(str(index))
                multi.append(str(value))
            years_str = f"[{', '.join(years)}]"
            multi_str = f"[{', '.join(multi)}]"
            model_files_str += f"New GrowthShape.{master.load_model.growthshape.uid} " \
                               f"npts={len(years)} " \
                               f"Year={years_str} " \
                               f"Mult={multi_str}\n"
        if OpenDssWriter.has_line_codes(master):
