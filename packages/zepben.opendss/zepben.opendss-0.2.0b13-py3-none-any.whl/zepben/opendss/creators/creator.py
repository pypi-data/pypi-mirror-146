...
class OpenDssNetworkCreator(
    ...
    def energy_source_creator(
        ...
        tx_bus_conn = BusConnection(connected_topological_node, connected_topological_node.nodes)
        cir_bus_conn = BusConnection(Bus(f"{uid}-ZSTX-bus", connected_topological_node.nodes), connected_topological_node.nodes)
        ...
        tx = Transformer(
            ...
            windings=[
                TransformerWinding(
                    ...
                    kv=circuit.base_kv,
                    kva=100000,
                    bus_conn=circuit.bus_conn
                ),
                TransformerWinding(
                    conn="wye",
                    kv=circuit.base_kv,
                    kva=100000,
                    bus_conn=tx_bus_conn
                )
            ]
        )

        bus_branch_network.set_circuit(circuit)
        bus_branch_network.add_bus(cir_bus_conn.bus)
        bus_branch_network.add_bus(tx_bus_conn.bus)
        bus_branch_network.add_transformer(tx)
        return {
            circuit.uid: circuit,
            cir_bus_conn.bus.uid: cir_bus_conn,
            tx_bus_conn.bus.uid: tx_bus_conn,
            tx.uid: tx
        }
