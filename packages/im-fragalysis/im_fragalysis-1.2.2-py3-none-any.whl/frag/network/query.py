...
def get_full_graph(smiles,
    ...
    """
    debug_msg = f"get_full_graph({smiles}, graph_url={graph_url}, isomericSmiles={isomericSmiles})"
    smiles = canon_input(smiles, isomericSmiles)
    debug_msg += f" canon_smiles={smiles}"
    print(debug_msg)
    ...
    with driver.session() as session:
        ...
        for record in session.read_transaction(find_proximal, smiles):
            ...
            ans = define_proximal_type(record)
            records.append(ans)
        debug_msg += f" n_proximals={debug_proximal_num_records}"
        debug_double_edge_num_records = 0
        for record in session.read_transaction(find_double_edge, smiles):
            debug_double_edge_num_records += 0
            ans = define_double_edge_type(record)
            records.append(ans)
        debug_msg += f" n_double_edge={debug_double_edge_num_records}"
        for label in list(set([x.label for x in records])):
            # Linkers are meaningless
            if "." in label:
                continue
        if records:
            ...
            debug_msg += f" orga_dict_len={orga_dict_len}"
            print(debug_msg)
            return orga_dict
        else:
            debug_msg += f" (fall-through) ret=None"
            print(debug_msg)
            return None
