...
def _convert_query_to_qlsql(query):
    if not isinstance(query, rockset.Query):
        raise ValueError(
            ...
            "Please pass a valid rockset.Query object as input.".format(type(query))
        )
...
class QueryLambdaClient(object):
    ...
    def create(self, name, query, workspace="commons", description=None, **kwargs):
        ...
        ql_model = self._api.create(workspace=workspace, body=request).get("data")
    ...
    def retrieveByVersion(self, name, version=None, workspace="commons", **kwargs):
    ...
    def retrieveByTag(self, name, tag=None, workspace="commons", **kwargs):
        ...
        ql_model = (
            self._api.get(workspace=workspace, query_lambda=name, tag=tag)
            .get("data")
            .get("version")
        )
        return QueryLambdaVersion(self._api, ql_model)
...
class QueryLambdaVersion(rockset.swagger_client.models.QueryLambdaVersion):
    ...
    def tag(self, tag):
        ...
        """
        tag_request = CreateQueryLambdaTagRequest(version=self.version, tag_name=tag)
        ql_model = (
            self._api.create_0(
                workspace=self.workspace, query_lambda=self.name, body=tag_request
            )
        ...
        if parameters:
            if not isinstance(parameters, rockset.ParamDict):
                raise ValueError(
                    ...
                    "Please pass a valid rockset.ParamDict object as input.".format(
                        type(parameters)
                    )
                )
            all_params = []
