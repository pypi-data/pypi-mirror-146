import pandas as pd
from tools.hun_dropbox import HunDropbox
    ...
    def __get_best_data(self, strategy_df):
        ...
        for market in df['market'].drop_duplicates().values:
            if market == self.market:
                con0 = (df['market'] == self.market)
                con1 = (df['hpr'] > self.hpr)
                con2 = (df['tradings_days_ratio']) > self.tradings_days_ratio
                con3 = (df['taking_days_max']) < self.taking_days_max
                df = df[con0 & con1 & con2 & con3]
                df = df.sort_values(by=['hpr', 'tradings_days_ratio'])

            if df.empty:
                raise Exception(f"{self.market}은 존재하지만 조건에 맞는 데이터 없음")
            else:
                _ = df.iloc[-1]
                return _.to_dict()
