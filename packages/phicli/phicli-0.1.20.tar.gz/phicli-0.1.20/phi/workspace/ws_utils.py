...
def get_ws_config_file_path(ws_dir: Path) -> Path:
    ...
    ws_workspace_dir = ws_dir.joinpath("workspace")
    ...
    if ws_workspace_dir.exists() and ws_workspace_dir.is_dir():
        ...
        if ws_config_file is not None and ws_config_file.exists() and ws_config_file.is_file():
    ...
    ws_name = ws_dir.stem
    ws_name_dir = ws_dir.joinpath(ws_name)
    ...
    # Case 3: Look for a folder defined by the pyproject.toml file
    ws_pyproject_toml = ws_dir.joinpath("pyproject.toml")
    if ws_pyproject_toml.exists() and ws_pyproject_toml.is_file():
        ...
        if phidata_conf is not None:
            ...
            phidat_conf_workspace_dir = ws_dir.joinpath(phidat_conf_workspace_dir_str)
            logger.debug(f"searching {phidat_conf_workspace_dir}")
            if phidat_conf_workspace_dir.exists() and phidat_conf_workspace_dir.is_dir():
                ws_config_file = phidat_conf_workspace_dir.joinpath("config.py")
                if ws_config_file is not None and ws_config_file.exists() and ws_config_file.is_file():
                    return ws_config_file

    print_error(f"Could not find a workspace/config.py for {ws_dir}")
    exit(0)
