...
def daily_seckill(client: Factory):
    ...
    :return:
    """
    goodsIds = client.goods.filter_goods(group='3858')
    ...
    if client.category.set_category(0, [3858], *goods_ids):
        client.logger.info(f'{len(goods_ids)}个商品成功加入每日特价分类')
    if client.group.update_groups('每日特价', *goods_ids):
        client.logger.info(f'{len(goods_ids)}个商品成功加入每日特价分组')
