...
from ..env import CLIENT_DB_USERNAME, CLIENT_DB_PASSWORD
...
def to_local(
    ...
    """Download the prediction results to local directory."""
...
def to_mysql(
    ...
    """Push the prediction results to mysql database."""
    ...
    if quiet:
        status.wait()
        typer.echo(
            f"Results for prediction id: {pred.id} are successfully pushed to mysql database."
        )
    else:
        typer.echo(
            f"Pushing the results for prediction id: {pred.id} to mysql database."
        )
        status.progress_bar()
