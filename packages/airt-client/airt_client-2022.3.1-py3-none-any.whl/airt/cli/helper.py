...
from ..env import SERVER_URL, CLI_CMD, SERVICE_TOKEN
    ...
    def wrapper_decorator(*args, **kwargs):
        try:
            ...
            if f"'{SERVICE_TOKEN}'" in str(e):
                typer.echo(
                    f"\nPlease run the command '{CLI_CMD} token' to get the application token and set it in the "
                    f"environment variable `{SERVICE_TOKEN}`."
                )
                typer.echo(f"\nTry '{CLI_CMD} token --help' for help.")
