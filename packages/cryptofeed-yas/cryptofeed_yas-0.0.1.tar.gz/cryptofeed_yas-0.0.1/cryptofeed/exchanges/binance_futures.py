...
class BinanceFutures(Binance, BinanceFuturesRestMixin):
    ...
    async def _account_update(self, msg: dict, timestamp: float):
        ...
        for position in msg['a']['P']:
            ...
            await self.callback(POSITIONS,message, timestamp)
        ...
        for balance in msg['a']['B']:
            ...
            await self.callback(BALANCES,message, timestamp)
    ...
    async def _order_update(self, msg: dict, timestamp: float):
        ...
        message = {
                ...
                'execution_type': msg['o']['x'],
                'type': msg['o']['o'],
                'status': msg['o']['X'],
                'side': msg['o']['S'],
                'symbol': msg['o']['s'].replace('USDT', '-USDT-PERP'),
                'avg_price': Decimal(msg['o']['ap']) if not Decimal.is_zero(Decimal(msg['o']['ap'])) else None,
                'amount': Decimal(msg['o']['q']),
                'remaining': Decimal(msg['o']['q']) - Decimal(msg['o']['z']),
                'timestamp': msg['E']/1000.0
            }
        await self.callback(ORDER_INFO, message, timestamp)
