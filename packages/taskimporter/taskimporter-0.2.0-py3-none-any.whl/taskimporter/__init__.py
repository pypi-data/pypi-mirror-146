...
from taskimporter.services import services, BaseService
from taskimporter.task_managers import task_managers, BaseTaskManager
...
async def add_task_worker(name, work_queue, task_manager, service):
    while not work_queue.empty():
        a_task = await work_queue.get()
        print("%s - Processing task: %s" % (name, a_task.name))
        if task_manager in task_managers:
            await task_managers[task_manager].add_task(a_task, project=service.project_key)


async def mark_task_done_worker(name, work_queue, task_manager, service):
    while not work_queue.empty():
        a_task = await work_queue.get()
        print("%s - Processing task: %s" % (name, a_task.name))
        if task_manager in task_managers:
...
async def process_service_worker(name, service: BaseService, task_manager: str):
    work_queue = asyncio.Queue()

    print("%s - Pulling open tasks from %s" % (name, service.name))
    tasks = await service.get_open_tasks()
    await asyncio.gather(
        *[work_queue.put(tsk) for tsk in tasks]
    )
    await asyncio.gather(
        *[add_task_worker("%s - Add Task Worker: %s" % (name, i), work_queue, task_manager, service) for i in range(5)]
    )
    ...
    await asyncio.gather(
        *[mark_task_done_worker("%s - Close Task Worker: %s" % (name, i), work_queue, task_manager, service) for i in range(5)]
    )
