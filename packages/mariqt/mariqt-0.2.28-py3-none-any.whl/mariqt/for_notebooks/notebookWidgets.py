...
class FileParserWidget(ItemWidgetBase):
    ...
    def __init__(self,defaultFields:dict,on_change_fct,requiredFields=[],iFDO=None,ignoreDateTime=True,allowConvert=True,converters:list=[],minFields=1,fixedItemWidthPixels:int=200,allowCustomFields:bool=True):
        """
        ...
        self.columnOptions = []
        self.fieldNameOptions = [key for key, value in self.defaultFields.items() if '-set-' not in key]
        ...
        self.auxFile_widget = widgets.GridBox([ widgets.HBox([widgets.Label("File:"),widgets.VBox([self.fileValid])],layout=widgets.Layout(justify_content='space-between')),self.auxFile_file_widget,
                                                ...
                                                auxFile_columsLabel_widget,widgets.VBox([self.auxFile_ColumnsBox_widget])],layout=widgets.Layout(
    ...
    def on_addButton_clicked(self,b,fieldName="",columnName="",converter={}):
        ...
        newHeaderFieldWidget = HeaderFieldWidget(self.columnOptions,self.fieldNameOptions,[self.on_change_fct,self.on_headerFieldWidget_changed],ignoreDateTime=self._ignoreDateTime,allowConvert=self._allowConvert,converters=self._converters, allowCustomFields=self.allowCustomFields)
    ...
    def copy(self):
        ret = FileParserWidget(self.defaultFields,self.on_change_fct,self.requiredFields,self.iFDO,ignoreDateTime=self._ignoreDateTime,allowConvert=self._allowConvert,converters=self._converters,fixedItemWidthPixels=self.fixedItemWidthPixels,allowCustomFields=self.allowCustomFields)
        ret.setParams(self.getParams())
        return ret
...
class HeaderFieldWidget(widgets.VBox):
    ...
    def __init__(self,columnOptions:list,fieldOptions:list,on_change_fct,ignoreDateTime=True,allowConvert=True,converters:list=[],allowCustomFields:bool=True):
        ...
        if not self.ignoreDateTime:
            self.dateTimeFieldKey = "datetime"
            self.dateFieldKey = "date"
            self.timeFieldKey = "time"
...
class ConverterBase(abc.ABC):
    ...
    @staticmethod
    def getAllNonAbstractSubclasses(baseClass):
        subClasses = []
        for subClass in baseClass.__subclasses__():
            if not inspect.isabstract(subClass):
                subClasses.append(subClass)
            subClasses += ConverterBase.getAllNonAbstractSubclasses(subClass)
        return subClasses
...
class FileParserDateTimeWidget(FileParserWidget):
    ...
    def __init__(self,defaultFields:dict,on_change_fct,dateTimeParser=[],iFDO=None,allowCustomFields=True):
        converters = [ e() for e in ConverterBase.getAllNonAbstractSubclasses(ConverterBase)]
        FileParserWidget.__init__(self,defaultFields,on_change_fct,requiredFields=[],iFDO=None,ignoreDateTime=False,allowConvert=True,converters=converters,allowCustomFields=allowCustomFields)
