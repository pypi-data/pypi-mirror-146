...
def 帮助():
    print(logs.颜色(
        ...
        色选="红色"))
    if isConnected():
        print(logs.系统日志(f"Fishconsole{fcv.version()} 检查更新开始。。。"))
        v = fcv.version()
        ...
        if res_v != v:
            print(logs.系统日志(f"Fishconsole{fcv.version()} 检查更新结束"))
    ...
    if help_mod == "1":
        ...
        print(
            f"logs.分割线('这是logs模块的', 'Fishconsole{fcv.version()}帮助文档'\n)"
            ...
            "‘input输入加强’，原版的input支持输出提示文字，但是默认为空且不好识别，我就针对这种情况对它进行重写了一次，这一次，辨识度绝对高/dogedoge/手臂\n"
            "---------------------\n"
            "‘分割线’的语法是:\n"
            "logs.分割线(标题,文本,颜色选择)\n\n"
            "‘日志’的语法是:\n"
        ...
        res = window.列表选择对话框("关于控制台输出模块的帮助文档会显示在命令行中，选中你想查看的内容，我们将会提供相应的窗口。为什么有的地方找不到",
        ...
        if res is not None:
            for a in res:
                if a == "logs控制台输出辅助模块":
                    print(logs.分割线("这是logs模块的", f"Fishconsole{fcv.version()} 帮助文档", "红色"))
                    print(
                        "你是不是不知道怎么用的？那就让我来告诉你吧!!\n"
                        "这个函数是一个输出工具的集合，是由鱼鱼有几斤整理的\n"
                        "我整理了这几个项目\n"
                ...
                if a == "tool玩具模块":
                    ...
                    for a in res:
                        ...
                        if a == "文件存在性检测模块":
                            window.弹窗(
                                "就是找文件，他返回的是布尔值，有呢就是True，没有就是False，虽然没什么鸟用，但是总有人要用/dogedoge/咖啡\n"
                                "-----------------------------\n\n"
                                "语法就是\n"
                                "tool.文件存在性检测(文件名)"
                                , "文件存在性检测模块", "ok")
