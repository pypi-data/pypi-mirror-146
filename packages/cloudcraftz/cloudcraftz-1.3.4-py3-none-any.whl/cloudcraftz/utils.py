...
def analyze_strategy(actions, prices_with_dates, plot=False):
    '''
    actions: a list , np.array or series of actions .
    plot: default-False, boolean
    prices_with_dates: a list of prices along with their respective dates (pd.DataFrame)
    ...
    Example:
    >>> analyze_strategy([1, 1, 1, ..-1], df, True)
    actions : +1 , 0 or -1 . (Buy, Hold or Sell)

    outputs summary for the actions taken .
    '''
    actions = pd.DataFrame(actions, columns=['actions'])
    df = pd.concat([prices_with_dates, actions], axis=1)
    df['returns'] = df['prices'].pct_change() * df['actions']
...
def get_benchmark_result(ticker, start, end, plot=False):
    '''
    start: start date
    end: end date
    ...
    df = data.loc[:, 'Close'].reset_index().rename(columns={"Close": "prices"})
