...
class image_scraper:
        ...
        def download(self, query=None, limit=1, arguments=None):
            ...
            for i in range(limit):
                skip = False
                url = urls[item]
                downloadPath = self.download_image(url, arguments, prefix, query)
        ...
        def download_image(self, url, arguments, prefix, query):
            if arguments and 'path' in arguments:
                path = arguments['path']
            else:
                path = self.path
            ...
            if arguments and 'download_format' in arguments:
                try:
                    ...
                    downloadPath = join(currentPath, f"{'-'.join(query)}-{prefix}.{imageFormat}")
                ...
                if formatEnd == -1:
                    formatEnd = len(url)
                formatStart = url.rfind('.', 0, formatEnd)
                imageFormat = url[formatStart:formatEnd]
                downloadPath = join(currentPath, f"{'-'.join(query)}-{prefix}.{imageFormat}")
                if exists(downloadPath):
                    return 2
...
if __name__ == '__main__':
        ...
        images = search.download(query='cats', limit=10, arguments={'download_format': 'png', 'color': 'black'})
        print(images)
