...
from .ir.django_index import DjangoIndex
from .ir.data_dict import DataDict
...
class DatabaseSchemaEditor(BaseDatabaseSchemaEditor):
    ...
    _alter_property_data = {
        'model': None,
        'property': None
    }  # "ALTER TABLE %(table)s ADD COLUMN %(column)s %(definition)s"
    _remove_property_data = {
        'model': None,
        'property': None
    }  # "ALTER TABLE %(table)s DROP COLUMN %(column)s"
    _create_constraints_data = {
        'constraints': None
    }
    _create_foreignkey_constraint_data = {
    ...
    _create_index_data = {
        'index': None, # DjangoIndex [Model, Properties]
    }
    _alter_model_data = {
        'model': None,
        'properties': None
    ...
    def _alter_column_set_null(self, table_name, column_name, is_null):
        # engine_ver = str(
        #     self.connection.connection.wikibase_info['_mediawiki_version']).split('.')
        ...
        # }
        cmd_data = { **self._alter_model_data, **{
            'action': f'{table_name}.{column_name} set_null'
        }}
        return Cmd('alter_model', data=cmd_data)

    def _column_has_default(self, params):
        # column_has_default_sql = """
        ...
        # }
        column_has_default_cmd = Cmd('field_has_default', data={
                                     'table_name': params['table_name'], 'field_name': params['column']})

        res = None
        with self.connection.cursor() as cursor:
            cursor.execute(column_has_default_cmd)
            res = cursor.fetchone()
    ...
    def add_index(self, model, index):
        ...
        #     self.execute(create_statement, params=None)
        cmd_data = {**self._create_index_data, **{
            'index': DjangoIndex(model, DataDict(index))
        }}
        self.execute(Cmd('create_index', data=cmd_data), None)
    ...
    def create_unique(self, create_statement):
        ...
        """
        cmd_data = {**self._create_constraints_data, **{
            'constraints': [create_statement.template],
        }}
        self.execute(Cmd('add_constraints', data=cmd_data))
    ...
    def _create_fk_sql(self, model, field, constraint_name):
        cmd_data = {**self._create_foreignkey_constraint_data, **{
            'constraint_name': constraint_name,
            'model': DjangoModel(model),
            'property': DjangoProperty(field),
        }}
        return Cmd('create_foreignkey_constraint', data=cmd_data)
    ...
    def add_field(self, model, field):
        ...
        cmd_data = {**self._add_property_data, **{
            'model': DjangoModel(model),
            'property': DjangoProperty(field),
        }}
        self.execute(Cmd('add_property', data=cmd_data), params)
        ...
        if not self.skip_default(field) and self.effective_default(field) is not None:
            ...
            self.execute(Cmd('alter_property', data=cmd_data), params)
    ...
    def remove_field(self, model, field):
        ...
        cmd_data = {**self._remove_property_data, **{
            'model': DjangoModel(model),
            'property': DjangoProperty(field),
        }}
        self.execute(Cmd('remove_property', data=cmd_data))
        ...
        if new_type == self.connection.data_types['TextField'] or new_type == self.connection.data_types['BinaryField']:
            alter_blob_actions.append(
                (self.sql_create_column % {
                    ...
                    "column": self.quote_name("migrate_temp_" + new_field.column),
                    "definition": new_type,
                }, [])
            )
            alter_blob_actions.append(
            ...
            alter_blob_actions.append(
                (self.sql_rename_column % {
                    "table": self.quote_name(table),
                    "old_column": self.quote_name("migrate_temp_" + new_field.column),
    ...
    def _alter_field(self, model, old_field, new_field, old_type, new_type,
        ...
        if old_field.column != new_field.column:
            # TODO: support update for rename
        ...
        if actions or null_actions:
            ...
            if self.connection.features.supports_combined_alters and actions:
                cmd, params = tuple(zip(*actions))
                actions = [(", ".join(cmd), sum(params, []))]
            # Apply those actions
            for cmd, params in actions:
                ...
                if isinstance(fragment, list):
                    ...
                    self.execute(Cmd('alter_model', data=cmd_data))
                else:
                    # self.execute(
            ...
            if four_way_default_alteration:
                ...
                # implement prepare_default
                if self.connection.features.requires_literal_defaults:
                    # FIXME: Implement updates (as request to the sparql => get entities ids => set claims)
                    # self.execute(
                    #     self.sql_update_with_default % {
                    #         "table": self.quote_name(model._meta.db_table),
                    #         "column": self.quote_name(new_field.column),
                ...
                # now
                for cmd, params in null_actions:
                    self.execute(
                        self.sql_alter_column % {
                            "table": self.quote_name(model._meta.db_table),
                            "changes": cmd,
        ...
        if post_actions:
            for cmd, params in post_actions:
                self.execute(cmd, params)
        # Added a unique?
        ...
        for old_rel, new_rel in rels_to_update:
            ...
            #     fragment[1],
            # )
            for cmd, params in other_actions:
                self.execute(cmd, params)
        ...
        if needs_database_default:
            ...
            #     }
            # }
            cmd_data = {**self._alter_property_data, **{
                'model': DjangoModel(model),
                'property': DjangoProperty(new_field)
            }}
            self.execute(Cmd('alter_property', data=cmd_data))
        # Reset connection if required
        if self.connection.features.connection_persists_old_columns:
            self.connection.commit()
    ...
    def create_model(self, model):
        ...
        cmd_data = {**self._create_model_data, **{
            'model': DjangoModel(model)
        }}
        self.execute(Cmd('create_model', data=cmd_data), None)
    ...
    def create_index(self, statement):
        ...
        #     self.execute(statement, params=None)
        cmd_data = {**self._create_index_data, **{
            'index': None, # TODO: DjangoIndex(model, DataDict(index))
        }}
        self.execute(Cmd('create_index', data=cmd_data), None)
    ...
    def sequence_exist(self, table):
        ...
        # """ % seq_name
        sequence_exists_cmd = Cmd('sequence_exists', data={
                                  'sequence_name': seq_name})
        value = None
        with self.connection.cursor() as cursor:
            cursor.execute(sequence_exists_cmd)
