...
"""
# typical way to plot violin 
def violinPlot(data,indx,axV,fc):
    quartile1_, medians_, quartile3_, whiskers_ = misc.Quartiles_Whiskers(data) 
    bp = axV.violinplot(data, positions = [indx], widths = 0.2,showmedians=False,showextrema=False,points=len(data))
    axV.scatter(indx, medians_, marker='o', color='white', s=5, zorder=3)
    axV.vlines(indx, quartile1_, quartile3_, color='k', linestyle='-', lw=3)
    axV.vlines(indx, whiskers_[0], whiskers_[1], color='k', linestyle='-', lw=1)
...
fig, ax = misc.create_fig(figsize=[3,3],nrows=2,ncols=1,hspace=0.02,left=0.25,right=0.99,bottom=0.25,top=0.98)
ax[0].set_ylabel(r"Widths ($\mu$m)",labelpad=-1)
ax[1].set_ylabel(r"Lengths ($\mu$m)",labelpad=-1)
ax[0].set_xticks([]) 
ax[1].set_xticks([1,1.3,1.6])
ax[1].set_xticklabels(["C1","C2","C3"])
    
violinPlot(Inc_data["length"],1,ax[1],colors[0])
violinPlot(RT_data["length"],1.3,ax[1],colors[1])
violinPlot(Lu_data["length"],1.6,ax[1],colors[2])

violinPlot(Inc_data["width"],1,ax[0],colors[0])
violinPlot(RT_data["width"],1.3,ax[0],colors[1])
violinPlot(Lu_data["width"],1.6,ax[0],colors[2])

fig.savefig("CT_FIRE_violinplot.pdf",dpi=fig.dpi*2,transparent=True) 
