...
def HT2D(imgRaw,verbose=False,returnSimple=False,subtract_mean=True,zero_pad=True,**kwargs):
    ...
    **kwargs: arguments 
    ...
    if verbose:
        ...
        ax2[1,1].imshow(np.abs(Pimg[:xSizeRaw,:ySizeRaw])**2,cmap="gray") 
...
class QPImage(object):
    ...
    def __init__(self,data=None,data_key=None,ref_data=None,meta_data=None,holo_kw=None,bg_kw = None, proc_phase=True,computeBg=True, slices=0, autoRun = True, verbose=False):
        ...
        if ref_data is None:
            ...
            if ref_data.ndim == 3:
                self.ref = np.mean(ref_data,axis=0) 

        # now force "ref" as empty to avoid doing reference subtraction in main procesing. 
        self.holo_kw = {"batchSize":50,"cr":0.5,"trans":False,"ref":np.empty((0,)),"onlyCPU":False, "verbose":verbose,"zero_pad":True,"subtract_mean":True, "returnContrastMat":True}  #
        ...
        if data is not None:
            ...
            if autoRun:
                if data.ndim == 3:
                    if slices == 0:
                        self.out, self.contrast_mat, self.out_int = self._get_amp_pha(data[:,:,:]) 
                    else:
                ...
                if self.subtract_ref:
                    _, tmp_ref, _, _ =  self._get_amp_pha(self.ref)  
                if proc_phase:
                    self.pha = proc_unwrap_phase(tmpPha)
                    if self.subtract_ref:
                        self.ref_out = proc_unwrap_phase(tmp_ref) 
                        del tmp_ref 
                    del tmpPha 
                else:
                    self.pha = tmpPha
                ...
                if self.subtract_ref:
                    self.pha = self.pha - self.ref_out    
                del data 
