...
def assert_less_or_equal(name: str, value, threshold):
    """
    Raises an exception if a given value is not less or equal to a specific threshold.
    ...
    """
    if value > threshold:
        raise AssertionError('Invalid value given for parameter "' + name + '": Must be less or equal to ' + str(
            threshold) + ', but is ' + str(value))


def assert_multiple(name: str, value, other):
    """
    Raises an exception if a given value is not a multiple of another value.
...
def assert_not_none(name: str, value):
    """
    Raises an exception if a given value is None.

    :param name:    The name of the parameter, the value corresponds to
    :param value:   The value
    """
    if value is None:
        raise AssertionError('Invalid value given for parameter "' + name + '": Must not be None')
