...
def oui_txt_parser(content: str, /, *, registry: OuiRegistry = None) -> list:
    _rows = []
    try:
        for mg in content.split('\n\n')[1:]:
            ...
            address_new = []
            for a in splitlines[2:]:
                address_new.append(a.strip())
            row = [registry.value, assignment, organization] + address_new + []
            _rows.append(row)
        _rows.sort()
