...
class Encoder(nn.Module):
    ...
    def forward(self, bert_input, form_chars, lemma_chars, to_scatter, n_words, frameworks):
        ...
        if not self.training and batch_size > 1 and 'xlm' in self.bert.name_or_path:
            # See https://github.com/huggingface/transformers/issues/2401#issuecomment-572544487
            hidden_states = []
            for t, m, l in zip(tokens, mask, mask.sum(1)):
                encoded = self.bert(t[:l].unsqueeze(0), attention_mask=m[:l].unsqueeze(0)).hidden_states[1:]
                hidden_states.append(encoded)
            hidden_states = [torch.cat(x, dim=0) for x in hidden_states]
            L, T, H = hidden_states[0].shape
            encoded = torch.empty((L, batch_size, T, H), device=tokens.device, dtype=torch.float)
            for i, h in enumerate(hidden_states):
                encoded[:, i, :h.size(1), :] = self.encoded_layer_norm(h)
        else:
