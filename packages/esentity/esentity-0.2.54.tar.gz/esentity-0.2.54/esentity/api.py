...
class ApiManagerComplaints(Resource):
    ...
    def post(self):
        ...
        def process_casino(_doc):
            return f"{current_app.config['PREFERRED_URL_SCHEME']}://{current_app.config['DOMAIN']}{get_page_url(_doc)}"

        return {
            'ok': True, 
            'items': [process_activity(item) for item in items], 
            'casinos': {casino['_id']: process_casino(casino['_source']) for casino in casinos}
        }
