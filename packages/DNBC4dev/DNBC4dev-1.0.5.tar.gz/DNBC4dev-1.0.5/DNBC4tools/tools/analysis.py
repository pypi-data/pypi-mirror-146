...
def parse_analysis(parser):
    ...
    parser.add_argument('--matrix',metavar='DIR',required=True,help='Count matrix dir,contain barcodes.tsv,features.tsv,matrix.mtx')
    parser.add_argument('--qcdim',metavar='INT',type=int,default=20,help="DoubleFinder's PCs parameter, the number of significant principal components, default is 20.")
    parser.add_argument('--clusterdim',metavar='INT',type=int,default=20,help='The principal components used for clustering, default is 20.')
    parser.add_argument('--doubletpercentage',metavar='FLOAT',type=float,default=0.05,help='Assuming doublet formation rate, tailor for your dataset, default is 0.05')
    parser.add_argument('--mitpercentage',metavar='INT',type=int,default=5,help='Filter cells with mtgenes percentage, default is 5',)
    parser.add_argument('--mtgenes',metavar='LIST',default='False',help='set mitochondrial genes(mtgene list file path) or Homo or False, default is False')
    parser.add_argument('--minfeatures',metavar='INT',type=int,default=200,help='Filter cells with minimum nfeatures, default is 200.',)
    parser.add_argument('--PCusage',metavar='INT',type=int,default=50,help='The total number of principal components for PCA, default is 50')
    parser.add_argument('--resolution',metavar='FLOAT',type=float,default=0.5,help='Cluster resolution, default is 0.5',)
