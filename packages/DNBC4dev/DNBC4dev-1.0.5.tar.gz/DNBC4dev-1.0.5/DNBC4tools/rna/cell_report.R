...
sa <- read.table( gzfile(args$inputsaturation),header = TRUE)
saturation <- tail(sa,1)[1,7]*100
cat(paste("Sequencing Saturation,", saturation, "%\n",sep=""),file=paste(args$output,"/cellCount_report.csv",sep=""), append=T)
