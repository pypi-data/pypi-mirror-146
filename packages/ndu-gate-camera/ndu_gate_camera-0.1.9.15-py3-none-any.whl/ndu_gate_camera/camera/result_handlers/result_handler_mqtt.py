...
class ResultHandlerMqtt:
    ...
    def __init__(self, access_token, mqtt_obj, config_file, plate_service_url, use_platform):
        ...
        self.url = ""
        self.http_topic = mqtt_obj.get("http_topic", "api/v1/")
    ...
    def send_frame_http(self, frame, device):
        ...
        try:
            ...
            if self.url == "":
                r = requests.get("http://" + self.host)
                self.url = r.url
            response = requests.post(self.url + self.http_topic + self.access_token + "/attributes",
                                     headers=headers, data=data)
            # print(response)
        except Exception as e:
            print(e)
