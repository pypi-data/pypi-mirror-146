...
RE_ObjectToFetch = re.compile(r"^ObjectToFetch\((.*)\)$")
...
class BinType(BaseType):
    def p_type(self, string: bytes):
        return string
...
class TupleType(BaseType):
    ...
    def __init__(self, name: str, **kwargs):
        ...
        if len(RE_TUPLE.findall(name)) != 0:
            tps = RE_TUPLE.findall(name)[0]
        else:
            tps = RE_ObjectToFetch.findall(name)[0]
        find_end = False
        new_tps = []
        for tp in tps.split(","):
            if find_end:
                substring+=tp
            if str(tp).find("Enum") != -1:
                substring = tp
                find_end = True
            if str(tp).find(")")!=1:
                find_end = False
                new_tps.append(substring)
            else:
                new_tps.append(tp.rpartition(" ")[2])
        self.types = tuple(
            what_py_type(tp, container=True) for tp in new_tps
        )
