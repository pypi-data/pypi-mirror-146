...
class Neo4jHelper:
    ...
    async def parse_relation_data(self, cql, result_list, id_key="id", name_key="name", database="neo4j"):
        data = []
        num = 0
        stock_strike = True
    ...
    async def create_node(self, node_name: str, table_name: str, node_property: dict = {}, database=""):
        sql = f"CREATE ({node_name}:{table_name} {json.dumps(node_property)})"
        result = await self.execute_fetchall(sql, database=database)
        return result

    async def create_node_relationship(self, relationship_list: list, database=""):
        sql = f"CREATE "
        sql += ",".join(relationship_list)
        result = await self.execute_fetchall(sql, database=database)
        return result

    async def delete_node(self, node_list: list, database=""):
        sql = f"DELETE "
        sql += ",".join(node_list)
        result = await self.execute_fetchall(sql, database=database)
        return result

    async def delete_property(self, node: str, property_node: dict, node_property: str, database=""):
        sql = f"MATCH ({node}:{json.dumps(property_node)}) REMOVE {node}.{node_property} RETURN {node}"
        result = await self.execute_fetchall(sql, database=database)
        return result

    async def update_node(self, node_name: str, table_name: str, node_property_dict: dict, database=""):
    ...
    async def query_all(self, node_name: str, table_name: str, node_property_dict: dict = {}, database=""):
        node_property_str = ",".join(
            [f"{node_name}.{property_name}={property_value}" for property_name, property_value in
             node_property_dict.items()])
    ...
    async def query_count(self, sql, database=""):
        result = await self.execute_fetchall(sql, database=database)
        return result

    async def query_page(self, node_name: str, table_name: str, node_property_dict: dict = {}, database="",
    ...
    async def execute_fetchall(self, sql, database):
        WorkspaceConfig.database = database
        async with self.pool.session() as session:
            tx = await session.begin_transaction()
    ...
    def sync_query_count(self, sql, database=""):
        result = self.sync_execute_fetchall(sql, database=database)
        return result

    def sync_query_page(self, node_name: str, table_name: str, node_property_dict: dict = {}, database="",
                        page_num: int = 1, page_size: int = 10):
        node_property_str = ",".join(
            [f"{node_name}.{property_name}={property_value}" for property_name, property_value in
             node_property_dict.items()])
    ...
    def sync_execute_fetchall(self, sql, database):
        WorkspaceConfig.database = database
        with self.pool.session() as session:
            with session.begin_transaction() as tx:
                result = tx.run(sql)
