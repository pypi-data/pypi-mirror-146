...
class Neo4jHelper:
    ...
    def sync_create_node(self, node_name: str, table_name: str, node_property: dict = {}):
        sql = f"CREATE ({node_name}:{table_name} {json.dumps(node_property)})"
        result = self.sync_execute_fetchall(sql)
        return result
    ...
    def sync_delete_node(self, node_list: list):
        sql = f"DELETE "
        sql += ",".join(node_list)
        result = self.sync_execute_fetchall(sql)
        return result

    def sync_delete_property(self, node: str, property_node: dict, node_property: str):
        sql = f"MATCH ({node}:{json.dumps(property_node)}) REMOVE {node}.{node_property} RETURN {node}"
        result = self.sync_execute_fetchall(sql)
        return result

    def sync_update_node(self, node_name: str, table_name: str, node_property_dict: dict):
        node_property_str = ",".join(
            [f"{node_name}.{property_name}={property_value}" for property_name, property_value in
             node_property_dict.items()])
        sql = f"MATCH ({node_name}:{table_name}) SET {node_property_str} RETURN {node_name}"
    ...
    def sync_query_all(self, node_name: str, table_name: str, node_property_dict: dict = {}):
        node_property_str = ",".join(
            [f"{node_name}.{property_name}={property_value}" for property_name, property_value in
             node_property_dict.items()])
        sql = f"MATCH ({node_name}:{table_name}) "
