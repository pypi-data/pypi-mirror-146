...
class SymbolicRegressor:
    def __init__(self,
        ...
        self.function_set = [function_map[f] for f in function_set] if function_set else list(function_map.values())
    ...
    def __build(self):
        for i in range(self.population_size):
            self.trees.append(SyntaxTree(id=i,
    ...
    def __evolve(self):
        offsprings = []
        method_probs = [self.p_crossover, self.p_subtree_mutate, self.p_hoist_mutate, self.p_point_mutate]
        method_probs = np.cumsum(method_probs)
        if method_probs[-1] > 1:
            raise ValueError('sum of crossover and mutation probabilities should no more than 1.0')
    ...
    def fit(self, X:pd.DataFrame, y:pd.Series):
        ...
        for i in range(self.generations):
            ...
            self.best_estimator = self.trees[np.nanargmax(self.metric.sign * self.fitness)]
            self.best_fitness = self.metric.sign * np.nanmax(self.metric.sign * self.fitness)
            self.__log(i)
