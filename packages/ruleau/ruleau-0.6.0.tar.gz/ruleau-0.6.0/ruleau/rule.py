...
class Rule:
    def __init__(
        ...
        info = Rule._get_relevant_call_stack_info()
    ...
    def _get_relevant_call_stack_info():
        ...
        stack = inspect.stack()
        for frame in stack:
            # The most helpful file to report is after rule.py and optionally,
            # decorators.py
            filename = frame.filename
            if filename.endswith("/rule.py") or filename.endswith("\\rule.py"):
                found_rule_py = True
                continue
            if found_rule_py and not filename.endswith("decorators.py"):
