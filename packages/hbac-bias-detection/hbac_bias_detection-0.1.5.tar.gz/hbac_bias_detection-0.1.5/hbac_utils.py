...
def pca_plot(data, title, alpha):
    ...
    pca_features = data.drop(['scaled_errors', 'predicted_value', 'true_value', 'errors', 'clusters', 'new_clusters'], axis=1)
    ...
    scatterplot = sns.scatterplot(data=temp_dataset, x='Principal Component 1', y='Principal Component 2', alpha=alpha, hue="clusters", size='errors', sizes=(1, 200), palette="tab10")
    scatterplot.set_title(title)
    scatterplot.legend(loc='center left', bbox_to_anchor=(1.0, 0.5), ncol=1)
    # plt.savefig('germancredit_kmeans.pngermancredit_kmeansg', dpi=300) # for saving a plot
    plt.show()

def initialize_dataset(raw_data, error_scaling_factor,with_errors=True, just_features=False, scale_features=True, with_classes=False):
    ...
    features = new_data.drop(['predicted_value', 'true_value', 'errors'], axis=1)
    if with_errors:
        scaling_factor = error_scaling_factor
        new_data['scaled_errors'] = StandardScaler().fit_transform(raw_data['errors'].values.reshape(-1, 1))
        new_data['scaled_errors'] = new_data['scaled_errors'] * scaling_factor

    if just_features:
        new_data = new_data.drop(['predicted_value', 'true_value', 'errors'], axis=1)

    if scale_features:
            to_scale = raw_data.drop(['predicted_value', 'true_value', 'errors'], axis=1).columns
            new_data[to_scale] = StandardScaler().fit_transform(features[to_scale])

    if with_classes:
        if MODEL_MODE == 'regression':
            to_scale = raw_data.columns
            new_data[to_scale] = StandardScaler().fit_transform(raw_data[to_scale])
        else:
            for col in ['predicted_value', 'true_value', 'errors']:
...
def get_random_cluster(clusters):
    ...
    print('This is the random cluster we have picked:', result) if print_mode else ''
    return result
...
def bias_acc(data, cluster_id, cluster_col):
    ...
    if len(cluster_x) ==0:
        print("This is an empty cluster! cluster ", cluster_id) if print_mode else ''
    remaining_clusters = data.loc[data[cluster_col] != cluster_id]
    if len(remaining_clusters) == 0:
        print("This cluster is the entire dataset. Cluster: ", cluster_id) if print_mode else ''
...
def get_next_cluster(data):
    ...
    for i in data['clusters'].unique():
        ...
        if variance_cluster > highest_variance:
            ...
            cluster_number = i
            print('--> Cluster with the highest variance:', cluster_number) if print_mode else ''
...
def get_max_negative_bias(fulldata, function=bias_acc):
    ...
    max_neg_bias = 999999
    # print('potential new clusters: ', fulldata['new_clusters'].unique()) if print_mode else ''
    for cluster_number in fulldata['new_clusters'].unique():
        if cluster_number == -1: #Outliers in DBScan
            continue
    ...
    print('max discriminating bias is:', max_neg_bias) if print_mode else ''
    return max_neg_bias

def get_max_bias_cluster(fulldata, printing=False, function=bias_acc):
    ...
    for cluster_number in fulldata['clusters'].unique():
        ...
        print(f"{cluster_number} has bias {current_bias}") if printing else ''
        if current_bias < max_neg_bias:
            max_neg_bias = current_bias
            best_cluster = cluster_number
    print('cluster with the highest discriminating bias:', best_cluster) if printing else ''
