...
#%%
def command_resistance_range(address,channel,mode,excitation,range_value,autorange,autoexcitation):
...
#%%
def query_resistance_range(address,channel,mode,excitation,range_value,autorange,autoexcitation):
...
#%%
def command_channel_parameter(address,channel,channel_status,dwell,pause,curve_number,coefficient):
...
#%%
def command_temperature_control_parameters(address,channel,filtering,unit,delay,current_power,htr_limit,htr_resistance):
    ...
    instru.write('CSET ' + channel + ',' + filtering + ',' + unit + ',' + delay + ',' + current_power + ',' + htr_limit + ',' + htr_resistance)
    return 
...
def query_intensity_output(address):
    ...
    heater_range = Lakeshore370.query_heater_range
    heater_percentage = Lakeshore370.query_heater_output
