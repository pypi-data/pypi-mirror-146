...
class AlphaBacktest:
    ...
    def set_backtest_config(self,config:Dict):
        ...
        self.config=config
    def buy_stock(self,stock,price,volume):
        
        self.position_volume[stock]+=volume
        self._principal-=volume*price*(1+self._buy_cost_fee)

    def sell_stock(self,stock,price,volume):
        self._principal+=volume*price*(1-self._sell_cost_fee)
        if self.position_volume[stock]==0:
            del self.position_volume[stock]
    def get_net_value(self,close_df):
...
#             print(short_index_return)
    ...
    def rebalance(self,close_df,hold_num):
        # hold_num=len(stock_list)
        average_money=self._net_value/hold_num
        for stock in list(self.position_volume.keys()):
            c_p=close_df[stock]
            difference_money=average_money-self.position_volume[stock]*c_p
            if difference_money>0:
                self.buy_stock(stock,c_p,difference_money/c_p//100*100)
            else:
                self.sell_stock(stock,c_p,(-difference_money/c_p)//100*100)
                
    def run(self,position_details:pd.DataFrame,open_price:Dict[str,pd.DataFrame],close_price:Dict[str,pd.DataFrame],vwap_price:Dict[str,pd.DataFrame],index_price:Dict[str,float],close_rebalance=True)->List[float]:
        """
        ...
        for date in tqdm(position_details):
            ...
            for stock in list(self.position_volume.keys()):
                ...
                if stock not in stock_list:
                    v_p=vwap_df[stock]
                    self.sell_stock(stock,v_p,self.position_volume[stock])
            ...
            for stock in stock_list:
                ...
                if stock not in self.position_volume:
                    ...
                    self.buy_stock(stock,v_p,buy_stock_num)

            """
            self._principal是剩余可用资金，在每天的收盘时算净值
            """
            self._net_value=self.get_net_value(close_df)
            if close_rebalance:
                self.rebalance(close_df,hold_num)
            self._net_value=self.get_net_value(close_df)
